...
For S3, Azure, and GCS, the browser uploads the file directly to storage instead of through Payload. Providers that need their own SDK or multipart behavior return a named instruction handled by their Admin helper. Payload then reads the stored file during document creation, so normal validation, image processing, and hooks still run.

If upload instructions are not enabled, the Admin sends the file through Payload with the normal create or update request.
...
| `clientUploads`  | Upload directly to UploadThing instead of through Payload. |               |
