...
Storage adapters are registered under the top-level `storageAdapters` key in your Payload config. This placement ensures that upload hooks, static file handlers, and presigned-URL endpoints are wired up before any plugin modifies the config.
...
<Banner type="info">
  **Migrating from an older version?** If you previously placed storage adapter
  packages inside `plugins`, run the codemod to migrate automatically: ```sh npx
  @payloadcms/codemod --transform migrate-storage-adapters-to-config ```
</Banner>
...
export default buildConfig({
  plugins: [
    // cloudStoragePlugin is a low-level building block — place it in plugins, not storageAdapters.
    // The built-in adapter packages (s3Storage, gcsStorage, etc.) wrap this internally
    // and are used via storageAdapters instead.
    cloudStoragePlugin({
      collections: {
        'my-collection-slug': {
