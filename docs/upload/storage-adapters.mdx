...
| `prefix`             | Base prefix prepended to all file keys. Use for organizational namespacing (e.g., multi-environment or multi-project setups) |                               |
...
### Configuration Options#r2-configuration

| Option        | Description                                                                                                                  | Default |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------- | ------- |
| `enabled`     | Whether or not to enable the plugin                                                                                          | `true`  |
| `collections` | Collections to apply the R2 adapter to                                                                                       |         |
| `bucket`      | The R2 bucket binding from Cloudflare Workers environment                                                                    |         |
| `prefix`      | Base prefix prepended to all file keys. Use for organizational namespacing (e.g., multi-environment or multi-project setups) |         |
...
| `prefix`             | `string`                            | Base prefix prepended to all file keys across all collections. Use for organizational namespacing (e.g., multi-environment or multi-project setups). See [Prefix Namespacing](#prefix-namespacing).           |
...
| `prefix`                      | `string`                                                                                                          | Collection-specific prefix appended after the adapter-level prefix. Set to `media/images` to upload files inside `media/images` folder. See [Prefix Namespacing](#prefix-namespacing) for how prefixes combine. |

## Prefix Namespacing

Storage adapters support two levels of prefix configuration:

1. **Adapter-level prefix** - Set at the top level of the storage adapter configuration. Applied to all collections using that adapter.
2. **Collection-level prefix** - Set per-collection within the `collections` object. Applied only to that specific collection.

When both are specified, they are joined with a `/` separator. The final key format is: `{adapter-prefix}/{collection-prefix}/{filename}`.
...
export default buildConfig({
  collections: [Media, Documents, ProfileImages],
  plugins: [
    s3Storage({
      prefix: 'my-project', // Adapter-level: applied to all collections
      bucket: process.env.S3_BUCKET,
      collections: {
        media: true, // Keys: my-project/filename.png
        documents: {
          prefix: 'docs', // Keys: my-project/docs/filename.pdf
        },
        'profile-images': {
          prefix: 'users/avatars', // Keys: my-project/users/avatars/filename.jpg
        },
      },
      config: {
        credentials: {
...
- **Multi-environment setups** - Use `prefix: process.env.NODE_ENV` to separate files by environment (development, staging, production) while sharing a single bucket
- **Multi-project setups** - Use `prefix: 'project-name'` to share a bucket across multiple Payload projects
- **Organizational namespacing** - Keep files organized within your bucket without modifying collection-level prefixes
