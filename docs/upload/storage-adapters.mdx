...
- Configure `cacheControl` per-collection in `collections` to define the `Cache-Control` header for uploaded files. This is especially useful when `disablePayloadAccessControl` is enabled and the S3-compatible storage service does not set a cache control header by default.
