...
| Uploadthing _(deprecated, removed in v5)_ | [`@payloadcms/storage-uploadthing`](https://github.com/payloadcms/payload/tree/main/packages/storage-uploadthing) |
| R2                                        | [`@payloadcms/storage-r2`](https://github.com/payloadcms/payload/tree/main/packages/storage-r2)                   |

## Configuration

Storage adapters are registered under the top-level `storage` key in your Payload config. This placement ensures that upload hooks, static file handlers, and presigned-URL endpoints are wired up before any plugin modifies the config.
...
<Banner type="warning">
  **Deprecated**: The Uploadthing storage adapter is deprecated and will be
  removed in Payload v5. Migrate to another storage adapter such as
  [S3](#s3-storage) before upgrading.
</Banner>
