...
Multipart requests default to `limits.fileSize: 20 * 1024 * 1024` (20 MiB per file), `limits.files: 3`, `limits.fields: 20`, and `limits.fieldSize: 1024 * 1024` (1 MiB per field). These defaults allow multiple uploads and metadata fields while bounding retained content to 80 MiB per request, before buffer copies, string storage, and parser overhead. Concurrent requests multiply this usage; choose lower limits when appropriate for your deployment.

Individual `limits` overrides preserve the other defaults. Explicit Busboy limit values, including `Infinity`, are respected. File size limits reject with HTTP 413 by default; `abortOnLimit: false` opts into truncated files. Field size, file count, field count, and configured part count limits always reject with HTTP 413. Failed requests cancel body consumption and remove temporary files.

For example, to set a smaller allowed `fileSize` for uploads sent to Payload:
