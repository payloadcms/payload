...
| **`limitHandler`**       | A function which is invoked if the file is bigger than configured limits.                                                                                                          |
...
| **`safeFileNames`**      | Set to `true` to strip non-alphanumeric characters except dashes and underscored. Can also be set to a regex to determine what to strip. Defaults to `false`                       |
