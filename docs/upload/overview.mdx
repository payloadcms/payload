...
##### Image Size Returns Null

By default, if you upload an image that has a smaller width **and** height than the defined image size, the resized image will not be generated.

To allow the image to be enlarged to the defined image size, add `withoutEnlargement: false` to the image size config.

If you do not want the image to be enlarged and **instead** want to return the original image size, add `withoutEnlargement: true`.
