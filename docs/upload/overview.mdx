...
<Banner type="info">
  **Note:** Image resizing (`imageSizes`, `resizeOptions`, and related Sharp
  options) is configured through
  [`@payloadcms/transformer-sharp`](./transformers#payloadcmstransformer-sharp),
  not on the collection's own `upload` config. See [Image Sizes](#image-sizes)
  below.
</Banner>
...
| **`focalPoint`**               | Set to `false` to disable the focal point selection tool in the [Admin Panel](../admin/overview). The focal point selector is only available when a registered transformer configures `imageSizes` or a main-image resize for this collection. [More](#crop-and-focal-point-selector)                                                                            |
...
| **`imageSizes`**               | Set automatically by a registered transformer (such as `sharpTransformer`) — see [File Transformers](./transformers). If specified directly, image uploads will be automatically resized in accordance to these image sizes. [More](#image-sizes)                                                                                                                |
...
<Banner type="info">
  **Note:** Sharp-specific options — `constructorOptions`, `formatOptions`,
  `resizeOptions`, `trimOptions`, and `withMetadata` — are no longer set here.
  They're configured through `sharpTransformer({collections})`. See [File
  Transformers](./transformers#payloadcmstransformer-sharp).
</Banner>
...
| Option                   | Description                                                                                                                                                                        |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`abortOnLimit`**       | A boolean that, if `true`, returns HTTP 413 if a file exceeds the file size limit. If `false`, the file is truncated. Defaults to `true`.                                          |
...
| **`requestSizeLimit`**   | Maximum size in bytes for the complete raw multipart request, including files, fields, headers, and boundaries. Accepts a non-negative safe integer or `Infinity`. Defaults to 50 MiB. Set to `Infinity` to disable this limit. |
...
Multipart requests default to `requestSizeLimit: 50 * 1024 * 1024` (50 MiB for the complete raw request), `limits.fileSize: 20 * 1024 * 1024` (20 MiB per file), `limits.files: 3`, `limits.fields: 20`, and `limits.fieldSize: 1024 * 1024` (1 MiB per field). The request-wide limit counts files, fields, headers, boundaries, and other multipart overhead before content is passed to the parser. It allows two files at the per-file maximum with room for metadata while bounding the complete request independently of the individual Busboy limits. Concurrent requests multiply memory and temporary-storage usage; choose lower limits when appropriate for your deployment.

Individual `limits` overrides preserve the other defaults. A non-negative safe integer `requestSizeLimit` overrides the aggregate cap, and `requestSizeLimit: Infinity` disables it intentionally. Invalid values are rejected before the request body is read. An `undefined` override retains the 50 MiB default. Explicit Busboy limit values, including `Infinity`, are also respected. Request size, field size, file count, field count, and configured part count limits reject with HTTP 413. File size limits reject by default; `abortOnLimit: false` opts into truncated files. Failed requests cancel body consumption and remove temporary files.

For example, to set a 75 MiB request-wide limit and a smaller per-file limit for uploads sent to Payload:
...
export default buildConfig({
  ...
  upload: {
    requestSizeLimit: 75 * 1024 * 1024, // 75 MiB for the complete multipart request
    limits: {
...
If you specify an array of `imageSizes`, Payload will automatically crop and resize your uploads to fit each of the sizes specified by your config.
...
`imageSizes` — along with the rest of Sharp's image-resizing options — is configured through [`@payloadcms/transformer-sharp`](./transformers#payloadcmstransformer-sharp), Payload's official image-resizing transformer:
...
export default buildConfig({
  collections: [
    /* your Media collection, with `upload: true` */
  ],
  upload: {
    transformers: [
      sharpTransformer({
...
`sharpTransformer` writes a Sharp-agnostic projection of each configured `imageSizes` entry (`name`, `admin`, and `generateImageName` only) back onto the collection's own `upload.imageSizes`, so the rest of this section — and the generated `sizes` shape on your documents — is unaffected by where the configuration is authored.
...
Setting `crop: false` and `focalPoint: false` on `sharpTransformer({ collections })` for this collection will disable the respective selector in the [Admin Panel](../admin/overview).
...
When editing a document in an upload-enabled Collection, the Admin Panel renders a preview of the file in the upload side panel. The preview that's used is chosen automatically based on the file's MIME type:

| File type         | Built-in preview                                   |
| ----------------- | -------------------------------------------------- |
| `image/*`         | Image thumbnail (falls back to a file icon)        |
| `video/*`         | Native HTML5 `<video>` player                      |
| `audio/*`         | Native HTML5 `<audio>` player                      |
| `application/pdf` | The browser's built-in PDF viewer (in an `iframe`) |
| Everything else   | Image thumbnail / generic file icon                |

These built-in previews require no configuration.

### Custom File Previews

To render your own preview component in place of the built-in one, use the `upload.admin.components.filePreview` property. It accepts either a single component that renders for every file, or a map keyed by MIME type pattern so you can target specific file types.

#### A single component for all file types
...
export const Media: CollectionConfig = {
  ...
  upload: {
    admin: {
      components: {
        // highlight-start
        filePreview: '/components/MediaPreview#MediaPreview',
        // highlight-end
      },
    },
  },
...
#### A map keyed by MIME type

When `filePreview` is an object, each key is a MIME type pattern. Payload resolves the component to render in the following priority order:

1. **Exact match** — e.g. `application/pdf`
2. **Category wildcard** — e.g. `video/*`
3. **Universal fallback** — `*`

If no pattern matches, Payload renders the default built-in preview for that file.
  ...
  upload: {
    admin: {
      components: {
        // highlight-start
        filePreview: {
          'application/pdf': '/components/PdfPreview#PdfPreview', // exact match
          'video/*': '/components/VideoPreview#VideoPreview', // category wildcard
          '*': '/components/GenericPreview#GenericPreview', // universal fallback
        },
        // highlight-end
      },
    },
...
#### Building a File Preview component

A File Preview component can be a Server or Client Component. It receives the following props describing the current file:

| Prop       | Type     | Description                          |
| ---------- | -------- | ------------------------------------ |
| `fileSrc`  | `string` | Resolved URL of the file (`doc.url`) |
| `filename` | `string` | The file's name                      |
| `filesize` | `number` | The file's size in bytes             |
...
export const MediaPreview: React.FC<UploadFilePreviewClientProps> = ({
  filename,
  fileSrc,
  mimeType,
}) => {
  if (mimeType?.startsWith('video/')) {
    return <video controls src={fileSrc} />
  }

  return <img alt={filename} src={fileSrc} />
}
```

Component paths use the same syntax as other [Custom Components](../custom-components/overview#component-paths) — a string with `#ExportName`, or an object with `path` and `exportName`.
...
Uploading files is supported through the REST API, Local API, and MCP. GraphQL does not support
file uploads.
...
The MCP plugin's `createDocument` and `updateDocument` tools accept files from a URL or as base64 data. Call `getCollectionSchema` first and use only the sources listed in `upload.sources`.

MCP does not expose the Local API's `filePath` option, preventing MCP clients from requesting arbitrary files from the server's filesystem.
...
Ensure that files match the specified mimeTypes or sizes defined for the collection, or custom `hooks`.
...
If any [file transformer](./transformers) is configured, requests for a dynamically-transformed variant of a file (for example, a resized image) run this same `read` access control, with `req.fileTransform` set to `true` for the duration of that one call — see [Access control](./transformers#access-control) for details.
