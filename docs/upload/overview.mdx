...
When uploading an image that has a smaller width **and** height than the defined image size, you have the following options:

1. To not enlarge the image and return null (default behavior)
2. To enlarge the image to the defined size (add `withoutEnlargement: false`)
3. To not enlarge and return the original image (add `withoutEnlargement: true`)

<Banner type="error">
  <strong>Note:</strong>
  <br />
  The image size will return NULL when the uploaded image is smaller than the defined image size.
</Banner>
