...
MCP does not expose the Local API's `filePath` option, preventing MCP clients from requesting arbitrary files from the server's filesystem. Base64 adds roughly 33% overhead, so use it for small local files and prefer a URL when the file is already online. See [MCP uploads](../plugins/mcp#uploading-files) for examples.
