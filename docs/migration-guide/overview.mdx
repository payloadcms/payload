...
2. Copy your existing `payload.config` to the new app, review the Payload Config breaking changes outlined [here](#payload-config) and update as necessary.

3. Copy your existing collections, review the breaking changes outlined [here](#collections) and update as necessary.

4. If you were using any custom components, endpoints, hooks, or plugins, make sure to update them to the new structure. Review the breaking changes outlined [below](#custom-components).
...
2. Update your `payload.config.ts` file to match the new structure. Review the Payload Config breaking changes outlined [here](#payload-config) and update as necessary.
...
5. `buildConfig` should be imported directly from `payload` instead of `payload/config`

6. `sharp` is now an optional dependency

Sharp should only be defined if you have configured image resizing or cropping

7. Use the `custom` property for ***server only*** files.
...
8. The `admin.favicon` property is now `admin.icons` and the types have changed
...
9. The `admin.meta.ogImage` property has been replaced by `admin.meta.openGraph.images`
...
10. The `admin.logoutRoute` and `admin.inactivityRoute` properties have been consolidated into a single `admin.routes` property
...
11. The args of the `livePreview.url` function have changed.

It no longer receives `documentInfo` as an arg, and instead, now has `collectionConfig` and `globalConfig`.


12. The `views.Edit` or `views.Edit.Default` or `views.Edit.Default.Component` properties are no longer of type `AdminViewComponent` like the other views.
...
13. The  `href` and `isActive` functions in the view tab config no longer sends the `match` or `location` arguments.
...
14. Remove any Rate Limiting configuration
...
15. Email functionality has been abstracted out into email adapters.
