...
NOTE: Not all of these changes will be applicable to you, and you may not need to make all of these changes.
...
- `views.Edit` / `views.Edit.Default` / `views.Edit.Default.Component` have a new type
...
:rotating_light: The `collection.admin.hooks` property has been removed.
  ...
  Instead, use the new `beforeDuplicate` field-level hook which takes the usual field hook arguments.
