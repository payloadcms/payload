...
### Public `draft` operation argument replaced by `version` and `action`

There is no v4 compatibility shim for the public `draft` boolean on read and write operations. Reads use `version`. Writes use `action`. `_status` remains on returned documents and in typed write inputs.

The `migrate-version-action-api` transform in `@payloadcms/codemod` rewrites unambiguous Local, REST, GraphQL, and SDK call sites and removes `typescript.strictDraftTypes`. It also rewrites static all-locale publication flags when they have a matching explicit action. It emits notes rather than guessing for dynamic `draft` values, update `draft: false` without a static `_status`, wrapper helpers, and conflicting `draft` / `_status` / `action` combinations.

**Reads**

| v3                       | v4                                                         |
| ------------------------ | ---------------------------------------------------------- |
| omitted / `draft: false` | omitted / `version: 'published'` (default)                 |
| `draft: true`            | `version: 'latest'` (newest draft with published fallback) |
| n/a                      | `version: 'draft'` (newest draft only, no fallback)        |

**Writes** — precedence is explicit `action`, then recognized `_status`, then the operation default. `_status: 'draft'` infers `saveDraft`; `_status: 'published'` infers `publish`; `_status` never infers `unpublish`. Create and duplicate default to `saveDraft`. Update and restore default to `publish`.

All-locale publication uses `locale: 'all'` with an explicit action. The former `publishAllLocales` and `unpublishAllLocales` write options were removed in v4. An omitted update action and `_status: 'published'` inference are rejected when `locale: 'all'`; use `action: 'publish'` or `action: 'unpublish'` to make the all-locale transition explicit. `action: 'saveDraft'` with `locale: 'all'` remains valid.

```ts
// Local API
await payload.find({ collection: 'posts', version: 'latest' })
await payload.create({ collection: 'posts', data: { title: 'Draft' }, action: 'saveDraft' })
await payload.update({ collection: 'posts', id, data: { title: 'Live' }, action: 'publish' })
await payload.update({ collection: 'posts', id, action: 'unpublish' })
await payload.update({ collection: 'posts', id, locale: 'all', action: 'publish', data: {} })
await payload.update({ collection: 'posts', id, locale: 'all', action: 'unpublish' })
...
mutation { updatePost(id: $id, action: saveDraft, data: { title: "Draft" }) { title } }

// SDK
await sdk.find({ collection: 'posts', version: 'latest' })
await sdk.create({ collection: 'posts', data: { title: 'Draft' }, action: 'saveDraft' })
```

`afterChange` receives the resolved `action` (including defaults). It is `undefined` on collections and globals without drafts.

`typescript.strictDraftTypes` is removed with no replacement. Local API and SDK version/action typing is always strict.

Cases the codemod will not rewrite, which you must migrate by hand:

- `draft: shouldSaveDraft` and other dynamic expressions
- Update `draft: false` without a static `_status` (old behavior depended on existing document state)
- Detached options objects and wrappers whose operation cannot be identified
- Conflicting `draft` + `_status` values
- Dynamic or conflicting `publishAllLocales` / `unpublishAllLocales` values, which must be replaced manually with `locale: 'all'` and a matching explicit publication action
- REST URLs and GraphQL strings without enough surrounding operation context

Search leftover operation arguments with:

```sh
rg -n "draft:\\s*(true|false)|[?&]draft=" src
```
...
### Jobs: generic collection access is denied by default

The generated `payload-jobs` collection now denies generic create, read, update, and delete access by default. This affects REST and GraphQL CRUD, plus Local API collection operations that use `overrideAccess: false`. Dedicated operations such as `payload.jobs.queue()`, `payload.jobs.run()`, and `payload.jobs.cancel()` continue to work as before.
...
{
  slug: 'posts',
  documents: [{ data: { title: 'Hello', _status: 'draft' } }],
  action: 'saveDraft',
  depth: 2,
}
```
