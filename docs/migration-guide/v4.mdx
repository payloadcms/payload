...
**Transformer groups now include `CODE` and `LINK`.** The groups exported from `@payloadcms/richtext-lexical/lexical/markdown` (`TRANSFORMERS`, `ELEMENT_TRANSFORMERS`, ...) are now lexical's defaults and no longer match Payload's enabled features. Use the converters instead, which build the transformer list from your editor config:
