...
The file generation helpers now return information that can also be included in CLI JSON output.

The main breaking change affects `generateTypes` with `returnString: true`. It previously returned the generated source directly:

```ts
const generatedTypes = await generateTypes(config, {
  log: false,
  returnString: true,
})
```

It now always returns an object. Read the generated source from `types`:

```ts
const { types: generatedTypes } = await generateTypes(config, {
  log: false,
  returnString: true,
})
```

The result also includes `outputFile` and `written`. `written` is `false` when the generated source is returned without writing a file, or when the existing file is already up to date.

`generateImportMap` now returns `{ outputFile, written }`, and `payload.db.generateSchema()` returns `{ outputFile }`. Code that only awaits these functions and ignores their return values does not need to change.
