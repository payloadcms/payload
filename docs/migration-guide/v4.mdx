...
The response also includes the sanitized `filename` and `clientUploadContext` needed by the existing document creation request.
...
The `data` value is passed from the server instruction to the client handler. The names must match exactly.
