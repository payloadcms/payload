...
### `getLocalizedPaths` and `hasLocalesTable` now require `parentIsLocalized`

`getLocalizedPaths` (exported from `payload`) and `hasLocalesTable` (exported from `@payloadcms/drizzle`) previously accepted an optional `parentIsLocalized` argument. It is now required. Pass `false` when calling at the root of a collection or global — the value is only `true` when the fields being processed are nested inside a localized parent (such as a localized array, block, group, or tab).
...
### Custom database adapters must set `name`

The `name` property on the object returned by a database adapter (the `DatabaseAdapterResult` type, also exported as `DatabaseAdapterObj`) is now required. Payload's official adapters already set this, so no action is needed unless you maintain a custom database adapter — in which case add a `name` to the object your adapter factory returns:

```diff
  export function myAdapter(args): DatabaseAdapterObj {
    return {
      defaultIDType: 'text',
+     name: 'my-adapter',
      init: ({ payload }) => {
