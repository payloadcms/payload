...
Payload request creation now uses `createPayloadReq` for local contexts and `createPayloadReqFromWebRequest` to convert a Web `Request`. Admin context setup is exposed as `getAdminContext`.

| Old name                                | New name                                |
| --------------------------------------- | --------------------------------------- |
| `createLocalReq`                        | `createPayloadReq`                      |
| `CreateLocalReqOptions` (internal type) | `Omit<CreatePayloadReqArgs, 'payload'>` |
...
`getAdminContext` and its cache types are primarily contracts for framework adapters. Custom Root layout adapters must rename their injected `initReq` callback to `getAdminContext`.
...
+const req = await createPayloadReqFromWebRequest({ config, request })
```
...
| `createPayloadRequest`        | `@payloadcms/next/utilities` | `createPayloadReqFromWebRequest` in `payload` |
