...
No fields are added to or removed from user objects at runtime. Payload also adds defensive handling for missing users, sessions, and session `createdAt` values during session handling.
