...
export const createSeedCommand = defineCLICommand({
  name: 'seed',
...
Import the command directly into your Payload config:
...
- Replace `bin` entries with `cli.commands`.
...
Custom commands now appear in `payload --help`, receive Standard Schema validation and Commander help generation, and cannot use the same name or alias as a built-in command. They also appear with a generated JSON Schema in `payload help --json`. Every command accepts the schema object directly through `--input <json|@file|->`. Command names are case-sensitive. Because custom commands must be registered before Commander parses input, the Payload config is now loaded before displaying root CLI help.

See [Custom CLI commands](../configuration/overview#custom-cli-commands) for the complete API.
