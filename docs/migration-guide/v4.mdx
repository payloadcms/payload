...
If you reference any of these components by string path in your Payload config (e.g. `Component: '@payloadcms/next/rsc#DefaultNav'`), update the path string too — then regenerate the import map with `payload generate:importmap`.
...
### `collections` dashboard widget (`CollectionCards`) removed

The built-in `collections` dashboard widget and its `CollectionCards` component have been removed. This widget rendered a grid of collection and global cards on the dashboard, duplicating the information already available in the navigation sidebar. The default dashboard now ships the `activity` (recently viewed documents) widget instead.

This is a breaking change. Anything that relies on the removed widget will fail:

- Configs that reference `CollectionCards` by import or by string path (`@payloadcms/ui/rsc#CollectionCards`, or the pre-v4 `@payloadcms/next/rsc#CollectionCards`) no longer resolve, because the export no longer exists. Remove those references and regenerate the import map with `payload generate:importmap`.
- `defaultLayout` entries with `widgetSlug: 'collections'` no longer match a registered widget and are dropped from the rendered dashboard. Remove them, or replace them with another widget such as `{ widgetSlug: 'activity', width: 'small' }`.
- Saved user dashboard preferences that contain the `collections` widget silently drop it on next render; the rest of the layout is unaffected.

The navigation sidebar continues to provide access to every collection and global, so no navigation capability is lost.
