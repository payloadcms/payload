...
The `migrate-version-action-api` transform in `@payloadcms/codemod` rewrites unambiguous Local, REST, GraphQL, and SDK call sites and removes `typescript.strictDraftTypes`. It emits notes rather than guessing for dynamic `draft` values, update `draft: false` without a static `_status`, wrapper helpers, and conflicting `draft` / `_status` / `action` combinations.
...
// after
{ collectionSlug: 'posts', data: { title: 'Hello' }, action: 'saveDraft', depth: 2 }
```
