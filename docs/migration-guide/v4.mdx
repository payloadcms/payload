...
### Jobs Queue breaking changes ([#17220](https://github.com/payloadcms/payload/pull/17220), [#17441](https://github.com/payloadcms/payload/pull/17441), [#17504](https://github.com/payloadcms/payload/pull/17504), [#17505](https://github.com/payloadcms/payload/pull/17505))

If your project uses the Jobs Queue with a SQL database, use the single predefined `jobs-v4` migration for all v4 Jobs Queue database changes. Run the command for your database adapter:

```sh
# PostgreSQL
pnpm payload migrate:create --file @payloadcms/db-postgres/jobs-v4

# Vercel PostgreSQL
pnpm payload migrate:create --file @payloadcms/db-vercel-postgres/jobs-v4

# SQLite
pnpm payload migrate:create --file @payloadcms/db-sqlite/jobs-v4

# D1 SQLite
pnpm payload migrate:create --file @payloadcms/db-d1-sqlite/jobs-v4
```

The migration replaces `processing` with `processingUntil`, adds `processingToken`, adds the `meta` and indexed `concurrencyKey` columns, creates the `payload-jobs-stats` global table, adds parent task log columns, and backfills task log input before making it required. It leaves stats, metadata, concurrency controls, or parent logging in place if you had already enabled them. It also keeps custom table, schema, and index names. Any old job with `processing: true` receives an expired lease so a jobs runner can recover it.

MongoDB does not require a database migration for these changes.

#### `BaseJob` has been removed
...
#### Stats and metadata are enabled by default
...
#### Concurrency controls and parent task logging are always enabled
...
The `jobs.enableConcurrencyControl` and `jobs.addParentToTaskLog` options have been removed.

#### Task and workflow types have been renamed
...
#### Custom database adapters must implement `updateJobs`

Payload now uses a unique `processingToken` when claiming jobs so multiple workers cannot run the same job. This adds an internal, nullable `processingToken` field to the `payload-jobs` collection.
...
#### Crashed workers are recovered with processing leases
