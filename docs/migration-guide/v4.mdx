...
Client uploads for S3, Azure, and GCS now start through one Payload endpoint instead of an endpoint and Admin handler registered by each adapter:

```text
POST /api/upload-instructions
```

If you only configure a storage adapter with `clientUploads: true`, no migration is needed. The Admin Panel uses the new endpoint automatically.

`upload.limits.fileSize` is now checked before Payload returns upload instructions. This is new for Azure, GCS, R2, and Vercel Blob; S3 already enforced this limit. If these client uploads previously exceeded the server limit, increase `upload.limits.fileSize` or Payload will reject them before the upload begins.

You need to migrate if you call an adapter's signing endpoint directly, import its client component, use `ClientUploadsAccess`, or register a custom upload handler with `setUploadHandler`.

#### Provider-specific signing endpoints were removed

The following endpoints no longer exist:

- `/api/storage-s3-generate-signed-url`
- `/api/storage-azure-generate-signed-url`
- `/api/storage-gcs-generate-signed-url`

Call `/api/upload-instructions` with the same file metadata:
...
The old endpoints returned `{ docPrefix, filename, url }`. The shared endpoint returns upload instructions. For S3 and GCS, follow the returned HTTP request exactly:
...
The response includes a complete `file` value for the document request. Send it unchanged as a multipart field:
...
The collection comes from `/api/media`, so the `file` value no longer includes `collectionSlug`. The old `clientUploadContext` property is replaced by `uploadReference`. Its presence tells Payload that the original file was uploaded before document creation, and its contents give the adapter any data needed to find it.

For custom adapters, `clientUploadContext` becomes `uploadReference` on the file and static handler parameters. `handleUpload` no longer receives it because files uploaded before document creation are skipped. To detect one of these files, check whether `file.uploadReference` is present.

#### Adapter client components no longer need manual registration

The S3 and GCS client entry points and their exported components were removed:
...
Remove manual registrations of `S3ClientUploadHandler`, `AzureClientUploadHandler`, and `GcsClientUploadHandler`. Payload registers the required Admin upload handler automatically when `clientUploads` is enabled.

#### `ClientUploadsAccess` was replaced

Use the core `UploadInstructionsAccess` type for `clientUploads.access` callbacks:
...
#### Upload access moved to the generator or endpoint

Access now runs where the upload is authorized. Signed URL and token generators check access before returning credentials. Named provider flows such as R2 and UploadThing check access in their supporting endpoint.

Built-in adapters still accept `clientUploads.access` and pass it to the correct generator or endpoint. Custom adapters should do the same:
...
#### Custom client upload handlers now use named instructions

`useUploadHandlers().setUploadHandler` was removed. `createClientUploadHandler` now requires a name and listens for a matching `dispatch` instruction:
...
The adapter must return the same name from its `uploadInstructions` capability:
...
#### `initClientUploads` was removed

Custom cloud adapters now define their complete direct-upload setup in `uploadInstructions`. The cloud-storage plugin registers the optional Admin handler and supporting endpoint automatically.
...
Both additions are optional. Use `adminHandler` when `generate` returns a named `dispatch` instruction, and use `endpoint` when that handler needs a Payload route. If one adapter is used by multiple collections, its endpoint is registered once and the final path is passed to the Admin handler as `endpointPath`.

Handlers made with `createClientUploadHandler` now receive custom `props` instead of `extra`, and `endpointPath` instead of `serverHandlerPath`. The old `enabled` component prop was removed because the handler is only registered when upload instructions are enabled.
