...
| `CreateLocalReqOptions` (internal type) | `Omit<CreatePayloadRequestArgs, 'payload'>` |
...
See [Token Data](../authentication/token-data) for the full token data shape.

### `overrideAccess` now defaults to `false` on Local API operations

Local API operations whose optional `overrideAccess` value previously defaulted to `true` now default it to `false`, so omitting it respects Access Control instead. Operations that do not expose `overrideAccess`, and operations such as `resetPassword` where it was already required or did not default to `true`, are unaffected by this change.
...
| `createPayloadRequest`        | `@payloadcms/next/utilities` | `createPayloadRequestFromWebRequest` in `payload` |
...
### Lexical: vendored `@lexical/markdown` fork removed and lexical upgraded to `0.50.0`

`@payloadcms/richtext-lexical` no longer ships a vendored copy of `@lexical/markdown`. The package is now a direct dependency, every `@lexical/*` package was upgraded from `0.41.0` to `0.50.0`, and `@payloadcms/richtext-lexical/lexical/markdown` is a pure passthrough of `@lexical/markdown`.
...
If you import lexical APIs directly through `@payloadcms/richtext-lexical/lexical/*`, also review the [lexical breaking changes](https://github.com/facebook/lexical/releases) between `0.41.0` and `0.50.0`. None of them require action for typical Payload usage.
...
### Admin UI Publish button now defaults to the active locale, and `localization.defaultLocalePublishOption` has been removed
...
File versioning adds a required `GeneratedAdapter.copyFile` method. It receives `collection`, `req`, complete `from` and `to` storage keys, and an optional `mimeType`. Implement it using the provider's copy operation or a safe authenticated stream: keep the source, reject an occupied destination, and wait until the destination is readable before returning. `moveFile` is optional. This is a 4.0 adapter contract change; custom adapters must implement the new method to support the updated interface. Path-addressed keys cannot be assumed for adapters whose provider assigns opaque object IDs. See [Custom Storage Adapters](../upload/storage-adapters) for the contract and first-party provider requirements.
...
### Sharp moved out of core into `@payloadcms/transformer-sharp`

Image resizing is no longer built into `payload` itself. It now ships as an official [file transformer](../upload/transformers), `@payloadcms/transformer-sharp`, that you register under `upload.transformers`.
...
3. `sharp` is no longer a dependency of `payload` at all (previously a `devDependency`, not a runtime `dependency`). Install `@payloadcms/transformer-sharp` explicitly.
...
Run `npx @payloadcms/codemod --transform migrate-sharp-to-transformer` to migrate automatically. The codemod moves a top-level `sharp` dependency and inline collection Sharp options into `sharpTransformer({ collections })`, appends to an existing `upload.transformers` array if you already have one, and leaves anything it can't safely rewrite in place with a note — for example, a `collections` array built by a function call, or a collection defined outside the `buildConfig` call. Review those manually.

`imageSizes`, `crop`, and `focalPoint` are still readable at `collection.upload.imageSizes`/`.crop`/`.focalPoint` — `sharpTransformer` writes a projection of what you configure back onto the sanitized collection at startup, so the Admin Panel, generated types, and the stored `sizes` shape on your documents are unchanged. This write-back is why `crop`/`focalPoint`/`imageSizes` are not in the list of removed options above — only the Sharp-specific processing options moved.
...
`sharpTransformer` can also resize images at request time (`/api/media/file/photo.png?width=400`). This is new in Payload 4 and **disabled by default**, so a migrated config keeps serving only the original file and your pre-generated `imageSizes`, as in Payload 3.x. The codemod doesn't enable it. To opt in, pass `dynamic: true` or `dynamic: { collections: ['media'] }` — see [Dynamic (request-time) resizing](../upload/transformers#dynamic-request-time-resizing), and [Securing dynamic resizing](../upload/transformers#securing-dynamic-resizing) before enabling it on a publicly readable collection.

### Storage adapters always insert the `prefix` field now — `alwaysInsertFields` removed
...
the same release. See the [payment confirmation guidance](../ecommerce/payments#confirmation-retries) for
...
admin panel. See the [Stripe REST proxy documentation](../plugins/stripe#stripe-rest-proxy) for
...
### Upload `abortOnLimit` now defaults to `true`

The Payload-wide [`upload.abortOnLimit`](../upload/overview#payload-wide-upload-options) option now defaults to `true`. A file that is bigger than the configured `fileSize` limit now returns an HTTP 413 error by default. Previously, the file was truncated to the limit without any error, and the upload continued.
