...
### `lexical` dependency bumped from 0.41.0 to 0.44.0 ([#16705](https://github.com/payloadcms/payload/pull/16705))

`@payloadcms/richtext-lexical` now depends on `lexical` 0.44.0 (previously 0.41.0). Lexical shipped several breaking changes across this minor bump. The majority of users are not affected, but if you maintain custom Lexical nodes, features, or directly import from `lexical` / `@lexical/*` packages, consult the [Lexical changelog](https://github.com/facebook/lexical/blob/main/CHANGELOG.md) for the 0.42.0, 0.43.0, and 0.44.0 releases and update your code accordingly.
