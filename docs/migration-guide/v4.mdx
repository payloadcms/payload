...
The collection comes from `/api/media`, so the `file` value no longer includes `collectionSlug`. The old `clientUploadContext` property is replaced by `directUpload`. Its presence tells Payload that the original file is already in storage, and its contents give the adapter any data needed to find it.

For custom adapters, `clientUploadContext` becomes `directUpload` on the file and static handler parameters. `handleUpload` no longer receives it because directly uploaded originals are skipped. To detect a file that was uploaded before document creation, check whether `file.directUpload` is present.
...
The `data` value is passed from the server instruction to the client handler. The names must match exactly. If the handler returns an object, it becomes `file.directUpload` in the document request.
