...
**Collections and globals are now opt-out.** Drop the `enabled` flag. Every collection and global is exposed by default with the standard CRUD tools (find, create, update, delete); turn individual ones off via `tools: { <op>: false }`.
...
**Custom tools, prompts and resources moved from arrays under `mcp` to top-level records.** Use the new `defineTool` / `defineCollectionTool` / `defineGlobalTool` / `definePrompt` builders. `parameters` (a Zod raw shape) became `input` (a Zod/Valibot schema or raw JSON Schema), and handlers take one object argument instead of positional ones.
...
**Built-in `create` / `update` tool inputs now take a `data` object.** Document fields used to sit at the top level alongside options like `depth`, `draft` and `locale`. They are now nested under `data`. `_status`, `id`, `createdAt` and `updatedAt` are also no longer accepted there; set publish state with a custom tool if you need to.
...
**Auth tools are per-collection and opt-in.** The old `experimental.tools.auth` block is gone. Opt in per collection via `tools: { login: true }`, and the wire name becomes `login<Collection>` (e.g. `loginUsers`).
...
**API keys must be recreated.** The `payload-mcp-api-keys` collection keeps its slug, but its fields changed completely. The old per-collection / per-global / per-tool checkbox fields are now a single `access` JSON field, plus an `overrideAccess` option. Existing keys keep authenticating but lose their permissions (an empty `access` is treated as "allow everything"), so an un-migrated key silently gets full access. Delete your existing API keys after upgrading and create fresh ones.
