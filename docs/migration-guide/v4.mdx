...
### Admin Publish button now defaults to the active locale

`localization.defaultLocalePublishOption` was removed. For collections and globals with localized fields, the main Publish button now always publishes only the currently active locale. "Publish all locales" is available via the secondary button next to it.

Previously, the button defaulted to publishing every locale unless `defaultLocalePublishOption: 'active'` was set, with "Publish in {locale}" as the secondary option. That split is gone — publishing a single locale is now the primary action for every project, and there's no config to change it.
...
Run `npx @payloadcms/codemod --transform remove-default-locale-publish-option` to migrate automatically.

If you had `defaultLocalePublishOption: 'all'` (or left it unset) and relied on the Publish button publishing every locale by default, click the secondary "Publish all locales" button instead, or call `payload.update()`/`payload.updateGlobal()` with `publishAllLocales: true` directly.
