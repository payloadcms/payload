...
The permanent `processing` checkbox has been replaced by an indexed `processingUntil` date. Payload renews this lease while a worker is alive and recovers it through the normal jobs runner after it expires.
