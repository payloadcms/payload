...
The old endpoints returned `{ docPrefix, filename, url }`. The shared endpoint returns upload instructions. For S3, Azure, and GCS, follow the returned HTTP request exactly:
...
#### S3, Azure, and GCS client component exports were removed

These client entry points and their exported components were removed:
...
Remove manual registrations of `S3ClientUploadHandler`, `AzureClientUploadHandler`, and `GcsClientUploadHandler`. Payload's shared Admin upload handler is registered automatically when `clientUploads` is enabled.
