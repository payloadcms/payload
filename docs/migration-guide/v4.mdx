...
The permanent `processing` flag on `payload-jobs` has been replaced by a renewable `processingUntil` lease. If a worker exits, the next jobs runner call recovers the job after its lease expires and applies its existing retry configuration.

Worker updates use a 30-second safety buffer by default, so an update only starts when enough lease time remains for it to finish before recovery. Configure the lease with `jobs.processingLease.duration` and `jobs.processingLease.safetyBuffer`.
