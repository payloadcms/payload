...
If `chunkLargeFiles` was the only property, collapse the object to `clientUploads: true`; otherwise just delete the `chunkLargeFiles` line. Behaviour is unchanged — chunking is always on in v4.
