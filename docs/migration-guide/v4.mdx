...
Run `npx @payloadcms/codemod --transform migrate-force-select` to migrate automatically. The codemod handles object-literal `forceSelect` values; non-literal values, configs that already define a sibling `select`, and nested object literals (which previously deep-merged) are surfaced as notes for manual review.
