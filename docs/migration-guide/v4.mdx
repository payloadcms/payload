...
For custom collection and global tools, do not add `collectionSlug` / `globalSlug` yourself. Payload adds the enabled slug enum to the MCP input schema automatically, and the handler receives the resolved slug as a top-level argument.
