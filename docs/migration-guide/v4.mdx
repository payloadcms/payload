...
`data` is passed to the client handler. If the handler returns an object, Payload uses it as `file.uploadReference`.
...
Both are optional. Custom handler props are now passed as `props`, and the supporting route is available as `endpointPath` instead of `serverHandlerPath`. The `enabled` prop was removed.
