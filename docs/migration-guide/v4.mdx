...
The concrete field component aliases named `<FieldType>FieldClientComponent` and `<FieldType>FieldServerComponent` have been removed. Use the corresponding `<FieldType>FieldClientProps` or `<FieldType>FieldServerProps` type instead.

For a named component, apply the props type through React:
...
Or annotate the function parameter directly:

```diff
- import type { TextFieldServerComponent } from 'payload'
+ import type { TextFieldServerProps } from 'payload'

- const CustomTextField: TextFieldServerComponent = async (props) => {
+ export default async function (props: TextFieldServerProps) {
    // ...
  }
- export default CustomTextField
```

This change covers the main client and server component types for every concrete field type. Admin-view types such as `AdminViewComponent` and `DocumentTabComponent` are tracked separately and are not part of this field-type migration.
