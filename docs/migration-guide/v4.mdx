...
**Custom apps embedding `RootProvider`** — `RootProvider` now requires a `RouterAdapter` prop. Apps using the default Next.js admin layout get this wired automatically. If you assemble `RootProvider` yourself, pass the framework's adapter component — `NextRouterAdapter` on Next.js, or `TanStackRouterAdapter` on TanStack Start:
...
On TanStack Start, import the adapter from `@payloadcms/tanstack-start/client` instead:
