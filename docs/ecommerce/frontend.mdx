...
The provider exposes a `clearSession` method that should be called when a user logs out. This clears all ecommerce session data including cart, addresses, user state, and localStorage cart data.
...
const LogoutButton = () => {
  ...
  const handleLogout = async () => {
    // Perform your logout logic (e.g., call auth API)
