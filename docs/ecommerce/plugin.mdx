...
For now it's quite rudimentary tracking with no integrations to 3rd party services. It will simply add an inventory field to the `products` and `variants` collections and decrement it when an order is placed. The field is named after `fieldName`, so the stock check performed on `initiate` and the decrement performed on `confirm-order` both use the configured field.

Setting `inventory: false` omits the field, skips the stock check on `initiate` and skips the decrement on `confirm-order`. Price validation still runs.
...
It also receives `inventoryFieldName`, the field configured for inventory tracking, or `false` when inventory tracking is disabled.
...
export const defaultProductsValidation: ProductsValidation = ({
  ...
  if (variant) {
    ...
    const variantInventory = inventoryFieldName
      ? variant[inventoryFieldName]
      : undefined

    if (
      variantInventory === 0 ||
      (variantInventory && variantInventory < quantity)
    ) {
      throw new Error(
    ...
    const productInventory = inventoryFieldName
      ? product[inventoryFieldName]
      : undefined
