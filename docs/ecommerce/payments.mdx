...
Stripe confirmation is idempotent per ecommerce transaction: retries return the same order and public `transactionID`. After Stripe validation, core atomically changes the matching transaction from `pending` to `processing`. Only that request creates the order, marks the cart as purchased, decrements inventory, and links the succeeded transaction.
