...
Stripe confirmation is idempotent per ecommerce transaction: retries return the same order and public `transactionID`. After Stripe validation, core atomically changes the matching transaction from `pending` to `processing`. Only that request creates the order, marks the cart as purchased, decrements inventory, and links the succeeded transaction. No additional order field or schema migration is required.

If a transaction is already `succeeded` and has exactly one matching bidirectional order relationship, a retry returns that order without writes. Ambiguous or mismatched relationships fail closed.

Database adapters with transaction support roll back the complete settlement on failure. Without database transactions, the atomic status claim still prevents concurrent winners, but a process crash can leave a transaction in `processing`. Payload does not reset that state automatically because it cannot safely infer whether order, cart, or inventory side effects completed. Monitor and manually reconcile stale `processing` transactions. A future inventory-reservation model can provide stronger recovery semantics.
...
| `data`             | `object`         | The cart associated with the transaction.                                                                                       |
| `finalizeOrder`    | `function`       | Atomically creates or finds the canonical order and applies the cart, inventory, and transaction side effects after validation. |
| `req`              | `PayloadRequest` | The Payload request object.                                                                                                     |
...
The public return must include the following:

| Property        | Type               | Description                                                |
| --------------- | ------------------ | ---------------------------------------------------------- |
| `message`       | `string`           | A success message to be returned to the client.            |
| `orderID`       | `string \| number` | The ID of the canonical order.                             |
| `transactionID` | `string \| number` | The ID of the ecommerce transaction, including on retries. |

> **Warning:** `confirmOrder` requests can be retried or run concurrently. Custom adapters must
> validate the provider payment against the canonical ecommerce transaction, then call
> `finalizeOrder`. The endpoint rejects a successful adapter response unless finalization completes;
> adapters that create orders themselves must migrate to this callback.

PostgreSQL users upgrading an existing project must generate and run a database migration that adds
`processing` to the ecommerce transaction status enum.
...
export const confirmOrder: NonNullable<PaymentAdapter>['confirmOrder'] =
  async ({ data, finalizeOrder, req, transactionsSlug = 'transactions' }) => {
    ...
    try {
      ...
      // Validate amount, currency, cart, customer, and items here before finalizing.
