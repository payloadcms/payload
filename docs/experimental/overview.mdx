...
const config = buildConfig({
  ...
  experimental: {
    ...
    deleteCurrentLocale: true, // highlight-line
  },
})
```
...
| **`deleteCurrentLocale`** | **Boolean.** When `true`, adds a button to the UI to delete data for the current locale while keeping the data in other locales. Defaults to `false`.                                       |
