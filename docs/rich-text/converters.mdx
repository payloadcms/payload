...
Do note that using the rest populate function will result in each node sending a separate request to the REST API, which may be slow for large documents. On the server, you can use the payload populate function, which will be more efficient:
