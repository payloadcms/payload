...
```ts
const defaultEditorConfig = editorConfigFactory.default({ config })
```
...
const customEditorConfig = editorConfigFactory.fromFeatures({
  config,
...
const instantiatedEditorConfig = editorConfigFactory.fromEditor({
  config,
  editor,
})
```
