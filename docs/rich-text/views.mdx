...
For blocks and inline blocks, you can also use `Block` and `Label` to replace the entire block UI or just the label. These receive Lexical-level props and can use the `useBlockComponentContext()` hook for React UI components.

#### Block

The `Block` property replaces the entire block component, including the collapsible header. It receives Lexical-level props:
...

{
  myView: {
    nodes: {
      blocks: {
        myBlockType: {
          // Block receives: formData, nodeKey, editor, config, node, isEditor, isJSXConverter
          Block: ({ formData, nodeKey }: ViewMapBlockComponentProps) => {
            // Use the hook to access React UI components
            const {
...
The `Label` property replaces just the block label in the collapsible header:
...
// Props for Block components in view maps
type ViewMapBlockComponentProps = {
  ...
  isEditor: true // Always true in editor
  isJSXConverter: false // Always false in editor
...
Use this hook inside `Block` or `Label` components to access React UI components:
