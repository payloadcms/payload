...
Here's an overview of all the features included:

// Table with 3 columns: Feature name, included by default, description
...
The `convertSlateToLexical` is the same method used in the `SlateToLexicalFeature` - it handles traversing the Slate JSON for you. Do note that the migration script will have to be adjusted for the specific shape of your document. Especially if you have nested richText fields, or localization enabled, this will have to be adjusted.

### Converting custom Slate nodes

If you have custom Slate nodes, you will have to create a custom converter for them. Here's an example converter (which is already included as part of the default converters):
