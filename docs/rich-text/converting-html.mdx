...
1. **Generate HTML in your frontend (Recommended)**: Convert JSON to HTML wherever you need it, on-demand.
...
To convert JSON to HTML in your frontend, use the `convertLexicalToHTML` function from `@payloadcms/richtext-lexical/html`:
...
If you need to convert raw HTML into a Lexical editor state, use `convertHTMLToLexical` from `@payloadcms/richtext-lexical`, along with the [editorConfigFactory to retriev the editor config](/docs/rich-text/converters#retrieving-the-editor-config):
