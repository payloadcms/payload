...
| **`slug`** | The slug of the Collection being accessed.                                                                                    |
...
If `readVersions` is not configured, Payload uses the Collection's `read` Access Control. Boolean results are preserved, and query constraints are automatically adjusted to target fields within the stored version document.

For `findVersionByID`, the inherited `read` function receives the parent Document's `id`, not the version record's `id`.
...
<Banner type="warning">
  **Note:** When you explicitly configure `readVersions`, returning a
  [Query](../queries/overview) will apply the constraint to the [`versions`
  collection](../versions/overview#database-impact), not the original
  Collection.
</Banner>
