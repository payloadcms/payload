...
Returns a boolean which allows or denies the ability to update a field's value. If `false` is returned, any passed values will be discarded. If a user tires to update a field that they do not have access to, the field data will be ommited from the update operation **without** throwing an error.
