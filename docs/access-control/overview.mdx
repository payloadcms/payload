...
By default, all [Collections](../configuration/collections) and [Globals](../configuration/globals) require that a user is at least logged in before they can interact with your data. To do this, Payload sets a default Access Control function that checks if a user is present on the request. To override this default behavior, you can define your own Access Control functions as needed.
...
<Banner type="warning">
  ...
  In the [Local API](../local-api/overview), all Access Control is _skipped_ by default. This allows your server to have full control over your application. To opt back in, set the `overrideAccess` option to `false` in your request.
</Banner>
