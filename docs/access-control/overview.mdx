...
<Banner type="warning">
  **Important:** [Local API](../local-api/overview) operations with an optional
  `overrideAccess` value respect Access Control by default because it defaults
  to `false`. When an operation acts on behalf of an authenticated user, pass
  that `user` with the request. Set `overrideAccess: true` only when the
  operation should bypass Access Control. See [Local API Access
  Control](../local-api/access-control).
</Banner>

## Base Access Control

Use the top-level `baseAccess` config to apply an additional constraint to
standard Collection and Global Access Control operations:
...
export default buildConfig({
  baseAccess: {
    collections: {
      create: ({ req, slug }) =>
        slug === 'public-posts' || req.user?.role === 'admin',
      delete: ({ req, slug }) =>
        slug === 'public-posts' || req.user?.role === 'admin',
      update: ({ req, slug }) =>
        slug === 'public-posts' || req.user?.role === 'admin',
    ...
    globals: {
      update: ({ req, slug }) =>
        slug === 'public-settings' || req.user?.role === 'admin',
    },
  },
  // ...
})
...
The `collections` and `globals` properties are typed as `CollectionAccess` and
`GlobalAccess`. All Collection and Global Access Control callbacks, including
normal document access, receive the document's `slug`.

Payload combines each configured base function with the corresponding
document-specific function using `AND` semantics. Returning `false` denies the
operation before the document-specific function runs. For operations that
support query constraints, returning one combines it with a document-specific
constraint using an `and` query. Collection `create` access only supports
booleans, so returning a query constraint for that operation throws an error.
Collection `admin` access also only supports booleans.

`baseAccess` applies only to Collection and Global Access Control. It does not
apply to Field Access Control, custom endpoints, direct database adapter calls,
job access, or other APIs with dedicated access configuration.

Like other Access Control, `baseAccess` is skipped when Local API operations use
`overrideAccess: true`.
