...
normal resource access, receive the resource's `slug`.

Payload combines each configured base function with the corresponding
resource-specific function using `AND` semantics. Returning `false` denies the
operation before the resource-specific function runs. For operations that
support query constraints, returning one combines it with a resource-specific
