...
The `collections` and `globals` properties use the same operation names and
return types as normal Collection and Global Access Control. Each callback also
receives the resource's `slug`.
...
apply to an auth Collection's `access.admin` function, Field Access Control,
custom endpoints, direct database adapter calls, job access, or other APIs with
dedicated access configuration.
