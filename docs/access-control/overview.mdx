...
export default buildConfig({
  baseAccess: ({ operation, req }) => {
    if (operation === 'read' || operation === 'readVersions') {
      return true
    }
...
Payload combines `baseAccess` with each Collection or Global Access Control
function using `AND` semantics. Returning `false` denies the operation before
the resource-specific function runs. For operations that support query
constraints, returning one combines it with a resource-specific constraint
using an `and` query. Collection `create` access only supports booleans, so
returning a query constraint for that operation throws an error.

In addition to the standard Access Control arguments, `baseAccess` receives:

- `entityType`: either `collection` or `global`
