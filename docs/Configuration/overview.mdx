...
| `paths`              | Specific paths used to override Payload functionality. Currently used to allow for the customization and theming of the Payload admin panel, although more uses will come over time. [More](/docs/admin/customizing-scss) |
...
<Banner type="warning">
	<strong>Important:</strong><br />If you use an environment variable to configure any properties that are required for the Admin panel to function (ex. serverURL or any routes), you need to extend the Webpack config to allow these variables to appear in the admin panel JS bundle. <a href="/docs/configuration/webpack#whitelisting-environment-variables">Click here</a> for more info.
</Banner>
