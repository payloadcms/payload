...
| ------------ | ------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| `limit`      | `10` when pagination is enabled | Limits the number of documents returned per page. [More details](#limit).                                        |
| `pagination` | `true`                          | Set to `false` to disable pagination calculations and document count queries. This respects any specified limit. |
...
For `find` operations within the Local API, you can disable pagination calculations and document count queries by passing `pagination: false` to the `find` local operation. If you do not specify a `limit`, Payload returns all matching documents. If you specify a positive `limit`, Payload still applies that limit.

You can also set `limit: 0` to return all matching documents and disable pagination automatically.
