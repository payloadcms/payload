...
| `some`               | For non-polymorphic `hasMany` relationship and upload fields. At least one related document must match a nested query.                                                           |
| `none`               | For non-polymorphic `hasMany` relationship and upload fields. No related document may match a nested query.                                                                      |
| `every`              | For non-polymorphic `hasMany` relationship and upload fields. Every related document must match a nested query.                                                                  |
...
Relationship fields can be queried through properties on their related documents. For `hasMany`
relationships, you can also control whether some, none, or every related document must match.
...
For non-polymorphic `hasMany` relationship and upload fields, use `some`, `none`, or `every`
to control how a nested query is applied to the related documents:
...
| Operator | Description                                                |
| -------- | ---------------------------------------------------------- |
| `some`   | At least one related document must match the nested query. |
| `none`   | No related document may match the nested query.            |
| `every`  | Every related document must match the nested query.        |
...
This finds directors who have at least one movie with a rating of 4 or higher that is either a
documentary or featured. The conditions cannot be satisfied by different movies.

An empty relationship does not match `some`, but it does match `none` and `every`.
