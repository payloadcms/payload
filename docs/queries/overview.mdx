...
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `equals`             | The value must be exactly equal. With a nested query on a non-polymorphic `hasMany` relationship, every related document must match.                                             |
| `not_equals`         | The query will return all documents where the value is not equal. With a nested query on a non-polymorphic `hasMany` relationship, no related document may match.                |
...
Relationship fields can be queried through properties on their related documents. You can also
apply a nested query to a non-polymorphic `hasMany` relationship or upload field.

### Nested relationship properties

Use dot notation to query a field on a related document. For example, if a `Song` collection has an
`artists` relationship field, you can query the `featured` field from the related `Artists`
...
const query: Where = {
  // `featured` is the nested property name to filter on
  'artists.featured': {
    exists: true,
  },
}
...
For a `hasMany` relationship, a dot-notation query matches when at least one related document
matches. For example, `'ingredients.isHealthy': { not_equals: false }` matches a burger when at
least one ingredient has an `isHealthy` value other than `false`. A burger with healthy lettuce and
an unhealthy sauce still matches because of the lettuce. Use the nested relationship operators
below when you need to control how many related documents must match.

### Has-many relationship operators

For non-polymorphic `hasMany` relationship and upload fields, pass a nested query to `contains`,
`not_equals`, or `equals` to control how many related documents must match. These examples assume
that `burgers.ingredients` is a `hasMany` relationship and that each ingredient has an `isHealthy`
field:

```ts
import type { Where } from 'payload'

// At least one ingredient is healthy.
const hasHealthyIngredient: Where = {
  ingredients: {
    contains: {
...
// No ingredient is unhealthy.
const hasNoUnhealthyIngredient: Where = {
  ingredients: {
    not_equals: {
...
const hasOnlyHealthyIngredients: Where = {
  ingredients: {
    equals: {
      isHealthy: { equals: true },
    },
...
| Operator     | Description                                                |
| ------------ | ---------------------------------------------------------- |
| `contains`   | At least one related document must match the nested query. |
| `not_equals` | No related document may match the nested query.            |
| `equals`     | Every related document must match the nested query.        |

The value passed to the relationship operator is a normal Payload `where` query. It supports field
operators, deeper relationship queries, and the same `and` and `or` groups as any other Payload
query. All conditions inside one operator are evaluated against the same related document:
...
This finds burgers with at least one healthy ingredient named either `lettuce` or `tomato`. The
conditions cannot be satisfied by different ingredients.

An empty relationship does not match a nested `contains` query, but it does match nested
`not_equals` and `equals` queries, just like JavaScript's `some` and `every` methods.
