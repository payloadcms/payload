...
`not_equals`, or `equals` to control how many related documents must match:
...
The nested query supports the same `and` and `or` groups as any other Payload query. All conditions
inside one operator are evaluated against the same related document:
...
`not_equals` and `equals` queries. Passing a normal value instead of a nested query keeps the
existing behavior of these operators.
