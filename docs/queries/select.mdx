...
<Banner type="warning">
  **Important:** To perform querying with `select` efficiently, Payload
  ...
  regardless of the `select` query you can use the [entity-level
  `select`](#entity-level-select-config) function on your Collection or Global
  config.
</Banner>
...
- Differentiate API requests from admin panel requests to optimize payload size.
