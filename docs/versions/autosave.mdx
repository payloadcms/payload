...
| `allowSaveDraftButton`  | Set this to `true` to show the "Save as draft" button even while autosave is running. By default, the button is hidden when autosave is enabled.                      |
