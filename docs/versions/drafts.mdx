...
Localized `_status` inference uses the active write locale. Publishing or unpublishing every locale requires `locale: 'all'` plus an explicit `action: 'publish'` or `action: 'unpublish'`. An omitted action and `_status: 'published'` inference are rejected for all-locale publication transitions. `action: 'saveDraft'` with `locale: 'all'` remains valid. `autosave: true` is valid only with `saveDraft`.
