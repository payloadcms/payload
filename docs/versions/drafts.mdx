...
At execution time, a scheduled publish [revalidates the latest stored
document](../validation/overview#publishing-and-required-locales). If validation fails, Payload
cancels the job without changing publication state.
