...
When drafts are enabled, the `create` and `update` operations for REST, GraphQL, and Local APIs expose a `draft` parameter that controls validation and where data is written.
...
However, if you pass `draft: true` to the read operation, Payload will return the most recent draft version if one exists. In the scenario above with two draft versions, you'll get the latest (second) draft instead of the published version.
