...
Whether you're building an app or troubleshooting an existing one, follow these guidelines to ensure that it runs as quickly and efficiently as possible—from the API to the Admin Panel.
...
For example, if you are querying for a specific document by an exact slug, you might want to disable pagination.
...
For example, if you are querying for a specific document and can rely on the first match, set `limit: 1`.
...
Making direct database calls can significantly improve performance for certain operations by completely skipping hooks, validations, and other overhead associated with Payload APIs.

For example, this can be especially useful for the `update` operation, where you might otherwise need to make multiple API calls to fetch, update, and fetch again. Making a direct database call can reduce this to a single operation.
...
It is possible for hooks to call themselves recursively, which can lead to memory leaks and crash your server.

This may happen if you make a Payload API call within a hook, which triggers the same hook again.

To prevent this, use the `context` object to track the state of your hooks.

For example, you can set a flag in the `context` object to indicate that a hook has already run:
...
[Full details](../hooks/context).

### Offload long-running tasks

For long running tasks that don't directly affect the API responses, you can offload to the job queue.

To go even further, run your jobs on another server!
...
// This also speeds up auth (permissions) when it goes to build up the obj. because it has less iteration of fields to do.
...
// `push: true` (by default its on in development) only effects relational database, skips migrations. But can make local dev slower if database is far away.
...
// getPayload // use the cache!!

## Bundles!

In your front-end, import directly using the module path.

In Admin Panel, doesn't matter. Otherwise you will duplicate the component and get React errors due to conflicting React.

In server components, be aware of what is being sent to the client. Everything is sent through the network!!!

// bundle analyzer?! maybe mention it. Prefer ESM?

https://nextjs.org/docs/app/guides/package-bundling

## Components

useFormFields instead of useFields

## Server components

// Be weary of making db calls! Duh...

// You can offset some JS execution by using server components instead of client components.

Use streaming too! This will provide fast initial loads.
...
// High memory usage
// High CPU usage
// The best way to debug is to simply
