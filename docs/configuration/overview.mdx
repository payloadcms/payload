...
| `pluralizeSlugs`      | By default, mongoose will pluralize all singular slugs so that they are saved as plural in the database (e.g. if your slug is `post`, it will be saved as `posts`in the database.)   |
