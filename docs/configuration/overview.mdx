...
Use `cli.commands` to add commands to `npx payload`. Custom commands use the same schema-backed API and lifecycle as Payload's built-in commands. The input accepts any [Standard Schema](https://standardschema.dev/) implementation that can generate JSON Schema. Payload exports Zod as a convenient default.
