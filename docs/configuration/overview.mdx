...
Path references are loaded only when the CLI starts, keeping the command and its dependencies out of the Payload config's module graph. You can also import and pass a `CLICommand` directly when that isolation is not needed.
