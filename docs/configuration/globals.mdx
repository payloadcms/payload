...
| `authorship`    | Automatically track the user that created and last updated this Global via polymorphic `createdBy` / `updatedBy` fields. Enabled by default. Set to `false` to disable, or to `{ createdBy: false }` / `{ updatedBy: false }` to toggle each field. [More details](./collections#authorship). |
...
| `editMenuItems`          | Inject custom components within the 3-dot menu dropdown. [More details](../custom-components/edit-view#editmenuitems).                                                                                     |
