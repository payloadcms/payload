...
To publish or unpublish every locale at once, use `locale: 'all'` with an explicit publication `action`. These set the status for all locales the user has access to, regardless of the active locale. An omitted action (or `_status: 'published'` in `data`) cannot be used for an all-locale publication transition:
...
`_status` remains a valid property on returned documents and in write data. If `action` is omitted, a recognized `_status` is used as the write intent; an explicit `action` always wins. This means `_status: 'draft'` can still infer `saveDraft`, while `_status: 'published'` with `locale: 'all'` is rejected because publishing all locales must be explicit.
...
<Banner type="info">
  ...
  document. `locale: 'all'` is still accepted for an explicit publication
  action.
</Banner>
