...
Specify your desired locale by providing the `locale` query parameter directly in the endpoint URL. To return all localization data in one request use `'all'` or `'*'`. The response will be structured so that field values come back as the full objects keyed for each locale instead of the single, translated value.
