...
You can also [validate one or more locales without saving](../validation/overview) and require
specific locales before publishing.
...
| **`required`**       | Require this locale to pass validation whenever any locale is published. Defaults to `false`. [More details](../validation/overview#publishing-and-required-locales). |
...
When [Localization](/docs/configuration/localization) is enabled and a collection or global has **drafts enabled** and contains at least one localized field, Payload automatically localizes the `_status` field. This means publication status is tracked independently per locale, so you can publish in English while keeping Spanish as a draft.

When the status field is localized, it is stored as an object keyed by locale:
...
If localization is not configured, or the collection/global has no localized fields, the `status` field remains a single string (`'draft'` or `'published'`) representing document status across all locales.

### Publishing and unpublishing per locale

When the `_status` field is localized, publishing and unpublishing are scoped to a single locale by default. The locale being acted on is the one provided to the operation (via the `locale` parameter, or the request locale). This means publishing in one locale leaves the other locales' statuses untouched.
...
To publish or unpublish every locale at once, use `publishAllLocales` or `unpublishAllLocales`. These set the status for all locales the user has access to, regardless of the active locale:
...
Passing `locale: 'all'` together with `_status: 'published'` also publishes every locale, and is equivalent to `publishAllLocales: true`.

<Banner type="info">
  When the `_status` field is **not** localized (localization disabled, or no
  localized fields), publishing and unpublishing always apply to the whole
  document, and `publishAllLocales` / `unpublishAllLocales` have no effect.
</Banner>

The same parameters are available on `payload.updateGlobal()` for globals, as well as on the REST and Local APIs.

In the admin UI, the document controls' main Publish button always publishes only the currently active locale. A secondary "Publish all locales" button, next to the main one, calls `publishAllLocales` for every locale you have access to.
