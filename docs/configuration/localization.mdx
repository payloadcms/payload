...
**Example Payload config set up for localization with labeled locales:**
...
export default buildConfig({
  ...
  locales: [
    ...
    },
    {
      label: "Arabic",
      value: "ar",
      // opt-in to setting default text-alignment on Input fields to rtl when current locale is rtl
      rtl: true,
    },
  ],
  defaultLocale: "en",
