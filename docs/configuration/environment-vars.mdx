...
For security and safety reasons, the [Admin Panel](../admin/overview) does **not** include Environment Variables in its _client-side_ bundle by default. But, both Next.js and TanStack Start provide a mechanism to expose Environment Variables to the client-side bundle when needed.

If you are building a [Custom Component](../custom-components/overview) and need to access Environment Variables from the client-side, you can do so by prefixing them with `NEXT_PUBLIC_` on Next.js, or with `VITE_` on TanStack Start.
...
On TanStack Start, the same rule applies but the mechanism is Vite's. Prefix any variable you want exposed to the client-side bundle with `VITE_`, then read it from `import.meta.env`:
...
const stripeKey = import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY // highlight-line
...
For more information, check out the [Vite documentation](https://vite.dev/guide/env-and-mode).
