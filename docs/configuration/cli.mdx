...
The `z` export from `payload/cli` is Zod Mini. Payload already uses Zod Mini for its built-in commands, so this export adds no extra schema code to the CLI bundle. Use the exported `strictObject` helper for the root input schema.
...
Return a number from the handler when the command needs to set the process exit code.
...
For less common cases, you can use an object to change the flag, argument position, or parser. Set a field to `false` when it should not be exposed as a shell argument or option.
