...
The `input` schema defines the values the command accepts. Payload uses it to type `args`, validate input, and generate help. You can use any [Standard Schema](https://standardschema.dev/) library, but it must also support JSON Schema so Payload can generate JSON help. For the easiest setup, import Zod from `payload/cli` as shown above.
...
defineCLICommand({
  input: z.strictObject({
    filePath: z.string().describe('File to import.'),
  }),
  cli: {
    filePath: 'argument',
  },
