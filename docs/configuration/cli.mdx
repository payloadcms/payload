...
Payload includes local commands for inspecting the config and working with collections and globals. They use the Local API, load the current Payload config on every invocation, and respect access control by default. Local CLI commands do not have a logged-in user, so access is checked as an anonymous request unless you explicitly bypass it. Run them only from a trusted environment with access to the project.
...
Omit `--override-access` or pass `--override-access false` to run a data command through Payload's access control:
...
```sh
pnpm payload createDocuments --slug posts --override-access true --documents @documents.json
```
...
```sh
pnpm payload createDocuments --input @create-posts.json --override-access true
```

`createDocuments` and `updateDocument` return affected document IDs by default.
Pass `--returning` when you need the complete documents:
