...
| `migrate:api-keys`      | Migrate legacy API keys into the payload-api-keys collection. |
...
Payload includes local commands for inspecting the config and working with collections and globals. They use the Local API, load the current Payload config on every invocation, and bypass access control by default. Run them only from a trusted environment with access to the project.
...
You can then query data without starting the development server:
...
Pass `--override-access false` to run a data command through Payload's access control. Local CLI commands do not have a logged-in user, so access is checked as an anonymous request:
...
The built-in read-only data commands cover config and schema inspection, collection queries, counts, distinct values, and collection and global version reads. Use `payload help --json` to list them or `payload help <command> --json` to inspect the exact input accepted by one command.
