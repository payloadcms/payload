...
You can place `--json` before or after the command. Commands that do not return data omit `result`. Payload writes logs and other command messages to stderr, so stdout remains one valid JSON response. A failed command returns `success: false`, an error code, and an error message. Input validation errors also include the issues and the command's input schema.
