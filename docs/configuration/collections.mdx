...
<Banner type="warning">
  **Using a relationship or join field as `useAsTitle`:** Payload will display only the ID of the related document. To show a specific field from the related document instead, define a relationship-path virtual field (e.g. `virtual: 'author.name'`) and point `useAsTitle` to that field. Note that this is the _only_ type of virtual field supported by `useAsTitle` — computed virtual fields (`virtual: true`) are not queryable and cannot be used.
</Banner>
