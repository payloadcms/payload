...
| `listSelectFields`           | Performance opt-in. When `true`, uses the Select API in the List View to process only the active columns as opposed to entire documents. [More details](#list-select-fields).                                                |
...
When `true`, the List View will use the [Select API](../queries/select) to process only the active columns as opposed to entire documents. This can greatly improve performance, especially for collections with large documents or many fields.

To enable this, set `listSelectFields: true` in your Collection Config:
...
<Banner type="info">
  **Note:** The `listSelectFields` property is labeled as experimental, as it
  will likely become the default behavior in v4 and be deprecated.
</Banner>
