...
| `listSelectActiveColumns`    | Performance opt-in. When `true`, uses the Select API in the List View to query only the active columns as opposed to entire documents. [More details](#list-select-fields).                                                  |
...
To enable this, set `listSelectActiveColumns: true` in your Collection Config:
...
<Banner type="info">
  **Note:** The `listSelectActiveColumns` property is labeled as experimental,
