...
| `authorship`         | Automatically track the user that created and last updated each document via polymorphic `createdBy` / `updatedBy` fields. Enabled by default. Set to `false` to disable, or to `{ createdBy: false }` / `{ updatedBy: false }` to toggle each field. [More details](#authorship). |
...
Payload automatically tracks the user that created and last updated each document through two polymorphic relationship fields, `createdBy` and `updatedBy`. This is the authorship equivalent of the `createdAt` / `updatedAt` [timestamps](#config-options).

Both fields relate to every authentication-enabled collection in your config, so setups with multiple user collections are supported out of the box. Their value is stored as `{ relationTo, value }`:

```ts
{
  // ...
  createdBy: { relationTo: 'users', value: '648b...' },
  updatedBy: { relationTo: 'admins', value: '92f1...' },
}
```

Authorship is **enabled by default**. Control it per-collection (and per-global) with the `authorship` option:
...
export const Posts: CollectionConfig = {
  ...
  // Disable both fields
  authorship: false,
  // Or toggle each field independently
...
- On create, both `createdBy` and `updatedBy` are set from the authenticated user on the request (`req.user`). Passing either field in your `data` overrides the system-derived value.
- On update, `updatedBy` is set from `req.user`; passing `updatedBy` in your `data` overrides it, and passing `null` clears it. `createdBy` is immutable once set, so passing it on update has no effect.
- When there is no user on the request (e.g. a Local API call without `req.user`), the existing value is left unchanged.

<Banner type="warning">
  These fields are hidden from the document edit view but are available in the
  API response and the versions view. Because they relate to your auth
  collections, enabling authorship adds new database columns — existing projects
  will need a migration when upgrading. Use the
  [`@payloadcms/codemod`](https://github.com/payloadcms/payload/tree/main/packages/codemod)
  `migrate-authorship-default` transform to add `authorship: false` everywhere
  and preserve the previous behaviour.
</Banner>

### Customizing the authorship fields

To change how `createdBy` / `updatedBy` are rendered — for example to make them visible, relabel them, or move them into the sidebar — add the exported `createCreatedByField` / `createUpdatedByField` builders to your collection's `fields`. Fields you add this way are used as-is (Payload won't re-inject its own), and the stamping hooks are preserved:
...
export const Posts: CollectionConfig = {
  ...
  fields: [
    ...
    // updatedBy is still auto-injected with its defaults — only add the builder for
    // the field you want to customize.
  ],
}
```

`overrides` win for `label`, `relationTo`, and other top-level props; `admin` and `access` are merged onto the secure defaults, and the stamping hooks are always kept (any hooks you pass are appended). The polymorphic `relationTo` is filled in automatically with your auth-enabled collections during sanitization, so you don't need to enumerate them.

If instead you define your own raw `createdBy` / `updatedBy` field (not via these builders), Payload accepts it wholesale and does **not** attach the stamping hooks — you're responsible for setting the value.
...
| `useAsTitle`                 | Specify a top-level field to use for a document title throughout the Admin Panel. If no field is defined, the ID of the document is used as the title. Computed virtual fields (`virtual: true`) cannot be used — only relationship-path virtual fields (e.g. `virtual: 'author.name'`) are supported. |
...
<Banner type="warning">
  **Using a relationship or join field as `useAsTitle`:** Payload will display
  only the ID of the related document. To show a specific field from the related
  document instead, define a relationship-path virtual field (e.g. `virtual:
  'author.name'`) and point `useAsTitle` to that field. Note that this is the
  _only_ type of virtual field supported by `useAsTitle` — computed virtual
  fields (`virtual: true`) are not queryable and cannot be used.
</Banner>
