...
- On create, both `createdBy` and `updatedBy` are set from the authenticated user on the request (`req.user`).
- On update, `updatedBy` is set from `req.user`. `createdBy` is immutable once set.
- Passing `createdBy` / `updatedBy` in your `data` overrides the system-derived user. Passing `null` clears the field.
