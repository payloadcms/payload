...
All collections are paginated automatically. Responses are returned with top-level pagination data, while returned documents are nested within a docs array.

| Option                      | Description  |
| --------------------------- | -------------|
| `docs`                      | Array containing all documents in the collection. |
| `totalDocs`                 | `number` of total available documents within the collection. |
| `limit`                     | Limit query parameter - defaults to `10`. |
| `totalPages`                | `number` of total pages available, based upon the `limit` queried for. |
...
Read more about pagination in [Queries](/docs/queries/pagination).
