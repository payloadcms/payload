...
For [Auth-enabled Collections](../authentication/overview), this hook runs after successful `resetPassword` operations. Returned values are discarded.

```ts
import type { CollectionAfterPasswordResetHook } from 'payload'

const afterPasswordResetHook: CollectionAfterPasswordResetHook = async ({
  req,
  user,
}) => {
  // Log the password reset activity for the user.
}
```

The following arguments are provided to the `afterPasswordReset` hook:
