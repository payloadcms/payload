...
The `afterOperation` Hook type can be used to modify the documents that are returned from the `find` operation.

The difference to using `afterRead` would be that this hook fires once and includes all `docs` where `afterRead` fires for every doc separately.

Available Collection operation is limited to `find`.
...
const afterOperationHook: CollectionAfterOperationHook = async ({
  ...
  operation, // name of the operation
  docs; // documents returned from the operation
}) => {
  return docs; // Return documents as necessary
}
```
