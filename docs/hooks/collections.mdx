...
Collection Hooks are [Hooks](./overview) that run on Documents within a specific [Collection](../configuration/collections). They allow you to execute your own logic during specific events of the Document lifecycle. All [Auth-enabled Collections](../collections/authentication) also receive an additional set of Hooks specific to the authentication flow.
...
Additionally, all [Auth-enabled Collections](../authentication/overview) feature the following Auth-enabled Hooks:
