...
```ts
import { CollectionRefreshHook } from 'payload/types';

const afterRefreshHook: CollectionRefreshHook = async ({
  req, // full express request
...
```ts
import { CollectionMeHook } from 'payload/types';

const afterMeHook: CollectionMeHook = async ({
  req, // full express request
