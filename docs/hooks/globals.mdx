...
<Banner type="info">
  **Tip:** You can also set hooks on the field-level to isolate hook logic to specific fields. [More
  details](./fields).
</Banner>
...
const beforeValidateHook: GlobalBeforeValidateHook = async ({ data, req, originalDoc }) => {
  return data
}
```
...
const beforeChangeHook: GlobalBeforeChangeHook = async ({ data, req, originalDoc }) => {
  return data
}
```
...
const afterChangeHook: GlobalAfterChangeHook = async ({ doc, previousDoc, req }) => {
  return data
}
```
