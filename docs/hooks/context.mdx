...
To help detect infinite loops, Payload provides a configuration option to limit the number of times an operation can be called in a single request. This is configured via the `hooksMaxRecursion` option in your Payload config.
...
export default buildConfig({
  // ...
  hooksMaxRecursion: 5, // default is undefined (no limit)
})
```

<Banner type="warning">
  **Important:** To ensure that recursive hooks are tracked correctly, you must
  pass the `context` object between Payload operations. Failing to do so may
  result in the recursion limit not being enforced as expected.
</Banner>
    ...
    beforeChange: [
      async ({ context, doc, req }) => {
        ...
        await req.payload.update({
          ...
          // highlight-start
          context, // Pass the context object here to track recursion across operations
          // highlight-end
        })
      },
    ],
