...
To help prevent infinite loops, Payload provides a configuration option to limit the number of times a hook can be called in a single request. This is configured via the `maxHookRecursion` option in your Payload config.
...
export default buildConfig({
  // ...
  maxHookRecursion: 5, // default is undefined (no limit)
})
```
