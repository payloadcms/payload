...
To help prevent infinite loops, Payload provides a configuration option to limit the number of times a hook can be called in a single request. This is configured via the `hooksMaxRecursion` option in your Payload config.
