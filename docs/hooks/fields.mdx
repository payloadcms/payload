...
| Option                   | Description                                                                                                                                                                                                           |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`collection`**         | The [Collection](../configuration/collections) which the field belongs to. If the field belongs to a Global, this will be `null`.                                                                                                                       |
| **`context`**            | Custom context passed between hooks. [More details](./context).                                                                                                                              |
| **`data`**               | In the `afterRead` hook this is the full Document. In the `create` and `update` operations, this is only the data to update the Document with.                                                                                 |
...
| **`global`**             | The [Global](../configuration/globals) which the field belongs to. If the field belongs to a Collection, this will be `null`.                                                                                                                       |
| **`operation`**          | A string relating to which operation the field is currently executing within. Useful within `beforeValidate`, `beforeChange`, and `afterChange` hooks to differentiate between `create` and `update` operations. |
| **`originalDoc`**        | In the `update` operation, this is the full Document. In the `afterChange` hook, this is the resulting Document.                                                                                        |
...
| **`siblingData`**        | The data of sibling fields adjacent to the field this Hook is running against.        |
