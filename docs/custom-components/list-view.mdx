...
| `listHeaderActions` | An array of components to render as action buttons in the list header (next to the title). [More details](#listheaderactions). |
...
The `listHeaderActions` property allows you to inject custom components as action buttons in the List View header, rendered alongside the built-in title actions (like Bulk Upload).

To add `listHeaderActions` components, use the `components.listHeaderActions` property in your [Collection Config](../configuration/collections):
...
export const MyCollection: CollectionConfig = {
  ...
  admin: {
    components: {
      // highlight-start
      listHeaderActions: ['/path/to/MyListHeaderActionComponent'],
      // highlight-end
    },
  },
}
...
Here's an example of a custom `listHeaderActions` component:

#### Server Component

```tsx
import React from 'react'
import type { ListHeaderActionsServerProps } from 'payload'

export function MyListHeaderActionComponent(
  props: ListHeaderActionsServerProps,
) {
...
'use client'
import React from 'react'
import type { ListHeaderActionsClientProps } from 'payload'

export function MyListHeaderActionComponent(
  props: ListHeaderActionsClientProps,
) {
  return <button>Custom Action</button>
}
```
