...
Payload includes two built-in widgets:

- `activity` — lists the documents the current user recently viewed.
- `collection-query` — renders a configurable list of documents from a related collection.

If you don't define a `defaultLayout`, the `activity` widget is automatically included in your dashboard.
