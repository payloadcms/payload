...
You can control the order in which tabs appear in the Document View by using the order property on each tab's configuration. This applies to both default system tabs (like Edit or API) and custom tabs you define.

Tabs are rendered in ascending order based on their order value. Lower numbers appear first. Tabs without an order value will appear after all tabs that have one, in the order they were defined in the config.

#### Example

The following configuration places the Edit tab first, a Custom tab second, and the Live Preview tab third:
