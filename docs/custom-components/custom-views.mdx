...
The top-level `user` has field read access applied and should be used for values passed toward client-facing components, so restricted data never reaches the client. `initPageResult.req.user` retains the complete server-side identity and should be used for access-control checks and Local API queries — do not use it for values sent to the client, and do not rely on the filtered `user` for authorization decisions. See [Server principal vs. read-filtered user](./overview#server-principal-vs-read-filtered-user) for details.
