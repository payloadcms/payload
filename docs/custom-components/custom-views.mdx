...
To add a _new_ views to the [Admin Panel](../admin/overview), simply add your own key to the `views` object with at least a `path` and `Component` property. For example:
...
<Banner type="warning">
  **Note:**
  The `root` property will replace the _entire_ [Edit View](./edit-view), including the title, tabs, etc., _as well as all nested [Document Views](#document-views)_, such as the API, Live Preview, and Version views. To replace only the Edit View precisely, use the `edit.default` key instead.
</Banner>
...
| Property   | Description                                                                                                                                                    |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `edit` | The Edit View is used to edit a single document for any given Collection. It is comprised of many different sub-views. [More details](#document-views).            |
...
| Property   | Description                                                                                                                                     |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `edit` | The Edit View is used to edit a single document for any given Global. It is comprised of many different sub-views. [More details](#document-views). |
...
Custom Views are just [Custom Components](./overview) rendered at the page-level. To understand how to build Custom Views, first review the [Building Custom Components](./overview#building-custom-components) guide. Once you have a Custom Component ready, you can use it as a Custom View.
