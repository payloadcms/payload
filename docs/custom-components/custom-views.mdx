...
<Banner type="warning">
  **Note:**
  The `root` property will replace the _entire_ [Edit View](../custom-components/edit-view), including the title, tabs, etc., _as well as all nested [Document Views](#document-views)_, such as the API, Live Preview, and Version views. To replace only the Edit View precisely, use the `edit.default` key instead.
</Banner>
...
| `clientConfig`        | The Client Config object. [More details](../custom-components/overview#accessing-the-payload-config).                                        |
