...
Actions are rendered within a dropdown in the header of the Admin Panel. Actions are typically used to display buttons that add additional interactivity and functionality, although these can be anything you'd like.
...
<Banner type="success">
  **Note:**
  You can also use set Actions on the [Edit View](./edit-view) and [List View](./list-view) in their respective configs.
</Banner>

### beforeDashboard

The `beforeDashboard` property allows you to inject Custom Components into the built-in Dashboard, before the default dashboard contents. This is useful for adding additional information or functionality to the Dashboard without replacing the entire view.

To add `beforeDashboard` components, use the `beforeDashboard` property in your `admin.components` config:
...
export default function MyBeforeDashboardComponent() {
  return (
    <p>This is a custom component injected before the Dashboard.</p>
  )
}
```
...
To add `afterDashboard` components, use the `afterDashboard` property in your `admin.components` config:
...
export default function MyAfterDashboardComponent() {
  return (
    <p>This is a custom component injected after the Dashboard.</p>
  )
}
```
...
The `beforeLogin` property allows you to inject Custom Components into the built-in Login view, before the default login form. This is useful for adding additional information or functionality to the Login view without replacing the entire view.

To add `beforeLogin` components, use the `beforeLogin` property in your `admin.components` config:
...
export default function MyBeforeLoginComponent() {
  return (
    <p>This is a custom component injected before the Login form.</p>
  )
}
```
...
To add `afterLogin` components, use the `afterLogin` property in your `admin.components` config:
...
export default function MyAfterLoginComponent() {
  return (
    <p>This is a custom component injected after the Login form.</p>
  )
}
```
...
The `beforeNavLinks` property allows you to inject Custom Components into the built-in [Nav Component](#nav), _before_ the links themselves. This is useful for adding additional information or functionality to the nav without replacing the entire nav component.

To add `beforeNavLinks` components, use the `beforeNavLinks` property in your `admin.components` config:
...
export default function MyBeforeNavLinksComponent() {
  return (
    <p>This is a custom component injected before the Nav links.</p>
  )
}
```
...
Similar to `beforeNavLinks`, the `afterNavLinks` property allows you to inject Custom Components into the built-in Nav, _after_ the links themselves.

To add `afterNavLinks` components, use the `afterNavLinks` property in your `admin.components` config:
...
The `Nav` property contains the sidebar / mobile menu in its entirety. This is useful for completely replacing the built-in Nav with your own custom navigation.

To add a custom nav, use the `Nav` property in your `admin.components` config:
...
To add a custom icon, use the `Icon` property in your `admin.components.graphics` config:
...
To add a custom logo, use the `Logo` property in your `admin.components.graphics` config:
...
The `header` property allows you to inject Custom Components above the Payload header. This is useful for adding additional information or functionality to your app without replacing the entire header.
...
To add `header` components, use the `header` property in your `admin.components` config:
...
The `logout.Button` property is the button displayed in the sidebar that logs the user out. This is typically a simple button that logs the user out when clicked.

To add a custom logout button, use the `logout.Button` property in your `admin.components` config:
