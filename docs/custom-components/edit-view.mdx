...
<Banner type="warning">
  **Note:** The Edit View is one of many [Document
  Views](./document-views) in the Payload Admin Panel. Each Document
  View is responsible for a different aspect of the interacting with a
  single Document.
</Banner>
...
export function MyCustomClientEditView(
...
<Banner type="warning">
  **Important:** Collection and Globals are keyed to a different
  property in the `admin.components` object have slightly different
  options. Be sure to use the correct key for the entity you are
  working with.
</Banner>
...
<Banner type="warning">
  **Note:** The `Description` component is shared between the Edit
  View and the [List View](./list-view).
</Banner>
...
export function MyDescriptionComponent(
  props: ViewDescriptionServerProps,
) {
...
export function MyDescriptionComponent(
  props: ViewDescriptionClientProps,
) {
