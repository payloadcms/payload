...
However sometimes this can be misleading in complex monorepos where multiple packages are installing Payload dependencies as you will see the same version installed but different applications might use different instances of the packages.  
It's best to check the lockfile directly to ensure only one version of `@payloadcms/ui` is present. Installing the Payload dependencies at the root of your monorepo/workspace could help guarantee that only one version is installed across all packages.
