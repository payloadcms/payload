...
<Banner type="warning">
  **Important:** The import map file should never be manually edited. However,
  ...
  Location](../admin/admin-panel-location) for details.
</Banner>
...
| `user`    | The current user with [field read access](#server-principal-vs-read-filtered-user) applied. |
...
#### Server principal vs. read-filtered user

Server Components that render an authenticated user receive two representations of that user, and choosing the wrong one either leaks data or breaks access checks:

- **`user`** (the top-level prop, e.g. on views and field components) is the authenticated user with **field read access applied** — read-restricted fields are removed and relationships are populated according to read access. Use it for anything passed toward Client Components or rendered to the page, so restricted data never reaches the client.
- **`req.user`** (from the `req` prop, or `initPageResult.req.user` in views) is the **complete server principal** with access bypassed. Use it for access-control checks, Local API queries, and role/authorization gating — `user` may omit fields your access logic relies on (for example a role stored in a read-restricted field).

<Banner type="warning">
  **Security:** Never pass `req.user` toward a Client Component or into client
  React state — use `user`. Conversely, never rely on `user` for authorization
  decisions — use `req.user`.
</Banner>
...
export const MyClientFieldComponent: React.FC<TextFieldServerProps> = ({
  field: { name },
}) => {
...
export const MyClientFieldComponent: React.FC<TextFieldClientProps> = ({
  clientField: { name },
}) => {
  return <p>{`This field's name is ${name}`}</p>
}
...
export default async function MyServerComponent({ payload, locale, user }) {
...
Payload exposes its design tokens as global [CSS Variables](https://developer.mozilla.org/en-US/docs/Web/CSS/Using_CSS_custom_properties), so you can consume them directly without importing anything. For example, using a breakpoint:
...
const MyComponent: React.FC<TextFieldClientProps> = ({ path }) => {
