...
There is a known issue when using the Local API with Next.js version `13.4.13` and higher. Next.js executes each operation in a child process. That means that unless you explicitly initialize Payload within your operation, it will return an empty object. 
