...
| `overrideLock`     | Ignore document locks and proceed with the operation, even if another user has locked the document. Default is `false`. [More details](../admin/locked-documents).|
...
const result = await payload.update({
  ...
  user: dummyUser,
  overrideAccess: false,
  overrideLock: true, // Ignores document lock (optional)
  showHiddenFields: true,
