...
| `overrideAccess`     | Skip [Access Control](./access-control). Set to `true` to bypass it for trusted server-side work; where optional, it defaults to `false`, which respects it.                                                                                                                                                                                                          |
...
| `user`               | The user to run access control checks against. Pass the authenticated user when the operation acts on their behalf; omit it to evaluate access as an anonymous request.                                                                                                                                                                                               |
...
Local API operations with an optional `overrideAccess` value respect Access Control by default.
Pass `overrideAccess: true` to skip it for trusted server-side work such as cron jobs, seeding,
and migrations. See [Access Control](./access-control).
