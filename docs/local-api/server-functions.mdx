...
Both supported Admin Panel frameworks provide them: Next.js calls them server functions (previously **server actions**), declared with the `'use server'` directive, while TanStack Start defines them with `createServerFn` from `@tanstack/react-start`. The examples below use the Next.js `'use server'` syntax; on TanStack Start the body is identical — only the function declaration differs.
...
- First, we use the headers function from `next/headers` to retrieve the request headers. On TanStack Start, get them from the request instead: `const headers = new Headers(getRequest().headers)`, where `getRequest` is imported from `@tanstack/react-start/server`.
...
<Banner type="info">
  The examples below import these helpers from `@payloadcms/next/auth`. On
  TanStack Start, import the same `login`, `logout`, and `refresh` functions
  from `@payloadcms/tanstack-start/server` instead.
</Banner>
