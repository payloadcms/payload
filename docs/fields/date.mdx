...
To create a Date Field, set the `type` to `date` in your [Field Config](./overview):
...
The Date Field inherits all of the default options from the base [Field Admin Config](./overview#admin-config), plus the following additional options:
