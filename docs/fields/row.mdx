...
To create a Row Field, set the `type` to `row` in your [Field Config](./overview):
...
| **`admin`**     | Admin-specific configuration excluding `description`, `readOnly`, and `hidden`. [More details](./overview#admin-config). |
