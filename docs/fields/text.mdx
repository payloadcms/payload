...
Payload provides a built-in `slug` field type so you don't have to build one from scratch. This field automatically generates a slug based on the value of another field, such as a title or name field. It provides UI to lock and unlock the field to protect its value, as well as to re-generate the slug on-demand.

To add a Slug Field, set a field's `type` to `slug`:
...
The slug field defaults to `required`, `unique`, and `index` set to `true`, and renders in the `sidebar`. In addition to the standard [field options](./overview#field-options), it exposes:
