...
| **`sortCaseInsensitive`** | Enable case-insensitive sorting for this field. When enabled, sorting will ignore letter case. Defaults to `false`.         |
...
By default, text fields use case-sensitive sorting where uppercase letters sort before lowercase letters (e.g., "Apple", "Cherry", "banana"). When `sortCaseInsensitive` is enabled, the field will sort alphabetically regardless of case (e.g., "Apple", "banana", "Cherry").
  ...
  fields: [
    {
      ...
      type: 'text',
      admin: {
        sortCaseInsensitive: true, // Enable case-insensitive sorting
      },
    },
  ],
}
...
- Sorting by `filename` ascending: "Apple.jpg", "banana.jpg", "Cherry.jpg", "date.jpg"
- Without case-insensitive sorting: "Apple.jpg", "Cherry.jpg", "banana.jpg", "date.jpg"

### Database Implementation

The case-insensitive sorting feature is implemented differently depending on your database adapter:

#### MongoDB

Uses [collation](https://docs.mongodb.com/manual/reference/collation/) with `strength: 2` to perform case-insensitive sorting at the database level. This is applied per-query and respects your locale settings.

```ts
// MongoDB query with collation
db.collection
  .find()
  .sort({ filename: 1 })
...
Wraps the sorted column in SQL's `LOWER()` function to normalize case before sorting:

```sql
-- PostgreSQL/SQLite query
SELECT * FROM media ORDER BY LOWER(filename) ASC;
```

### Performance Considerations

Case-insensitive sorting can impact query performance, especially on large collections. Consider these optimization strategies:

#### PostgreSQL

Create a functional index on the lowercase version of the field:

```sql
CREATE INDEX idx_filename_lower ON media (LOWER(filename));
```

Add this to your database migrations or initialization scripts.

#### SQLite

Create a case-insensitive collation index:

```sql
CREATE INDEX idx_filename_lower ON media (filename COLLATE NOCASE);
```

#### MongoDB

MongoDB's collation-based sorting is generally performant, but for very large collections, ensure you have a standard index on the field:
...
The Media collection's `filename` field has case-insensitive sorting enabled by default to provide intuitive file browsing.
