...
To create a Text Field, set the `type` to `text` in your [Field Config](./overview):
...
The Text Field inherits all of the default options from the base [Field Admin Config](./overview#admin-config), plus the following additional options:
