...
By default, the entire related document is returned when querying for a relationship field. You can limit the fields returned by using the `select` property. This property can either be a function that returns an array of field names, or an array of field names. The function will be called with an argument object with the following properties:
...
and should return an array of field names to select.
