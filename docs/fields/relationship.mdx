...
| **`select`**         | An array of fields to be returned from the related collection. [More](#selecting-fields). |
...
By default, the entire related document is returned when querying for a relationship field. You can limit the fields returned by using the `select` property. This property is an array of field names to be returned. The `id` field is always returned. 
