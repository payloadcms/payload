...
To create a Relationship Field, set the `type` to `relationship` in your [Field Config](./overview):
...
export const MyRelationshipField: Field = {
  // ...
  type: 'relationship', // highlight-line
  relationTo: 'products', // highlight-line
}
```
...
| **`admin`**            | Admin-specific configuration. [More details](#admin-config).                                                        |
...
The Relationship Field inherits all of the default options from the base [Field Admin Config](./overview#admin-config), plus the following additional options:
