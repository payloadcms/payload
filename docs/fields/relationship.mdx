...
By default, the entire related document is returned when querying for a relationship field.
You can limit the fields returned by using the `select` property.
This property can either be a function or an array of field names that will be selected.
The function will be called with an argument object with the following properties:
...
and should return an array of field names to select or `true` to select all fields.
...
<Banner type="warning">
  ...
  This can lead to unexpected behavior when using the <strong>select</strong> property in conjunction with <strong>QraphQL</strong>.
</Banner>
