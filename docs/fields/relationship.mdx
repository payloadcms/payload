...
The `filterOptions` property can either be a `Where` query directly, or a function (synchronous or asynchronous) that
returns `null` to not filter, `false` to avoid querying the relationship or the `Where` query to use. When using a
function, it will be called with an argument object containing the following properties:
