...
You can link virtual fields with to fields in other collections through a relationship (or upload) field, for example:
...
Here, `categoryTitle` will _always_ be populated with the corresponding value, even if the current `depth` is `0`, You can always query and sort by this field.
