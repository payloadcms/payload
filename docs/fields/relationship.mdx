...
### Atomic Operations for hasMany Relationships

For relationship fields with `hasMany: true`, you can perform atomic operations that append or remove items without replacing the entire array. These operations work with both high-level APIs (`payload.update`) and low-level database APIs (`payload.db.updateOne`, `payload.db.updateMany`).
You cannot use both regular data updates and atomic operations on the same field in a single request.

#### High-Level API Operations

Use the `operations` parameter to perform atomic updates:

```ts
// Append items to a relationship array
await payload.update({
  ...
  operations: {
    $push: {
      categories: ['featured-id', 'trending-id'],
    },
  },
})

// Remove items from a relationship array
await payload.update({
  collection: 'posts',
  id: 'post123',
  data: {},
...
#### Low-Level Database API Operations

For direct database operations, use the atomic syntax in the `data` object. Keep in mind that hooks and validations are not performed on low-level API calls:
...
await payload.db.updateMany({
  ...
  data: {
    categories: { $push: ['featured-id', 'trending-id'] },
  },
})
...
await payload.db.updateMany({
  ...
  data: {
    tags: { $remove: ['draft-id', 'private-id'] },
  },
})
```
...
These operations work seamlessly with polymorphic relationships:
...
Atomic operations also support dot-notation paths for nested fields:

```ts
// Update nested fields within groups, tabs, or other containers
await payload.update({
  collection: 'posts',
  id: 'post123',
  data: {},
...
#### Bulk Update with Atomic Operations

Apply atomic operations to multiple documents using a `where` clause:

```ts
// Add categories to all published posts
await payload.update({
  collection: 'posts',
  where: { status: { equals: 'published' } },
  data: {},
  operations: {
    $push: { categories: ['featured-id'] },
  },
})
```

<Banner type="success">
  Atomic operations work correctly even when the modified field is not in
  `select`. The full document is fetched internally, but only selected fields
  are returned.
</Banner>
