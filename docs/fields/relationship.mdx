...
The `filterOptions` property can either be a `Where` query directly, or an async function that returns one. When using a function, it will be called with an argument object with the following properties:
