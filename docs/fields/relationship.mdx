...
### Atomic Array Operations for hasMany Relationships

For relationship fields with `hasMany: true`, you can perform atomic array operations that append or remove items without replacing the entire array:
...
await payload.db.updateMany({
  ...
  data: {
    categories: { $push: ['featured', 'trending'] },
  },
})
```
...
These operations also work with polymorphic relationships:
