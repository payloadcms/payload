...
The `filterOptions` property can either be a `Where` query directly, or a function that returning `null` to not alter
the filter, `false` to avoid querying the relationship or the `Where` query to use. When using a function, it will be
