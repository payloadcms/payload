...
To create a Code Field, set the `type` to `code` in your [Field Config](./overview):
...
_This field uses the `monaco-react` editor syntax highlighting._
...
The Code Field inherits all of the default options from the base [Field Admin Config](./overview#admin-config), plus the following additional options:
