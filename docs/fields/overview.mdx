...
Fields can be prefilled with a starting value using the `defaultValue` property. You can assign the defaultValue directly in the field configuration or make it dynamic by supplying a function.
...
<Banner>
  defaultValue functions also support `async` callbacks allowing requests to be made to pre-populate field data further.
</Banner>
