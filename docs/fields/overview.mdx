...
| **`disabled`**    | Controls where the field is disabled in the [Admin Panel](../admin/overview). Pass `true` to disable everywhere, or an object `{ edit?, column?, filter?, groupBy?, bulkEdit? }` for granular per-area control. [More details](#disabled). |
...
- `disabled: true` — disables the field everywhere (edit form, list column selector, list filter dropdown, groupBy selector, bulk edit modal).
- `disabled: false` (or `undefined`) — field is enabled everywhere.
- `disabled: { ... }` — granular control per area:

| Key        | Disables                    |
| ---------- | --------------------------- |
| `edit`     | Edit view form rendering.   |
