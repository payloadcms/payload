...
In addition to each field's base configuration, you can define specific traits and properties for fields that only have effect on how they are rendered in the Admin panel. The following properties are available for all fields within the `admin` property of a Field Config:

| Option              | Description                                                                                                                                                                                                                      |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `condition`         | Programmatically show / hide fields based on what other fields are doing. [More details](../admin/fields/conditional-logic).                                                                                               |
| `components`        | All Field Components can be completely and easily swapped out for [Custom Components](../admin/components) that you define. [More details](../admin/fields).                                                                             |
| `description`       | Helper text to display with the field to provide more information for the editor user. [More details](../admin/fields#description).                                                                                                 |
