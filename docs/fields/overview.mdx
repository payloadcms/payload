...
Fields that support a `defaultValue` can be configured either as the value you wish to pre-populate the field value with or a function that return it dynamically.

When using a function, it will be called with an optional argument object containing:
  ...
  fields: [
    {
      ...
      maxLength: 20,
      admin: {
        description: ({ user, locale }) => (localizedDefault[locale])
      }
    }
  ]
}
