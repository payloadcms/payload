...
You can show and hide fields based on what other fields are doing by utilizing conditional logic on a field by field basis. The `condition` property on a field's admin config accepts a function which takes four arguments:
...
```ts
{
  fields: [
    ...
    },
    {
      ...
      admin: {
        // highlight-start
        condition: (data, path, siblingData, { user }) => {
