...
To create a Collapsible Field, set the `type` to `collapsible` in your [Field Config](./overview):
...
export const MyCollapsibleField: Field = {
  // ...
  type: 'collapsible', // highlight-line
  fields: [
    // ...
  ],
}
...
The Collapsible Field inherits all of the default options from the base [Field Admin Config](./overview#admin-config), plus the following additional options:
