...
The `select` property is a function that will be called with an argument object with the following properties:
...
and should return an object were the key is the field name and the value is either a nested object
or `true` to select or `true` to select all fields.
...
const fields = {
  ...
  name: 'content',
  label: 'Content',
  select: ({ data, collection, siblingData, req }) => {
    if (collection.slug === 'posts' && data.type === 'related-post') {
      // only select the title and slug field from the related collection
      return { title: true, slug: true };
    }
    // select all fields
    return true;
  },
