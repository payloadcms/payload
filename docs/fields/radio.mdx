...
To create a Radio Field, set the `type` to `radio` in your [Field Config](./overview):
...
The Radio Field inherits all of the default options from the base [Field Admin Config](./overview#admin-config), plus the following additional options:
