...
| **`admin.disabled`**            | Pass `true` to disable the UI field everywhere, or an object `{ edit?, column?, filter?, groupBy?, bulkEdit? }` for granular control. UI fields default to `disabled: { bulkEdit: true }`. [More details](./overview#disabled). |
