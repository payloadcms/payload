...
To create a Blocks Field, set the `type` to `blocks` in your [Field Config](./overview):
...
The Blocks Field inherits all of the default options from the base [Field Admin Config](./overview#admin-config), plus the following additional options:
