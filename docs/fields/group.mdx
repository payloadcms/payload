...
To create a Group Field, set the `type` to `group` in your [Field Config](./overview):
...
The Group Field inherits all of the default options from the base [Field Admin Config](./overview#admin-config), plus the following additional options:
