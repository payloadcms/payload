...
| **`label`**            | Used as a heading in the Admin Panel and to name the generated GraphQL type. Defaults to name converted to words. Can be undefined to completely omit having a label when name is undefined.                                                                                       |
...
The label will be optional when a `name` is not provided. Not defining one will render just the grouped fields without a label.
