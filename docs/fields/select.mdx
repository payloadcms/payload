...
To create a Select Field, set the `type` to `select` in your [Field Config](./overview):
...
The Select Field inherits all of the default options from the base [Field Admin Config](./overview#admin-config), plus the following additional options:
