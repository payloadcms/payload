...
When used, it will be called with an argument object with the following properties:
...
| `options`     | The field's configured `Option[]` to filter from.                                                                                                                                        |
...
If a returned option's `label` differs from the field's configured `label` for the same `value`, the Admin Panel will use the returned `label` wherever the option is displayed, for example after an async lookup that resolves labels from another collection.
