...
| **`reduceOptions`**    | Reduce the available options based on the current user or value of another field. [More details](#reduce-options)                                                                                                                                                                                       |
...
Used to dynamically filter and validate the options available to the user based on various criteria.
...
To do this, use the `reduceOptions` property in your [Field Config](./overview):
...
The result of this function will determine:

- Which options are displayed in the Admin Panel
- Which options can be saved to the database

<Banner type="info">
  **Note:** The `reduceOptions` property is similar to `filterOptions` in
