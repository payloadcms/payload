...
| **`sortable`**     | Boolean when, if set to `true`, allows this field to be sortable using drag and drop. (Only works when `hasMany` is set to `true`)                                                                  |
  ...
  fields: [
    {
      ...
      hasMany: true,
      sortable: true, // use mouse to drag and drop different values, and sort them according to your choice
      admin: {
        isClearable: true,
      },
      options: [
