...
To create a JSON Field, set the `type` to `json` in your [Field Config](./overview):
...
The JSON Field inherits all of the default options from the base [Field Admin Config](./overview#admin-config), plus the following additional options:
