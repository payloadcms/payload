...
To create a Textarea Field, set the `type` to `textarea` in your [Field Config](./overview):
...
The Textarea Field inherits all of the default options from the base [Field Admin Config](./overview#admin-config), plus the following additional options:
