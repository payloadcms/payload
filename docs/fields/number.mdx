...
To create a Number Field, set the `type` to `number` in your [Field Config](./overview):
...
The Number Field inherits all of the default options from the base [Field Admin Config](./overview#admin-config), plus the following additional options:
