...
To do this, pass the shared block slug to the `blocks` option in your blocks field:
...
On TanStack Start there is no equivalent flag — it builds with [Vite](https://vite.dev), which already uses fast, esbuild-based dev transforms by default.
...
This option is Next.js-specific. TanStack Start uses Vite, which does not bundle server packages into the client, so no equivalent setting is needed.
