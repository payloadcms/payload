...
This server doesn't do anything just yet. But, after you have this in place, we can initialize Payload via its `initAsync()` method, which accepts a small set of arguments to tell it how to operate. For a full list of `initAsync` arguments, please consult the [main configuration](/docs/configuration/overview) docs.
...
Here is a list of all properties available to pass through `payload.initAsync`:
