...
Then just follow the prompts! Payload will automatically detect if you are installing into an existing Next.js project. If you prefer, you can also [install Payload into an existing Next.js project](../admin/overview) yourself. Once finished, all necessary packages will be installed to use Payload's Local API, along with an HTTP layer to serve the REST and GraphQL APIs.
