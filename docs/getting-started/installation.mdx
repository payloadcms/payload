...
- Next.js `16.2.6`+
- TypeScript version `6.0.3`+. Types are not guaranteed to work on older versions.
...
<Banner type="info">
  The steps below target the **Next.js App Router**. Payload's Admin Panel also
  runs on [TanStack Start](https://tanstack.com/start/latest) via the
  `@payloadcms/tanstack-start` package — the differences are called out inline
  where they apply. See [Project Structure](../admin/overview#project-structure)
  for the TanStack Start file layout.
</Banner>
...
If you are running the Admin Panel on TanStack Start, install `@payloadcms/tanstack-start` in place of `@payloadcms/next`:

```bash
pnpm i payload @payloadcms/tanstack-start
```
...
<Banner type="info">
  **TanStack Start:** there is no `next.config` step. TanStack Start builds with
  [Vite](https://vite.dev), so Payload integrates through the `payloadPlugin`
  Vite plugin exported from `@payloadcms/tanstack-start/vite`, which wraps the
  `tanstackStart`, React, and RSC Vite plugins. Add it to your `vite.config`
  instead of wrapping a Next.js config.
</Banner>
