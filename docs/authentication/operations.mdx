...
<Banner type="success">
  **Server Functions:** Payload offers a ready-to-use `login` server function
  ...
  the [Server Function docs](./server-functions).
</Banner>
...
<Banner type="success">
  **Server Functions:** Payload provides a ready-to-use `logout` server function
  ...
  and examples, check out the [Server Function docs](./server-functions).
</Banner>
...
- Updating the password of the user making the request - through the admin Account view, the REST and GraphQL update endpoints, or a Local API update that receives the authenticated user - keeps the session that the request was made with and ends the rest.
...
A Local API update that runs without an authenticated user has no session to keep, so it ends all of the user's sessions. Pass the `user` returned by `payload.auth` when the user making the request should stay logged in:
