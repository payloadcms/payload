...
If a request with a valid API key receives a `403` response, first check the target collection's Access Control. API key authentication identifies the user and assigns them to `req.user`, but it does not bypass collection-level `read`, `create`, `update`, or `delete` access functions. For example, `GET /api/users` must still pass the `read` access function on the `users` collection for the user that owns the API key.
