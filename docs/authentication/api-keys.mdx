...
To enable API keys on a collection, set the `useAPIKey` auth option to `true`. From there, a new interface will appear in the [Admin Panel](../admin/overview) for each document within the collection that allows you to generate any number of independently revocable API keys for each user in the Collection.
...
Every generated key is stored in a shared, hidden `payload-api-keys` collection, matched only by
a one-way hash of the key - the plaintext secret is shown once, at creation or regeneration time,
and is never stored anywhere in a recoverable form. Because keys live in their own collection,
changing your `PAYLOAD_SECRET` does not invalidate them.

By default, anyone who can access the [Admin Panel](../admin/overview) can view and revoke any
owner's keys, mirroring `access.admin`. To scope that down, pass an object instead of `true`:
...
export const ThirdPartyAccess: CollectionConfig = {
  ...
  auth: {
    useAPIKey: {
      access: {
        // Whether this collection's users can view OTHER owners' key metadata
        // (name, creation date) - never the decrypted secret.
        readOthers: ({ req }) => Boolean(req.user?.roles?.includes('admin')),
        // Whether this collection's users can rename, regenerate, or delete (revoke)
        // OTHER owners' keys. Never exposes another owner's decrypted secret.
        manageOthers: ({ req }) => Boolean(req.user?.roles?.includes('admin')),
      },
    },
  },
  fields: [],
...
Every generated secret is a run of random characters with no prefix by default. To make a
collection's keys recognizable at a glance (or to match a pattern your secret-scanning tooling
expects), set `apiKeyPrefix`:
...
export const ThirdPartyAccess: CollectionConfig = {
  ...
  auth: {
    useAPIKey: {
      apiKeyPrefix: 'tpa_', // highlight-line
    },
  },
  fields: [],
}
...
<Banner type="warning">
  **Breaking change:**
  In previous versions of Payload, `useAPIKey: true` stored a single encrypted key directly on
  the auth document, and rotating `PAYLOAD_SECRET` required re-keying that field. If you are
  upgrading from a version where you already have API keys in production, run
  `payload migrate:api-keys --dry-run` followed by `payload migrate:api-keys` to convert your
  existing keys into the new collection before switching a collection over to `useAPIKey`
  collection storage. Collection storage mode never reads the legacy fields, so any key left
  unmigrated will stop authenticating with no warning at startup.
