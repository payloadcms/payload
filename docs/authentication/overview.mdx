...
| **`localStrategy`**            | Controls Payload's built-in local auth strategy. Defaults to `true` (enabled). Set to `false` to disable, or pass `{ enabled, disableFields?, optionalPassword? }` for fine-grained control. Only override this if you have replaced Payload's auth mechanisms with your own. |
