...
For testing and demo purposes you may want to skip forcing the user to login in order to access payload. Typically, all users should be required to login to access payload, however, you can speed up local development time by enabling auto-login.

To enable auto-login, set the `autoLogin` property in the [Payload Config](../configuration/overview):
