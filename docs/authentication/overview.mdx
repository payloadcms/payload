...
Authentication is a critical part of any application. Payload provides a secure, portable way to manage user accounts out of the box. Payload Authentication is designed to be used in both the [Admin Panel](../admin/overview), we all as your own external applications, completely eliminating the need for paid, third-party platforms and services.
...
When Authentication is enabled on a [Collection](../configuration/collections), Payload injects all necessary functionality to support the entire user flow. This includes admin UI, all [auth-related operations](./operations) like account creation, logging in and out, and resetting passwords, as well as all [auth-related emails](./email) like email verification and password reset.
...
Auth-enabled Collections with be automatically injected with the following [Fields](../fields/overview) which are used internally by Payload to manage the authentication flow:
...
<Banner type="info">
  <strong>Tip:</strong>
  If you provide your own `email` field, Payload will use that instead.
</Banner>
