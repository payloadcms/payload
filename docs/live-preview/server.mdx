...
Server-side Live Preview works by making a roundtrip to the server every time your document is saved, i.e. draft save, autosave, or publish. While using Live Preview, the Admin Panel emits a new `window.postMessage` event which your front-end application can use to invoke this process. In Next.js, this means simply calling `router.refresh()`; in TanStack Start, calling `router.invalidate()`. Either re-runs the route on the server and hydrates the HTML using new data straight from the [Local API](../local-api/overview).
...
If your front-end application is built with server-side [React](https://react.dev) like the [Next.js App Router](https://nextjs.org/docs/app) or [TanStack Start](https://tanstack.com/start/latest), you can use the `RefreshRouteOnSave` component that Payload provides.
...
On TanStack Start, swap the router: import `useRouter` from `@tanstack/react-router`, refresh with `router.invalidate()`, and read the server URL from `import.meta.env.VITE_PAYLOAD_URL`:
