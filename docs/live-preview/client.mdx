...
<Banner type="info">
  ...
  Router](https://nextjs.org/docs/app), etc., we suggest setting up [server-side Live
...
<Banner type="info">
  It is important that the `depth` argument matches exactly with the depth of your initial page
...
<Banner type="warning">
  **Reminder:** If you are using [React Server
  Components](https://react.dev/reference/rsc/server-components), we strongly suggest setting up
  [server-side Live Preview](./server) instead.
</Banner>
