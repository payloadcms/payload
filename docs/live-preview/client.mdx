---
title: Implementing Live Preview in your client-side front-end application
...
While using Live Preview, the Admin panel emits a new `window.postMessage` event every time the document is _changed_. Your front-end application can listen for these events and re-render accordingly.

<Banner type="info">
  If your front-end application is built with server-side framework like Next.js App Router, React Server Components, etc., see [Server](./server).
</Banner>

If your front-end application is built with client-side React, Next.js, Vue or Nuxt.js, use the `useLivePreview` hook that Payload provides. In the future, all other major frameworks like Svelte will be officially supported. If you are using any of these frameworks today, you can still integrate with Live Preview yourself using the underlying tooling that Payload provides. See [building your own hook](#building-your-own-hook) for more information.
