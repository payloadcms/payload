...
| **`handleMessage`**   | Handles the `window.postMessage` events emitted by the parent window.                                                                                                               |
...
<Banner type="info">
  When building your own hook, ensure that the args and return values are consistent with the ones listed above. This will ensure that all hooks follow the same API.
</Banner>
