...
Hierarchy paths are virtual fields computed on read when requested, using the same `version` context as the document. When `locale: 'all'` is requested, localized title or slug fields produce per-locale path maps. Publishing a single locale is appropriate only when intentionally publishing a subset; the non-localized parent change still applies to the document as a whole.
