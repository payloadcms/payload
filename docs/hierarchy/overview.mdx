...
When versioning with drafts is enabled, hierarchy updates both published and draft versions to keep them in sync:
...
- ✅ **Independent paths** - Draft can have different path based on draft title

**Example:**

```ts
// Published version has one path
{
  _status: 'published',
  title: 'Clothing',
...
// Draft version has its own path based on draft title
{
  _status: 'draft',
  title: 'Apparel',  // Draft change
  _h_slugPath: 'products/apparel',  // Uses draft title
}

// When parent moves to 'categories':
// Published: 'categories/products/clothing' ✅
// Draft: 'categories/products/apparel' ✅
```

**How it works:**

1. **Draft-only title changes** update the draft version record in the versions table
2. **Parent moves** update both published and draft descendants separately
3. **Each version maintains its own paths** based on its own title

**Performance Impact:**

- Published document with draft = 2× updates (one per version)
...
**Use with caution:** When using `publishSpecificLocale`, avoid changing the `parent` field in drafts. This is because:

- The `parent` field is **not localized** (tree structure must be consistent across locales)
- Publishing a specific locale publishes the draft's parent value for **all locales**
- This causes paths to be recalculated for all locales based on the new parent
- Only the published locale's paths will be correct; other locales will have incorrect paths

**Problematic scenario:**

```ts
// 1. Create published document
await payload.create({
  collection: 'pages',
  data: { title: 'Page', parent: 'parent-1' },
})

// 2. Create draft with different parent
await payload.update({
  collection: 'pages',
  id: 'doc-id',
  data: { parent: 'parent-2' }, // Draft has different parent
  draft: true,
})

// 3. Publish only French
await payload.update({
  collection: 'pages',
  id: 'doc-id',
  publishSpecificLocale: 'fr', // ❌ Problem: parent changes to 'parent-2' for ALL locales
})
// Result: French paths correct, but English/German paths now incorrect
```

**Recommendation:** When moving documents in the hierarchy (changing parent), always use `publishAllLocales` (default behavior):

```ts
// ✅ Safe: publishes all locales at once
await payload.update({
  collection: 'pages',
  id: 'doc-id',
  data: { parent: 'new-parent' },
...
**Note:** Title changes work fine with `publishSpecificLocale` since path fields (`_h_slugPath`, `_h_titlePath`) are localized.
