...
## Publishing Parent Changes Across Locales

Hierarchy supports `locale: 'all'`. The `parent` field is not localized, so the hierarchy `beforeChange` hook still validates parent changes, including circular references, regardless of the locale. To publish a parent change across all locales, use an explicit all-locale publication action:
...
Hierarchy paths are virtual fields computed on read when requested, using the same `version` context as the document. When `locale: 'all'` is requested for a collection whose `useAsTitle` field is localized, both path fields are returned as per-locale maps. Publishing a single locale is appropriate only when intentionally publishing a subset; the non-localized parent change still applies to the document as a whole.
