...
### 1. Locale 'all' Not Supported

Updates with `locale: 'all'` will skip hierarchy processing. **Workaround:** Update each locale individually.

### 2. Descendant Hooks Not Triggered

When a parent moves or changes title, descendants are updated directly without triggering their `afterChange` hooks (for performance).

### 3. No Built-in Circular Reference Prevention

Add validation to your parent field to prevent circular references:
