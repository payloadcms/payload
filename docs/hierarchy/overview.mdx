...
### Locale 'all' and hierarchy processing

Updates with `locale: 'all'` will skip hierarchy processing. **Workaround:** Update each locale individually when the update needs hierarchy processing. `locale: 'all'` remains available for explicit publication transitions, but it should not be used when the hierarchy needs to recalculate paths.
...
**Recommendation:** When moving documents in the hierarchy (changing parent), publish each locale individually so hierarchy processing can run:
