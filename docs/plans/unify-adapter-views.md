...
- [~] **Stage 4.5.5:** local smoke validation in `_community` confirms the core form-interaction path works end-to-end through the unified RSC-Flight pipeline:
  - Document load (`renderAdminPage` → `DocumentViewRSC`) — array field with `TitleField` server component renders `Character count: 12 (rendered on the server, path: array.0.title)`.
  - Add-row mid-form (`form-state` shared handler returns React elements; client mounts via Flight) — newly added `array.1` row immediately renders `Character count: 0 (rendered on the server, path: array.1.title)`. This is the original motivating bug.
  - Not-found redirect contract (`renderAdminPage` throws `Error('redirect:<url>')` on missing doc; adapter translates to TanStack `redirect`) — `/admin/collections/posts/<missing>` redirects to `?notFound=<id>` with the standard banner.
  - Remaining buckets (copy-from-locale, drawers with custom server components, plugin-seo) ride the same shared `form-state` / `render-list` / `render-document` handlers and require multi-suite configs to exercise; see Stage 4.6.
- [~] **Stage 4.6:** full TanStack e2e matrix is a CI-bound activity (the previous documented run had 102 jobs and most failures were unrelated to the wire-format change — see `packages/tanstack-start/E2E_TEST_RESULTS.md`). Local validation done so far: lint clean (0 errors, 244 pre-existing warnings); `@payloadcms/ui` / `@payloadcms/next` / `@payloadcms/tanstack-start` build cleanly; `_community` Add-Row scenario green. The "data-only-pipeline rendering gaps" buckets called out in Stage 4.5/4.6 (hierarchy, document-view, versions) need a CI run on this branch to confirm they now pass — defer to CI.
