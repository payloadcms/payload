...
Payload can be deployed _anywhere that its host framework can run_ — anywhere Next.js or TanStack Start runs, including Vercel, Netlify, SST, DigitalOcean, AWS, and more. Because it's open source, you can self-host it.
...
Payload's build is driven by whichever framework hosts the Admin Panel. On Next.js it uses the [Next.js build process](https://nextjs.org/docs/app/building-your-application/deploying); on TanStack Start it uses the [Vite](https://vite.dev) build. Either way, if you've used `create-payload-app` to create your project, executing the `build` npm script will build Payload for production. The Next.js-specific `next.config` and `standalone` details below apply only to Next.js deployments.
