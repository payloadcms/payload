...
- [Azure Blob Storage](https://github.com/payloadcms/payload/tree/3.x/packages/storage-azure)
- [Google Cloud Storage](https://github.com/payloadcms/payload/tree/3.x/packages/storage-gcs)
- [AWS S3](https://github.com/payloadcms/payload/tree/3.x/packages/storage-s3)
- [Uploadthing](https://github.com/payloadcms/payload/tree/3.x/packages/storage-uploadthing)
- [Vercel Blob Storage](https://github.com/payloadcms/payload/tree/3.x/packages/storage-vercel-blob)
...
The `.npmrc*` and `.yarnrc*` globs ensure your package manager config is available before dependencies are installed. This matters because projects created with `create-payload-app` ship an `.npmrc` containing `legacy-peer-deps=true`, which `npm ci` needs in order to install against the generated lockfile. The globs are optional matches, so the build still works if those files are absent.

<Banner type="warning">
  **Security:** The default `.npmrc` only contains `legacy-peer-deps=true` and
  is safe to copy. However, if your `.npmrc` or `.yarnrc` contains private
  registry tokens, do not copy it into an image layer where it can be recovered
  by inspecting the image. Instead, use a [BuildKit secret
  mount](https://docs.docker.com/build/building/secrets/) (for example `RUN
  --mount=type=secret,id=npmrc ...`) so the credentials are never persisted in
  the final image.
</Banner>
