...
<Banner type="info">
  The solutions below are Next.js-specific (they use `next build` flags and the
  `dynamic` route-segment export). On TanStack Start the Vite build does not
  statically pre-render routes that hit the Local API, so this DB-at-build-time
  requirement generally does not apply.
</Banner>
