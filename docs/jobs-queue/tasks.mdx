---
description: A Task is a distinct function declaration that can be run within Payload's Jobs Queue.
...
You can r**egister Tasks on the Payload config, and then create [Jobs](../jobs-queue/jobs) or [Workflows](../jobs-queue/workflows) that use them. Think of Tasks like tidy, isolated "functions that do one specific thing".**
...
|  Option                       |  Description                                                                       | Hey |
| --- | --- | --- |
|  `slug`  |  Define a slug-based name for this job. This slug needs to be unique among both tasks and workflows. | lalala |
