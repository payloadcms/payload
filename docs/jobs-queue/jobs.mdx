...
Payload increments `totalTried` after each attempt. While the handler runs, Payload automatically renews `processingUntil`. If the worker disappears and the lease expires, the next jobs runner call counts the abandoned attempt and recovers the job using its existing retry and backoff configuration.
