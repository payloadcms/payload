...
| `concurrency`   | Control how jobs with the same concurrency key are handled. Jobs with the same key will run exclusively (one at a time). Concurrency controls are enabled by default. See [Concurrency Controls](#concurrency-controls) below for details.                                                                                                                                                                                                                                                                                                                                                                               |
...
<Banner type="warning">
  **Important:** Concurrency controls add an indexed `concurrencyKey` field to
  your jobs collection schema and may require a database migration depending on
