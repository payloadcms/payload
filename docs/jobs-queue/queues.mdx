...
Running jobs are owned through a renewable `processingUntil` lease and an internal `processingToken`. Payload claims eligible jobs with an atomic database update, then renews the lease while the worker is alive. Every worker update must still match the token, so a worker that lost ownership cannot overwrite the new attempt.

If the worker is killed, a later jobs runner call recovers the expired job and applies its existing retry configuration. Multiple runners can safely race for the same job because only one atomic claim succeeds.

The default lease duration is two minutes. You can change it globally:

```ts
export default buildConfig({
  jobs: {
    processingLeaseDuration: 5 * 60 * 1000,
    tasks: [
      // your tasks here
    ],
  },
...
This is not a maximum job runtime. Healthy long-running jobs keep renewing their leases. Expiry does not start a new process by itself, so serverless deployments must continue calling the jobs runner from a cron or another callback system.
...
- `processingUntil` in the past → The worker may have crashed; the next runner call will recover it
