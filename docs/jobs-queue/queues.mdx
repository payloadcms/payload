...
If the worker is killed, a later jobs runner call recovers the expired job and applies its existing retry configuration.

By default, Payload only starts a worker update when more than 30 seconds remain on its lease. This safety buffer gives the update time to finish before the lease expires and another worker can recover the job. If too little time remains, the update is rejected and the job is recovered after the lease expires.

The default lease duration is two minutes and the default safety buffer is 30 seconds. You can change both globally:
