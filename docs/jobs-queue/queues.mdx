...
<Banner type="default">
  A **Queue** is a grouping of jobs that should be executed in order
...
<Banner type="warning">
  autoRun is intended for use with a dedicated server that is always
  running, and should not be used on serverless platforms like Vercel.
</Banner>
