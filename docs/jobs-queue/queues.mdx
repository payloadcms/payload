...
As mentioned above, you can queue jobs, but the jobs won't run unless a worker picks up your jobs and runs them. This can be done in three ways:
...
export default buildConfig({
  ...
  jobs: {
    ...
    autoRun: (payload) => [
      {
        default: '*/5 * * * *', // every hour at minute 0
        limit: 100, // limit of retries
        queue: 'nightly', // name of the queue
      },
...
Important: autoRun is intended for use with a dedicated server that is always running, and should not be used on serverless platforms like Vercel. 
