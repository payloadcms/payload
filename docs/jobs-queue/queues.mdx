...
Running jobs are owned through a renewable `processingUntil` lease. Payload renews the lease while the worker is alive. If the worker is killed, a later jobs runner call recovers the expired job and applies its existing retry configuration.
