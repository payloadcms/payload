...
export default buildConfig({
  ...
  jobs: {
    ...
    shouldAutoRun: async (payload) => {
      ...
      // If this function returns false, this run is skipped and the cron checks again on its next scheduled run.
      return true
    },
  },
})
...
const results = await payload.jobs.run({ overrideAccess: true })

// You can customize the queue name and limit by passing them as arguments:
await payload.jobs.run({ queue: 'nightly', limit: 100, overrideAccess: true })
...
await payload.jobs.queue({ task: 'myTask', overrideAccess: true })
