...
| `idType`                          | The Mongoose schema type used for the auto-generated `_id` of collections that don't define a [custom ID field](../fields/overview#custom-id-fields). Accepts `String`, `Number`, `mongoose.Schema.Types.ObjectId`, `mongoose.Schema.Types.BigInt`, or `mongoose.Schema.Types.UUID`. Defaults to MongoDB's native `ObjectId`. See [Setting a custom ID type](#setting-a-custom-id-type). |
...
By default, MongoDB stores each document's `_id` as a native [`ObjectId`](https://www.mongodb.com/docs/manual/reference/method/ObjectId/). You can change the type used for the auto-generated ID of every collection that doesn't define its own [custom ID field](../fields/overview#custom-id-fields) with the `idType` option.

This is most commonly used to store IDs as `UUID`s. Unlike generating a UUID yourself in a custom `id` field of type `text` (which is stored as a 36-byte string), `mongoose.Schema.Types.UUID` stores the value as MongoDB's native 16-byte binary UUID, which is more compact and more performant.
