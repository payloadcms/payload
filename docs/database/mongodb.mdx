...
| `idType`                          | The ID type used for the auto-generated `_id` of collections that don't define a [custom ID field](../fields/overview#custom-id-fields). One of `'objectId'`, `'uuid'`, `'text'`, `'number'`, or `'bigint'`. Defaults to `'objectId'` (MongoDB's native `ObjectId`). See [Setting a custom ID type](#setting-a-custom-id-type). |
...
## Setting a custom ID type

By default, MongoDB stores each document's `_id` as a native [`ObjectId`](https://www.mongodb.com/docs/manual/reference/method/ObjectId/). You can change the type used for the auto-generated ID of every collection that doesn't define its own [custom ID field](../fields/overview#custom-id-fields) with the `idType` option. It accepts `'objectId'` (the default), `'uuid'`, `'text'`, `'number'`, or `'bigint'`.

This is most commonly used to store IDs as `UUID`s. Unlike generating a UUID yourself in a custom `id` field of type `text` (which is stored as a 36-byte string), `idType: 'uuid'` stores the value as MongoDB's native 16-byte binary UUID, which is more compact and more performant.
...
export default buildConfig({
  ...
  db: mongooseAdapter({
    url: process.env.DATABASE_URL,
    // Use UUIDs for the ID of every collection without a custom ID field
    idType: 'uuid',
  }),
})
```
...
<Banner type="warning">
  `idType` only applies to collections that rely on the auto-generated `_id`.
  Collections that define a [custom ID
  field](../fields/overview#custom-id-fields) always use that field's type. This
  option should be set once, before creating any documents — changing it on a
  database that already has data will break lookups against the existing IDs.
</Banner>
