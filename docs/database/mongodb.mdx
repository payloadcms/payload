...
To enable collation, enable `useCollation` and then set the `collation` option in the adapter configuration:
...
export default buildConfig({
  ...
  db: mongooseAdapter({
    ...
    useCollation: true, // IMPORTANT: Enable collation support, this will be automatically turned on in v4
  }),
})
```

<Banner type="warning">
  useCollation is an experimental feature in v3 and will be enabled by default
  in v4. Please test thoroughly before using in production as it can affect your
  queries.
</Banner>
