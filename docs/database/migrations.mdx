...
Check if there are any schema changes that would require a new migration to be created.
Exits with code 0 if there are changes, and code 1 if there are no changes.

```text
npm run payload migrate:has-changes
```
