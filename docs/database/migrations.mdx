...
```bash
# 1. Check if changes need a migration (exit code 0 = changes, 2 = no changes)
if payload migrate:create --dry-run --skip-empty; then
  # 2. Changes detected — create the migration
