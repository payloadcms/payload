...
- `--check`: Check if there are any schema changes that would require a new migration to be created. Exits with code 1 if there are changes, and code 0 if there are no changes. Useful for CI to ensure that all schema changes have corresponding migrations.
