...
When upgrading a SQL project to a Payload release that adds upload file versioning, generate a migration in that project after updating Payload and before deploying with `push: false`. Inspect the generated migration for each upload collection: it should add nullable `original` metadata and `_managedFiles` columns to both the collection and its versions table, if versions are enabled. Then run `npm run payload migrate` during deployment. Payload does not ship a single migration for these fields because upload collection tables are defined by each project's config. Existing rows retain null values until they are read or updated by the versioning flow.
