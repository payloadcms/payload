...
Local API calls that pass a `user` are checked, even with `overrideAccess: true`. If server-side code needs to write across tenants, omit the `user` or use a user that passes `userHasAccessToAllTenants`.
