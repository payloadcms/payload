...
`toCSV` and `fromCSV` are deprecated and will be removed in v4.0. Existing hooks continue to work unchanged. New code should use `hooks.beforeExport` / `hooks.beforeImport`, which also run for JSON exports and imports.
