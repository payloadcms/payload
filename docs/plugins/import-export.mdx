...
Both are read once when the run starts and passed unchanged to every batch. They are a snapshot of the document at that moment, not live state — editing the document while a large import runs does not change what later batches receive.
...
<Banner type="warning">
  On the paths without an `id`, this is the submitted request body — it has not been through
  field validation or access control, so treat those values as untrusted input. The plugin drops
  `id`, `createdAt` and `updatedAt` from the body, so a caller cannot make an unsaved run look
  saved, but every other field is whatever was sent. Gate privileged behaviour on
  `if (exportDoc.id) { ... }`.
</Banner>
