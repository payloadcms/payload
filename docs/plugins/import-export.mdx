...
| `toCSV`              | function | **Deprecated** — use `hooks.beforeExport` instead. Removed in v5.           |
| `fromCSV`            | function | **Deprecated** — use `hooks.beforeImport` instead. Removed in v5.           |
...
`toCSV` and `fromCSV` are deprecated and will be removed in v5. Existing hooks continue to work unchanged. New code should use `hooks.beforeExport` / `hooks.beforeImport`, which also run for JSON exports and imports.
