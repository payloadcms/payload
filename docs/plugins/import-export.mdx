...
| `export`   | function | Field-level export hook — transforms the value when exporting (CSV and JSON). |
| `import`   | function | Field-level import hook — transforms the value when importing (CSV and JSON). |
| `toCSV`    | function | **Deprecated** — use `export` instead. Removed in v4.0.                       |
| `fromCSV`  | function | **Deprecated** — use `import` instead. Removed in v4.0.                       |
...
### Field-Level Export Hook

Use `export` to transform a field's value when exporting. Works for both `csv` and `json` formats.

The `export` function receives:

| Property     | Type    | Description                                                                                   |
| ------------ | ------- | --------------------------------------------------------------------------------------------- |
| `columnName` | string  | The field path/column name (underscore-separated for nested fields).                          |
| `data`       | object  | Alias for `row` — kept for compatibility.                                                     |
| `doc`        | object  | The top-level document being exported.                                                        |
| `format`     | string  | Export format: `'csv'` or `'json'`.                                                           |
| `row`        | object  | For CSV: the flat row accumulator. For JSON: the sibling object at the current nesting level. |
| `siblingDoc` | object  | The source document data at the same nesting level as the field.                              |
| `value`      | unknown | The field value from the database document.                                                   |

Return a value to replace the field, `undefined` to use default behavior, or mutate `row` to add extra columns at the same level.

Example — splitting a relationship into multiple columns:

```ts
import type { FieldExportHook } from '@payloadcms/plugin-import-export'
...
const authorFieldWithHooks = {
  ...
  custom: {
    'plugin-import-export': {
      export: (({ value, columnName, row, format }) => {
        if (
          format === 'csv' &&
          value &&
          typeof value === 'object' &&
...
### Field-Level Import Hook

Use `import` to transform a field's value when importing. Works for both `csv` and `json` formats.

The `import` function receives:

| Property     | Type    | Description                                                |
| ------------ | ------- | ---------------------------------------------------------- |
| `columnName` | string  | The field path/column name.                                |
| `data`       | object  | The full flat row (CSV) or document (JSON) being imported. |
| `format`     | string  | Import format: `'csv'` or `'json'`.                        |
...
```ts
import type { FieldImportHook } from '@payloadcms/plugin-import-export'

{
  ...
  custom: {
    'plugin-import-export': {
      import: (({ value, columnName, data, format }) => {
        if (format === 'csv') {
...
1. **Field-level `export`/`import`**: run per-field, per-document, during transformation
...
`toCSV` and `fromCSV` are deprecated and will be removed in v4.0. Migrating is a 1:1 rename — the only addition is the `format` arg (which you can ignore if you don't need it):
