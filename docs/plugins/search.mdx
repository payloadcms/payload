...
The `shouldSkipSync` function allows you to conditionally skip syncing specific documents to the search index based on locale, document properties, or any other criteria. This is particularly useful for multi-tenant applications where different tenants use different languages, or when you need fine-grained control over what gets indexed.
...
// payload.config.ts
{
  // ...
  searchPlugin({
    shouldSkipSync: async ({ locale, doc, collectionSlug, req }) => {
...
- `locale`: The locale being synced (e.g., `'en'`, `'es'`), or `undefined` for non-localized collections
