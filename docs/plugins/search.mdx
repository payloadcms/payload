...
This plugin generates records of your documents that are extremely fast to search on. It does so by creating a new `search` collection that is indexed in the database, then it saves a static copy of each of your documents using only search-critical data. Search records are automatically created, synced, and deleted behind-the-scenes as you manage your application's documents.

When querying search results, simply use all the Payload APIs that you are already familiar with. Search records are static, so querying these records has the advantage of bypassing any hooks that may be present on the underlying document. You can also prioritize search results by defining a custom priority for each collection. For example, you may want to prioritize blog posts over pages. Or you may want one specific post to always take appear first.
