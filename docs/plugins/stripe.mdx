...
With this plugin you can easily integrate [Stripe](https://stripe.com) into Payload as the payment processor for your next e-commerce or SaaS application. Simply provide your Stripe credentials and this plugin will open up a two-way communication channel between the two platforms. This enables you to easily sync data back and forth, as well as proxy the Stripe REST API through Payload's access control. Use this plugin to completely offload billing to Stripe, while you retain full control over your application's data.

For example, you may have a `products` collection in Payload that you wish to sell. You can to tie each of your products to Stripe, whether they require one-time payments or a subscription. Then you can easily open Stripe webhook handlers to react to billing-related events in real-time as they take place, such as an active purchases, or subscription cancellations. The entirety of your application's business logic can be handled in Payload, while Stripe handles the billing alone.

To build a checkout flow on your front-end you can either use one of Stripe's turn-key checkout solutions, or you can build a completely custom checkout experience using one of their robust component libraries. But instead of opening three or four _different_ endpoints for this, such as looking up the customer or retrieving their default payment methods, you can proxy the Stripe API through a single, consolidated route.
...
The [Templates Directory](https://github.com/payloadcms/payload/tree/main/examples) contains an official [E-commerce Template](https://github.com/payloadcms/payload/tree/main/templates/ecommere) which demonstrates exactly how to configure this plugin into your Payload instance and implement it on your front-end. You can also check out the [How to Build An E-Commerce Site With Next.js](https://payloadcms.com/blog/how-to-build-an-e-commerce-site-with-nextjs) blog post for a bit more context.
