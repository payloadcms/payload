...
<Banner type="info">
  ...
  here](https://github.com/payloadcms/payload/tree/3.x/packages/plugin-stripe).
...
The plugin provides the following custom endpoints. The Stripe REST proxy is registered only when the `rest` option enables it:

| Endpoint               | Method | Description                                                                                                                                                                                                                                       |
| ---------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/api/stripe/rest`     | `POST` | When enabled, proxies allowlisted [Stripe REST API](https://stripe.com/docs/api) methods behind [Payload access control](https://payloadcms.com/docs/access-control/overview). See the [REST Proxy](#stripe-rest-proxy) section for more details. |
| `/api/stripe/webhooks` | `POST` | Handles all Stripe webhook events                                                                                                                                                                                                                 |

##### Stripe REST Proxy

Enable the REST proxy with an explicit allowlist of exact Stripe method names. Omit `rest` to disable the endpoint. Requests require an authenticated user and default to Payload admin access; provide `access` to use a custom access callback. Requests for methods outside the allowlist are rejected before reaching Stripe.
...
The old boolean forms are no longer accepted. Replace `rest: true` with an explicit object configuration, or omit `rest` to disable the endpoint.
...
Opening the REST proxy endpoint in production is a potential security risk. Keep `allowedMethods` narrowly scoped and ensure the default admin access or your custom `access` callback is appropriate for every allowed method. For server-side use, consider the [stripeProxy](#node) function instead.
