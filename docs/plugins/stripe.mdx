...
| `rest`                         | `StripeRESTConfig` | `undefined` | Opens the `/api/stripe/rest` endpoint with an exact Stripe method allowlist                                              |
...
The webhook endpoint is opened automatically. The REST proxy endpoint is registered only when
`rest` is configured:

| Endpoint               | Method | Description                                                                                                                                                                                                                                                           |
| ---------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `/api/stripe/rest`     | `POST` | When `rest` is configured, proxies the [Stripe REST API](https://stripe.com/docs/api) behind [Payload access control](https://payloadcms.com/docs/access-control/overview) and returns the result. See the [REST Proxy](#stripe-rest-proxy) section for more details. |
| `/api/stripe/webhooks` | `POST` | Handles all Stripe webhook events                                                                                                                                                                                                                                     |

##### Stripe REST Proxy

To enable the REST proxy, configure `rest` with the exact Stripe SDK methods that requests may
invoke:
...
By default, the endpoint requires an authenticated user who can access the Payload admin panel.
This uses the auth collection's `access.admin` policy. You can replace the default with a custom
asynchronous access callback, for example to require a billing role:

```ts
stripePlugin({
  stripeSecretKey: process.env.STRIPE_SECRET_KEY,
  rest: {
    access: async ({ req }) => Boolean(req.user?.roles?.includes('billing')),
    allowedMethods: ['subscriptions.list'],
  },
})
```

Entries in `allowedMethods` are exact method names such as `subscriptions.list`; wildcard entries
are not supported. Keep the allowlist as narrow as possible. For customer- or resource-specific
authorization, prefer a narrow application endpoint that validates the request before calling
Stripe.

After enabling and configuring the proxy, call it with an allowlisted method and an array of
arguments:
...
For Payload v4, migrate `rest: true` to the allowlisted configuration:

```ts
// Before: insecure and no longer accepted
rest: true

// After: admin-only by default and exactly allowlisted
rest: {
  allowedMethods: ['subscriptions.list'],
}
```

To disable the proxy, omit `rest` instead of setting `rest: false`.
...
Opening the REST proxy endpoint in production is sensitive because every allowlisted method uses
your Stripe secret key. An allowlist does not enforce resource ownership or application-specific
billing rules. Prefer a narrow application endpoint and use the [stripeProxy](#node) function
server-side when you need those checks.
