...
The `forms` and `form-submissions` collections track [authorship](../configuration/collections#authorship) by default (`createdBy` / `updatedBy`); set `authorship: false` in `formOverrides` / `formSubmissionOverrides` to disable it.
...
#### Using Upload Instructions for Large Files

If your upload collection uses a storage adapter with `clientUploads` enabled (e.g., S3, Azure, GCS), the browser can upload files directly to cloud storage instead of through Payload. The upload instructions endpoint returns the URL and headers needed for the direct upload:

```ts
async function uploadWithInstructions(file: File, collectionSlug: string) {
  // 1. Get upload instructions from Payload
  const instructionsResponse = await fetch('/api/upload-instructions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    credentials: 'include',
    body: JSON.stringify({
  ...
  const uploadResponse = await fetch(url, { ...request, body: file })
...
Other adapters may return a named instruction for their own browser helper instead of an HTTP request. The example above reports those instructions as unsupported.
