...
### `custom.overridePaymentFields`

Override the default payment fields by sending an array of [Payload fields](https://payloadcms.com/docs/fields/overview) to the `custom.overridePaymentFields` property.

<Banner type="info">
  By default, the payment block includes basic fields for handling payments. You can override these fields to match your specific payment processing needs, such as adding custom fields for different payment providers or tracking additional payment metadata.
</Banner>
