...
If your upload collection uses a storage adapter with `clientUploads` enabled (e.g., S3, Azure, GCS), you can upload files directly to cloud storage for better performance with large files. The upload instructions endpoint returns the URL and headers needed for the direct upload:
...
async function uploadWithInstructions(file: File, collectionSlug: string) {
  ...
  formData.append(
    ...
    JSON.stringify({
      ...
      collectionSlug,
      filename: instructions.filename ?? file.name,
      mimeType: file.type,
      size: file.size,
    }),
  )
