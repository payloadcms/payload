...
The `paymentFields` property allows you to customize or disable the default payment-related fields that are automatically included in the form submission collection when a payment field is added to your form. Pass a function that can:

- Return `false` to disable payment fields entirely
- Return `true` to use the default payment fields (with a group field wrapper)
- Accept `defaultPaymentFields` as a parameter and return custom payment fields

<Banner type="info">
  The `defaultPaymentFields` are returned as individual fields without a group field wrapper. This gives
  you the flexibility to use these fields within your own custom field structure while still
  maintaining easy access to the default payment fields.
</Banner>
