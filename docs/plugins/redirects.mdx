...
The `redirects` collection tracks [authorship](../configuration/collections#authorship) by default (`createdBy` / `updatedBy`); set `authorship: false` in `overrides` to disable it.
