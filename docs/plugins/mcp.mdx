...
```ts
import { type MCPAccess, mcpPlugin } from '@payloadcms/plugin-mcp'

// ... other config

mcpPlugin({
  overrideAuth: (req, getDefaultMCPAccess) => {
    const { payload } = req

    // This will return the default MCPAccess
    // getDefaultMCPAccess()
...
If you want the default `MCPAccess`, you can use the additional argument `getDefaultMCPAccess`.
This will use the Bearer token found in the headers on the req to return the `MCPAccess` related to the user assigned to the API key.
