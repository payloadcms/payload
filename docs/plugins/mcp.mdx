...
The recommended approach for most MCP clients is to use the `mcp-remote` package via `npx`. Below are configuration examples for popular MCP clients.

#### VSCode MCP Extension

Add your Payload MCP server to VSCode's settings (`.vscode/settings.json` or user settings):
...
Add your Payload MCP server to Cursor's configuration file:
...
For other MCP clients that support direct HTTP connections, you can use this configuration format:
...
- **Recommended:** The `npx mcp-remote` approach is recommended as it provides better compatibility across different MCP clients
...
// Use Spanish as fallback instead of default
```
