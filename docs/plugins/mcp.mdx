...
That's it for local development - every collection and global is now available through the generic MCP tools. For example, call `getConfigInfo` to see available slugs, inspect a collection with `getCollectionSchema`, then create one or more documents with `createDocuments`.
...
The `create` key controls `createDocuments`, which handles both single and
...
`createDocuments` accepts a `documents` array with at least one item. Each item
has its own `data` and optional `file`. Options such as `depth`, `draft`,
`locale`, `fallbackLocale`, and `select` apply to the whole request.

Documents are created one at a time through Payload's Local API. The operation
is best-effort: a failed item does not roll back successful items. Results and
errors include their original zero-based input index so failed items can be
retried without creating successful items again. The tool has no fixed item
limit, but normal request-size and execution-time limits still apply.
...
The built-in `findDocuments`, `findGlobal`, `createDocuments`, `updateDocument`, and `updateGlobal` tools accept an optional `select` parameter. It limits which fields come back, which significantly reduces token usage on large documents.
...
| `toolName` | The registered MCP tool name, such as `createDocuments` or `getConfigInfo`.      |
