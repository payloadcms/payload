...
If you're running the admin panel on TanStack Start instead of Next.js, follow Sentry's [Vite / React setup guide](https://docs.sentry.io/platforms/javascript/guides/react/) before configuring the Payload `sentryPlugin`. The Payload plugin configuration itself is identical regardless of framework.
