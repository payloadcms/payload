...
<Banner type="warning">
  This plugin currently supports Sentry 10.x. Keep the Sentry SDK version used
  by your app aligned with the plugin's Sentry major version, and prefer the
  latest available 10.x release.
</Banner>
...
<Banner type="info">
  Sentry 10.x upgrades OpenTelemetry support to v2. If your app relies on custom
  OpenTelemetry setup or self-hosted Sentry, review Sentry's migration notes and
  use a self-hosted version of at least `24.4.2`.
</Banner>
...
- `captureErrors`: number[] | optional
  ...
  By default, this plugin captures only errors with a status code of `500` or higher
  from Payload's `afterError` hook. To capture additional error codes, pass the
  values as numbers in an array.
