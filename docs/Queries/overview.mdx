...
<Banner>
  <strong>Here, "querying" relates to filtering or searching through documents within a Collection.</strong> You can build queries to pass to Find operations as well as to <a href="/docs/access-control/config">restrict which documents certain users can access</a> via access control functions.
</Banner>
...
You may eventually have a lot of documents within this Collection. If you wanted to find only documents with `color` equal to `red`, you could write a query as follows:

```js
const query = {
  color: { // property name to filter on
    equals: 'red', // operator to use and value to compare against
  },
}
```
...
Written in plain English, if the above query were passed to a `find` operation, it would translate to finding posts where either the `color` is `red` OR the `color` is `white` AND `featured` is set to false.
...
**`https://localhost:3000/api/posts?where[color][equals]=red`**
