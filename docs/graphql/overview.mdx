...
GraphQL is opt-in. Its packages are optional peer dependencies of `@payloadcms/next`, so projects
that only use the REST and Local APIs do not install or bundle them.

The templates and `create-payload-app` include GraphQL by default. If you are adding it to an
...
and make sure the GraphQL routes exist in your app:
...
<Banner type="info">
  To remove GraphQL from a project, delete those two route files and uninstall
  the packages above. Setting `graphQL.disable` to `true` also turns the
  endpoints off, but keeps the packages installed.
</Banner>
