...
| `disablePlaygroundInProduction`    | A boolean that if false will enable GraphiQL IDE in production environments, defaults to true. [More](/docs/graphql/overview#graphiql) |
...
| `disable`                          | A boolean that if true will disable GraphQL entirely, defaults to false.                                                           |
...
GraphiQL IDE is enabled by default for development purposes, but disabled in production. You can enable it in production by passing `graphQL.disablePlaygroundInProduction` a `false` setting in the main Payload Config.

You can even log in using the `login[collection-singular-label-here]` mutation to use GraphiQL as an authenticated user.

<Banner type="success">
  **Tip:**

To see more regarding how the above queries and mutations are used, visit GraphiQL
(by default at
