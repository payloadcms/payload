...
When working with GraphQL it is useful to have the schema for development. In Payload the schema is controlled by your collections and globals and is made available to the developer or third parties, it is not necessary for developers using Payload to write schema types manually. The schema definitions are made accessible to in two possible ways.
...
You can run this command whenever you need to regenerate your graphQL schema and output it to a file, and then you can use the schema for writing your own graphQL.

### Query GraphQL schema

While your project is running you can query for any schema definitions from the GraphQL endpoint using an `IntrospectionQuery` query.
