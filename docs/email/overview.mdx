...
in the `email` property object of your payload initAsync call. Payload will make use of the transport that you have configured for it for things like reset password or verifying new user accounts and email send methods are available to you as well on your payload instance.
...
The following options are configurable in the `email` property object as part of the options object when calling payload.initAsync().
...
To take full control of the mail transport you may wish to use `nodemailer.createTransport()` on your server and provide it to Payload initAsync.
