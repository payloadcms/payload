...
If you are experiencing a CPU spike when performing certain operations, it may be due to infinite loops in your hooks. This can happen if a hook triggers an operation that calls the same hook again, leading to a recursive loop that can potentially crash your server.

To learn more, see the [Hooks Context documentation](https://payloadcms.com/docs/hooks/context#preventing-infinite-loops).
