...
This happens because one package imports a hook (most commonly `useConfig`) from version A while the context provider comes from version B. The fix is always the same: make sure every payload-related and React package resolves to one identical version in one location.

### Confirm whether duplicates exist

**pnpm built-in inspection:**
...
**(pnpm only) Clean the global store**

```bash
pnpm store prune
```

**Delete the lockfile (pnpm-lock.yaml, package-lock.json, or yarn.lock)**
