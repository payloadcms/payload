...
You can also define access control to control exactly who can make changes to the specific constraints of a preset. This is useful in restricting the ability to set a specific constraint to only certain users.

For example, you want to ensure that only admins are allowed to make a preset available to everyone. When a user lacks the permission to set a constraint, the option will either be hidden from them or disabled if it is already set.

To do this, you can use the `allowedConstraints` property in your [Payload Config](../configuration/overview):
