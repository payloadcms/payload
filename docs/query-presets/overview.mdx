...
| `reduceConstraints` | Used to define which constraints are available to users when managing presets. [More details](#constraint-access-control).      |
...
You can also specify exactly who can change the constraints within a preset. This is useful to grant or restrict access to certain constraints based on any given user criteria. For example, you want to ensure that only "admins" are allowed to make a preset available to "everyone".
...
To do this, you can use the `reduceConstraints` property in your [Payload Config](../configuration/overview):
...
The `reduceConstraints` function receives the same arguments as [`reduceOptions`](../fields/select#reduceOptions) in the [Select field](../fields/select).
