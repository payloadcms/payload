...
| `allowedConstraints` | Used to define which constraints are available to users when managing presets. [More details](#constraint-access-control).      |
  ...
  queryPresets: {
    ...
    allowedConstraints: ({ req, options }) =>
      !req.user?.roles?.includes('admin')
        ? options.filter((option) => option !== 'everyone')
        : options,
    // highlight-end
  },
})
