...
<Banner type="warning">
  ...
  The [default Payload editor](/docs/text-editor/overview) is currently based on Lexical. This documentation 
  is about our old Slate-based editor. You can continue using it because it is still supported, or you can 
  see the optional [migration guide](/docs/text-editor/migration) to migrate from Slate to Lexical (recommended).
