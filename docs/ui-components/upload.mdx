...
The empty `Upload` state composes Payload's `Dropzone` with controls for selecting a local file or pasting a remote URL.

<ComponentPreview component="Dropzone" example="uploadEmptyState" />
...
The **Create New** and **Choose from existing** controls shown by an Upload field belong to the relationship field interface, which selects documents from an upload-enabled Collection. They are not rendered by this `Upload` component or by `Dropzone`.
