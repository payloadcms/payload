...
Use `success` to confirm an action, `error` when something failed, and `info` for additional context. Use the default type for neutral messages.
...
| `type`      | `'default' \| 'success' \| 'error' \| 'info'` | `'default'` | Sets the message style.                     |
