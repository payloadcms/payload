...
<Banner type="info">
  This walkthrough uses Next.js' `draftMode()` API and a `route.ts` handler.
  TanStack Start has no `draftMode()` equivalent — implement the same concept in
  a [server function](../local-api/server-functions) that verifies the preview
  secret and sets your own draft cookie, then read that cookie in your loaders
  to toggle `draft: true`.
</Banner>
