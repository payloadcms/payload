...
Collection Views are views that are scoped under the `/collections` route, such as the List and Edit views. You can easily swap out Collection Views with your own or [create entirely new ones](#adding-new-tabs-to-edit-view), through the `admin.components.views` property of your [Collection Config](../collections/overview).
...
<Banner type="warning">
  <strong>Note:</strong>
  The `Edit` property will replace the _entire_ Edit View, including the title, tabs, etc., _as well as all nested [Document Views](#custom-document-views)_, such as the API, Live Preview, and Version views. To replace only the Edit View specifically, use the `Edit.Default` key instead.
</Banner>
...
Global Views are views that are scoped under the `/globals` route, such as the Edit View. You can easily swap out Global Views with your own or [create entirely new ones](#adding-new-tabs-to-edit-view), through the `admin.components.views` property of your [Global Config](../globals/overview).
...
Each Document View can be given a new tab in the Edit View, if desired. Tabs are completely configurable, from simply changing the label to swapping out the entire component. To add or customize tabs in the Edit View, use the `Component.Tab` key:
...
Custom Views are [Custom Components](./components) rendered at a high-level. To understand how to build Custom Views, first review the [Building Custom Components](./components#building-custom-components) guide. Once you have a Custom Component ready, you can use it as a Custom View.
...
| `navGroups`               | The navigation groups.                                                                                |
