...
| **`access`**       | Access control function that receives `req` and must return a boolean. When true, this path will be accessible to the given request. Combine it with `exact: true` if you want nested paths to not have the same logic. |
