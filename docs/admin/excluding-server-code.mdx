...
import hook from './hooks/createStripeSubscription' // <-- contains server-only code
```

By default the browser bundle will now include the code from that file, even though code in that file is only meant to run on the server. This will throw an error and prevent the the admin panel from loading.

To fix this, we need to alias the `createStripeSubscription` file to a different file so the browser bundle is built with a browser-safe file.
...
const pathToFileWithServerOnlyModule = path.resolve(
  __dirname,
