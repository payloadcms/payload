...
files using server-only modules need to be excluded.
...
Say we have a collection called `Subscriptions` that has a `beforeChange` hook that
creates a Stripe subscription whenever a Subscription document is created in Payload.
...
#### How to fix this
If you are using the Webpack bundler, you can use the [Webpack path aliasing approach](/docs/admin/webpack#aliasing-server-only-modules).

TODO: ADD LINK TO VITE DOCS BELOW

If you are using the Vite bundler you can use [Vite's `optimizeDeps` config](/docs/admin/vite#optimizing-dependencies).
