...
Root Metadata is the metadata that is applied to all pages within the Admin Panel. This is where you can control things like suffix appended onto the page title, the favicon displayed in the browser tab, and the Open Graph data that is used when sharing the Admin Panel on social media.
