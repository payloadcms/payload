...
This metadata is used to generate the `<head>` of pages within the Admin Panel. On Next.js it is injected into the [`generateMetadata`](https://nextjs.org/docs/app/api-reference/functions/generate-metadata) function; on TanStack Start it feeds the route's `head`. The metadata option shape follows Next.js' [`Metadata`](https://nextjs.org/docs/app/api-reference/functions/generate-metadata) API in both cases, and all of those options are exposed by Payload.
