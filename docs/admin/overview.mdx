...
When using the admin panel Payload stores user specific preference data to the database in the `_preferences` collection. Each `preference` is tied to one user and accessed using a `key` string. The preference `value` is not given a type and can be any data needed such as a `locale` string or collection list columns as an array of strings. Preferences are available to both [GraphQL](/docs/graphql/overview#preferences) and [REST](/docs/rest-api/overview#) APIs. The preferences endpoints can be used to add functionality to custom components as part of the admin UI or for an external need. It can be thought of a user specific key-value store.
