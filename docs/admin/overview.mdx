...
| Option            | Description                                     |
| ----------------- | ----------------------------------------------- |
| `options`         | An array of label/value options for selectable timezones where the value is the IANA name eg. `America/Detroit`                        |

<Banner type="info">
  **Good to know**
  You can enable timezones on each individual date field. See [Date Fields](../fields/overview#date) for more information.
</Banner>
