...
The `admin` directory contains all the _pages_ related to the interface itself. The `api` and `graphql` directories contains all the _routes_ related to the [REST API](../rest-api/overview) and [GraphQL API](../graphql/overview). And all [admin routes are easily configurable](#customizing-routes) to meet your application's requirements.
...
Root-level routes are those that are not behind the `/admin` path. You can configure custom paths for the following root-level routes through the `routes` property:
...
Admin-level routes are those behind the `/admin` path. You can configure custom paths for the following admin-level routes through the `admin.routes` property of your [Payload Config](../configuration/overview):
