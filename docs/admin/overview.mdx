...
The Admin Panel is designed to [white-label your brand](https://payloadcms.com/blog/white-label-admin-ui). You can endlessly customize and extend the Admin Panel UI by swapping in your own [Custom Components](../custom-components/overview)—everything from simple field labels to entire views can be modified or replaced to perfectly tailor the interface for your editors.

The Admin Panel is written in [TypeScript](https://www.typescriptlang.org) and built with [React](https://react.dev) with your choice of [Next.js App Router](https://nextjs.org/docs/app) or [TanStack Start](https://tanstack.com/start/latest). It supports [React Server Components](https://react.dev/reference/rsc/server-components), enabling the use of the [Local API](/docs/local-api/overview) on the front-end. You can install Payload into your [existing app in just one line](../getting-started/installation) and [deploy it anywhere](../production/deployment).
...
The Admin Panel serves as the entire HTTP layer for Payload, providing a full CRUD interface for your app. This means that both the [REST](../rest-api/overview) and [GraphQL](../graphql/overview) APIs are routes that exist directly alongside your front-end application.
...
Payload integrates with Next.js through a small set of helpers exported from `@payloadcms/next`. The `(payload)` [route group](https://nextjs.org/docs/app/building-your-application/routing/route-groups) creates a boundary that scopes all Admin Panel layouts and styles.

The `layout.tsx` file within this directory, for example, is where Payload manages the `html` tag of the document to set proper [`lang`](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/lang) and [`dir`](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/dir) attributes, etc.
...
Finally, the `custom.css` file is where you can add or override globally-oriented styles in the Admin Panel, such as modify the color palette. Customizing the look and feel through CSS alone is a powerful feature of the Admin Panel, [more on that here](./customizing-css).
...
<Banner type="info">
  If you are not familiar with TanStack Start project structure, you can [learn
  more about it
  here](https://tanstack.com/start/latest/docs/framework/react/build-from-scratch).
</Banner>

Payload integrates with TanStack Start through a small set of helpers exported from `@payloadcms/tanstack-start`. Rather than a Next.js route group, the `_payload` [pathless layout route](https://tanstack.com/router/latest/docs/framework/react/routing/routing-concepts#pathless-layout-routes) creates the boundary that scopes all Admin Panel layouts and styles.

The `__root.tsx` file is the single integration touch point for the document shell. Wrapping your root's `shellComponent` with `withPayloadRoot` renders the Payload admin document (managing the `html` tag's [`lang`](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/lang) and [`dir`](https://developer.mozilla.org/en-US/docs/Web/HTML/Global_attributes/dir) attributes, theme, etc.) on `/admin` routes, and your own shell everywhere else. The `_payload.tsx` layout route then mounts Payload's providers via `payloadLayoutRoute`.

The `_payload` directory contains the routes that power the Admin Panel:

- `admin.$.tsx` and `admin.index.tsx` are the _pages_ of the interface itself, wired through `payloadAdminSplatRoute` and `payloadAdminIndexRoute`.
- `api.$.ts` mounts the [REST API](../rest-api/overview) via `payloadApiHandlers`.
- `server.functions.ts` defines the [server functions](https://tanstack.com/start/latest/docs/framework/react/server-functions) that load [React Server Components](https://react.dev/reference/rsc/server-components) and dispatch Payload's server actions — the TanStack Start equivalent of Next.js route handlers.
- `importMap.js` is the auto-generated import map. Like its Next.js counterpart, it is the only file regenerated automatically; do not edit it by hand.

The `router.tsx` file is where you create the router. Payload's list-view `where` filter is serialized with `qs` bracket notation, so the router must parse and stringify search params with `qs` to keep the nested query intact across navigations.
