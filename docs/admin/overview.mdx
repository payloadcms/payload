...
The Admin Panel is written in [TypeScript](https://www.typescriptlang.org) and built with [React](https://react.dev) using the [Next.js App Router](https://nextjs.org/docs/app). It supports [React Server Components](https://react.dev/reference/rsc/server-components) and the use of the [Local API](/docs/local-api/overview) on the front-end. You can install Payload into any [existing Next.js app in just one line](../getting-started/installation).

<Banner type="success">
  The Admin Panel is meant to be simple enough to give you a solid starting point without too much
  complexity, so that you can get started quickly as you fine-tune to meet the of your application and your
  editors.
</Banner>
...
All Payload routes are nested within the `(payload)` route group. This creates a boundary between the Admin Panel and the rest of your application by scoping all layouts and styles. For example, `layout.tsx` file within this directory is where Payload manages the `html` tag of the document to set proper `lang` and `dir` attributes.

The `admin` directory contains all the _pages_ related to the interface itself, while the `api` and `graphql` directories contains all the _routes_ related to the [REST API](../rest-api/overview) and [GraphQL API](../graphql/overview), respectively. All [admin routes are configurable](#customizing-routes) to meet your application's requirements.

The `custom.scss` file is where you can add or override styles to the Admin Panel. Customizing the look and feel through CSS alone is a powerful feature of the Admin Panel, [more on that here](./customizing-css).
...
All admin-specific options for the Admin Panel are defined in your Payload config under the `admin` key:

| Option            | Description                                                                                                                                                                                                                                                                 |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `user`            | The `slug` of the Collection that you want to allow to login to the Admin Panel. [More](#the-admin-user-collection).                                                                                                                                                        |
...
| `components`      | Component overrides that affect the entirety of the Admin Panel. [More](./components).                                                                                                                                                                                      |
| `routes`          | Replace built-in Admin Panel routes with your own custom routes. [More](#customizing-routes).                                                                                                                                                                               |
...
To do this, add a `roles` or similar field to your auth-enabled Collection, then use the `access.admin` property to define granular access. See [Access Control](/docs/access-control/overview) for full details. For a complete, working example of role-based access control, check out the official [Auth Example](https://github.com/payloadcms/payload/tree/main/examples/auth/payload).

## Customizing Routes

You have full control over the routes that Payload binds itself to. This includes both root-level routes such as the REST API, and admin-level routes such as the user's account page. You can customize these routes to meet the needs of your application by simply specifying the desired path in your Payload config.

#### Root-level routes

You can configure custom paths for the following root-level routes through the `routes` key:
...
Here is an example of how you might change the root path for the Admin Panel itself:
...
You can also configure custom paths for the following admin-level routes through the `admin.routes` key:
...
Here is an example of how you might change the path for the user's account page:
...
<Banner type="success">
  <strong>Note:</strong>
  <br />
  You can also customize document-level routes such as a Collection's `/:id/api` view. See [Views](./views) for more information.
</Banner>
