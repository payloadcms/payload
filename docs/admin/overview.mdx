...
The Payload Admin panel is built with [Next.js](https://nextjs.org) which handles all building. When bundled, it is code-split, highly performant (even with 100+ fields), and written fully in TypeScript.
...
| `buildPath`       | Specify an absolute path for where to store the built Payload Admin bundle used in production. Defaults to `path.resolve(process.cwd(), 'build')`.                                                                                                                            |
| `meta`            | Base meta data to use for the Payload Admin panel. Included properties are `titleSuffix`, `ogImage`, and `favicon`.                                                                                                                                                                 |
| `disable`         | If set to `true`, the entire Payload Admin panel will be disabled.                                                                                                                                                                                                                  |
| `indexHTML`       | Optionally replace the entirety of the `index.html` file used by the Payload Admin panel. Reference the [base index.html file](https://github.com/payloadcms/payload/blob/main/packages/payload/src/admin/index.html) to ensure your replacement has the appropriate HTML elements. |
| `css`             | Absolute path to a stylesheet that you can use to override / customize the Payload Admin panel styling. [More](/docs/admin/customizing-css).                                                                                                                                        |
...
| `dateFormat`      | Global date format that will be used for all dates in the Payload Admin panel. Any valid [date-fns](https://date-fns.org/) format pattern can be used.                                                                                                                              |
...
| `components`      | Component overrides that affect the entirety of the Payload Admin panel. [More](/docs/admin/components)                                                                                                                                                                             |
