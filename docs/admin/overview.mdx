...
To get started using the Admin Panel, visit the [Installation](/docs/installation) page and follow the steps to get up and running. Once finished, the following packages will be installed in your project:

- `next` — The React framework used by the Admin Panel
- `@payloadcms/next` — Source code for the Admin Panel views, routes, etc.
- `@payloadcms/ui` — Payload's React library for building views, components, etc.

You will also find the following structure in your project, notice how Payload is nested within _your_ project:
...
<Banner type="info">
  When you use Payload, you plug it into **_your_** server. This is a fundamental difference between Payload and other application frameworks. When you use Payload, you're technically adding Payload to your app, and not building a "Payload app".
</Banner>

### Adding to an existing app

Adding Payload to either a new or existing Next.js is super straightforward.

#### Using NPX

To easiest way to install into an existing app is to run `npx create-payload-app` and it will automatically detect your existing app and safely install Payload into it.

#### Manually

You could also install Payload into your existing app by hand. First, locate your `next.config.js` file and wrap it with the `withPayload` function from the `@payloadcms/next` package:
...
| `components`      | Component overrides that affect the Admin Panel generally. [More](/docs/admin/components)                                                                                                                                                                             |
