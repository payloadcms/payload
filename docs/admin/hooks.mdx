...
In any custom component you can get the selected locale code with the `useLocaleCode` hook. Here is a simple example:

```tsx
import { useLocaleCode } from 'payload/components/utilities';
...
In any custom component you can get the selected locale object with the `useLocale` hook. Unlike `useLocaleCode`, `useLocale`gives you the full locale object, consisting of a `label`, `rtl`(right-to-left) property, and then `code`. Here is a simple example:
