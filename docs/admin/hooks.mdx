...
The `usePayloadAPI` hook is a powerful tool for making API requests to your Payload instance and handling responses reactively. It allows you to fetch and interact with data while automatically updating when parameters change.
...
| **`options`** | An object containing initial request parameters. Supports `initialParams`.                      |
...
This is useful for scenarios where you need to refresh data dynamically.
