...
Route transitions are useful in showing immediate visual feedback to the user when navigating between pages. By default, any instances of `Link` from `@payloadcms/ui` will already have trigger route transitions.
...
However, you may need to trigger route transitions programmatically in other scenarios, such as when using `router.push` from `next/router`. To do this, you'll need to wrap your function calls with the `startRouteTransition` method provided by the `useRouteTransition` hook.
