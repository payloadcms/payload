...
All [Custom Field Components](./fields) are rendered on the server, and as such, only have access to static props at render time. But fields can be dynamic, like when nested in an [`array`](../fields/array) or [`blocks`](../fields/block) field. This means that the paths to their subfields are subject to change as items are added, re-ordered, or deleted.
