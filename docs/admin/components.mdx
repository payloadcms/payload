...
In any custom component you can get the selected locale with either the `useLocaleCode` or the `useLocale` hook. `useLocaleCode` only returns the locale code as a string, and `useLocale` returns the full locale object, consisting of a `label`, `rtl`(right-to-left) property, and then `code`. Here is a simple example:

```tsx
import { useLocaleCode, useLocale } from "payload/components/utilities";
...
const Greeting: React.FC = () => {
  ...
  return (
    <div>
      <span>With useLocaleCode: {trans[localeCode]} </span>;
      <span>With useLocale: {trans[locale.code]} </span>;
    </div>
  );
};
```
