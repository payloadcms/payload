...
To override Collection Components, use the `admin.components` property on any [Collection Config](../configuration/collections):
...
To override Global Components, use the `admin.components` property on any [Global Config](../configuration/globals):
...
From any Server Component, the [Payload Config](../configuration/overview) can be accessed directly off the `payload` prop:
