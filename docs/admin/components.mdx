...
As Payload compiles the Admin Panel, it checks your config for Custom Components. When detected, Payload either replaces its own default component with yours, or if none exists, renders yours outright. There are _many_ places where Custom Components are supported in Payload, but each is defined in the same way using [Component Paths](#component-paths).
...
Components using named exports are identified either by appending `#` followed by the export name. If the component is the default export, this can be omitted.
...
#### Defining Custom Components in External Packages

When using custom components from external packages, Payload cannot resolve relative paths. Instead, you must use an import path that's accessible from in your project's base directory.

For example, to export a field with a custom component from an external package named `some-external-package`:
...
export const MyCustomField: Field = {
  ...
  admin: {
    components: {
      Field: 'some-external-package/client#MyFieldComponent'
    }
  }
}
```
