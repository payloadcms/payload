...
As Payload compiles the Admin Panel, it constantly checks your config for any Custom Components. When detected, Payload will either replace its own default component with your own, or render yours outright if there is no default.

There are many places throughout the config where custom components can be defined, all of which follow the same structure.

To add a Custom Component, specify the _file path_ that points to the component:
...
Named exports are identified either by appending `#` followed by the export name. If the component is the default export, this can be omitted. For example, in the path `src/components/Logout#MyComponent`, `src/components/Logout` is the file path, and `MyComponent` is the named export.

<Banner type="info">
  <strong>Note:</strong>
  The reason why components are defined as file paths instead of named imports is to ensure the Payload Config itself is fully Node.js compatible and remains as lightweight as possible.
</Banner>

To simplify component paths, you can also configure the base directory using the `admin.importMap.baseDir` property:
...
|---------------|-------------------------------------------------------------------------------------------------------------------------------|
| **`clientProps`** | Props to be passed to the React Component if it's a Client Component. [More details](#custom-props).                         |
...
| **`serverProps`** | Props to be passed to the React Component if it's a Server Component. [More details](#custom-props).                         |

For more details on how to build Custom Components, see [Building Custom Components](#building-custom-components).

### Import Map

In order for Payload to make use of component paths, an "Import Map" is automatically generated at `app/(payload)/admin/importMap.js`. This file contains a Map of the imports for your Custom Components, keyed to their respective paths.

The Import Map is regenerated whenever you modify Custom Components. This regeneration occurs at startup and whenever Hot Module Replacement (HMR) runs.

The `payload generate:importmap` command can also be used to manually create a new Import Map. If you encounter any errors running this command, see the [Troubleshooting](../local-api/outside-nextjs#troubleshooting) section.
