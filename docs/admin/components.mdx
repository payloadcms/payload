...
When [Building Custom Components](#building-custom-components), it's still possible to use client-side code such as `useState` or the `window` object, simply define your component in a new file with the `use client` directive at the top:
...
In any Custom Component you can support multiple languages to be consistent with Payload's [i18n](../configuration/i18n) support. The best way to do this is to add your translation resources to the [i18n config](../configuration/i18n) and import `useTranslation` from `@payloadcms/ui` in your components.
