...
Projects created from older templates may include this warning in `layout.tsx`:
...
**This warning is misleading.** The file is created during initial project setup by `create-payload-app`, but it is **never regenerated** by Payload. Current templates say "MODIFY AT YOUR OWN RISK." You can modify the file, for example to update import paths when customizing your folder structure, but changes to the required Payload integration may break the Admin Panel.
