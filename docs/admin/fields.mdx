...
| **`_isAffectingData`** | A boolean indicating that the field directly affects data in the database. True when `_isPresentational: false` _and_ the field has a `name` property, i.e. a [Text Field](../fields/text) but not an unnamed [Row Field](../fields/row).
