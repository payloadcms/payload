...
| **`field`** | Either the raw field config or the client field config depending on whether this is a client or server component. [More details](#the-field-prop) |
| **`clientField`** | Server components will receive the client field config through this prop. [More details](#the-field-prop) |
...
All Field Components are passed their own Field Config through a common `field` prop. Within a Server Component, this is the raw Field Config. Within Client Components, however, the raw Field Config is [non-serializable](https://react.dev/reference/rsc/use-client#serializable-types). Instead, Client Components receives a [Client Config](./components#accessing-the-payload-config) in its place, which is a sanitized version of the Field Config that is safe to pass into Client Components.
...
export const MyServerField: TextFieldServerComponent = ({ payload, field, clientField }) => {
...
<Banner type="info">
  ...
  Server component still receive the client field config through a `clientField` prop.
</Banner>
