...
| **`_field`** | Server components receive the raw field config through this prop. [More details](#the-field-prop) |
...
All Field Components are passed their own Client Field Config through a common `field` prop. Because the raw Field Config is [non-serializable](https://react.dev/reference/rsc/use-client#serializable-types), it cannot be passed into Client Components without first transforming it into a [Client Config](./components#accessing-the-payload-config).

The Client Field Config is a sanitized version of the Field Config that is safe to pass into Client Components. The exact shape of this prop is unique to the specific [Field Type](../fields/overview) being rendered, minus all non-serializable properties. All functions, like field labels, are evaluated at this time, and all [Custom Components](../components) are resolved into a "mapped component".
...
export const MyServerField: TextFieldServerComponent = ({ field }) => {
...
<Banner type="info">
  <strong>Tip:</strong>
  Server Components can still access the raw Field Config through the `_field` prop.
</Banner>
