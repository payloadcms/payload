...
| Property         | Description                                                                                                                                                                                                                      |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`_fieldIsPresentational`** | A boolean indicating that the field is purely visual and does not directly affect data or change data shape, i.e. the [UI Field](../fields/ui). |
| **`_isFieldAffectingData`** | A boolean indicating that the field directly affects data in the database. True when `_fieldIsPresentational: false` _and_ the field has a `name` property, i.e. a [Text Field](../fields/text) but not an unnamed [Row Field](../fields/row).
| **`_isSidebar`** | A boolean that indicates whether `admin.position` is `sidebar`. |
