...
[Fields](../fields/overview) within the [Admin Panel](./overview) can be customized in their appearance and behavior without affecting the underlying data structure. From [Conditional Logic](#conditional-logic) to [Custom Field Components](#custom-field-components) and more, fields can be endlessly customized for your specific use case.
...
The `description` property is used to provide additional information to the editor about a field, such as special instructions. It's placement varies from field to field, but it is typically displayed beneath the field input in a subtle style.
