...
All Field Components receive the following default props:
...
| **`path`***       | The static path of the field at render time.                                                                                                                                                                                                            |                                                                                                    |
...
_\* The **`path`** prop is static, defined at render time. To use the dynamic path of the field, subscribe to it using the `useFieldProps` hook. [More details](./hooks#usefieldprops)._

<Banner type="warning">
  <strong>Note:</strong>
  Custom Fields also receive various other props that are specific [Field Type](../fields/overview) that is being rendered. For example, [Text Field](../fields/text) receives the `hasMany` prop. For a full list of available props, consult the documentation related to the specific field you are working with.
</Banner>
...
All Cell Components receive the following default props:

| Property         | Description                                                       |
| ---------------- | ----------------------------------------------------------------- |
| **`name`** | The name of the field. |
| **`cellData`** | The value of the field. |
| **`rowData`** | The data of the entire row, including the cell. |
| **`className`**  | The `admin.className` property defined in the [Field Config](../fields/overview). |
...
| **`customCellContext`** | An object that contains the context for the custom cell, such as `collectionSlug` or `uploadConfig`. |
...
The Label Component is rendered where a field needs to be represented by a label. This is typically used in the Edit View, but can also be used in the List View and elsewhere.
...
The `description` property is used to provide additional information to the editor about a field, such as special instructions. Its placement varies from field to field, but it is typically displayed beneath the field input with subtle style differences.
