...
For operations like update and delete, Payload includes an `overrideLock` option. This boolean flag, when set to `true`, allows you to bypass the document lock, ensuring that the operation proceeds even if another user currently holds the lock.

By default, `overrideLock` is set to `false`, which means that if a document is locked, the operation will not proceed and an error will be returned. When `overrideLock: true` is passed, the lock is ignored, and the update or delete operation is executed.
...
const result = await payload.update({
  ...
  overrideLock: true, // Allows the operation to bypass the document lock
})
```
