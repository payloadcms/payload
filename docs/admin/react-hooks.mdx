...
You can also trigger route transitions programmatically, such as when calling `router.push`. To do this, wrap your function calls with the `startRouteTransition` method provided by the `useRouteTransition` hook. Import `useRouter` from `@payloadcms/ui` (not `next/navigation`) so your component works on both the Next.js and TanStack Start adapters.

```ts
'use client'
import React, { useCallback } from 'react'
import { useRouter, useTransition } from '@payloadcms/ui'
