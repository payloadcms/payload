...
<Banner type="warning">
  **Reminder:** All Custom Components are [React Server
  ...
  client component](../custom-components/overview#client-components).
</Banner>
...
const MyComponent: React.FC = () => {
  ...
  return <p>The document's Form has ${Object.keys(parentDocumentFields).length} fields</p>
}
```
...
const MyComponent: React.FC = () => {
  ...
  return (
    <div>
      <h1>{data?.title}</h1>
      <button onClick={() => setParams({ cacheBust: Date.now() })}>Refresh Data</button>
    </div>
  )
}
```
