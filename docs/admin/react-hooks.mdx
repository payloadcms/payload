...
| **`setActiveColumns`** | A method to set specific columns to active state while preserving column order and the active state of columns not mentioned. Accepts an array of column names to activate |
| **`setColumns`** | A method to completely replace the column configuration. Accepts an array of column names that will become the new column list |
...
const MyComponent: React.FC = () => {
  // highlight-start
  const { setColumns, setActiveColumns, resetColumnsState } = useTableColumns()

  const replaceAllColumns = () => {
    // Completely replaces the column list with only id, createdAt, and updatedAt
    setColumns(['id', 'createdAt', 'updatedAt'])
  }
  ...
  return (
    <div>
      <button type="button" onClick={replaceAllColumns}>
        Replace All Columns
      </button>
