...
const GlobalView: React.FC<IndexProps> = (props) => {
  ...
  const { docPermissions, getDocPermissions, getDocPreferences, getVersions, preferencesKey } = useDocumentInfo();
  ...
  const onSave = useCallback(async (json) => {
    ...
    const state = await buildStateFromSchema({ data: json.result, fieldSchema: fields, locale, operation: 'update', preferences, t: t as any, user });
  ...
  useEffect(() => {
    const awaitInitialState = async () => {
      const preferences = await getDocPreferences();
      const state = await buildStateFromSchema({ data: dataToRender, fieldSchema: fields, locale, operation: 'update', preferences, t: t as any, user });
