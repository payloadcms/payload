...
import type { Context, Fields, Context as FormContextType, GetDataByPath, Props, Row, SubmitOptions } from './types.js';

import { isNumber } from '../../../../utilities/isNumber.js';
import { setsAreEqual } from '../../../../utilities/setsAreEqual.js';
import { splitPathByArrayFields } from '../../../../utilities/splitPathByArrayFields.js';
...
import { WatchFormErrors } from './WatchFormErrors.js';
import { buildFieldSchemaMap } from './buildFieldSchemaMap.js';
import buildInitialState from './buildInitialState.js';
import buildStateFromSchema from './buildStateFromSchema/index.js';
...
const Form: React.FC<Props> = (props) => {
  ...
  const addFieldRow: Context['addFieldRow'] = useCallback(async ({ data, path, rowIndex }) => {
    ...
    if (fieldConfig) {
      const subFieldState = await buildStateFromSchema({ data, fieldSchema: fieldConfig, id, locale, operation, preferences, t: t as any, user });
  ...
  const reset = useCallback(async (fieldSchema: Field[], data: unknown) => {
    const preferences = await getDocPreferences();
    const state = await buildStateFromSchema({ data, fieldSchema, id, locale, operation, preferences, t: t as any, user });
