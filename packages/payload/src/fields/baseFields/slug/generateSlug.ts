...
export const generateSlug =
  ({ slugFieldName = 'slug', useAsSlug }: HookArgs): FieldHook =>
  async (args) => {
    const { collection, global, operation, originalDoc, siblingData, value: isChecked } = args

    // Throughout this function, we can safely assume the `slug` field is the direct sibling of this checkbox
    // This allows us to make very fast lookups via `siblingData`, i.e. avoid using `getDataByPath`
    ...
    if (operation === 'create') {
      if (siblingData) {
        siblingData[slugFieldName] = slugify(
