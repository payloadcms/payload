...
async function loadPrettierConfig() {
  ...
  let actualConfigPath = prettierConfigPath

  if (prettierConfigPath) {
    if (prettierConfigPath.endsWith('package.json')) {
      // If config is inside package.json, read and parse it
      const pkgJson = JSON.parse(fs.readFileSync(prettierConfigPath, 'utf8'))
      if (pkgJson.prettier && typeof pkgJson.prettier === 'string') {
        // Resolve relative path from package.json location
        actualConfigPath = path.resolve(path.dirname(prettierConfigPath), pkgJson.prettier)
      } else {
        return await prettier.resolveConfig(prettierConfigPath)
      }
    }

    const ext = path.extname(actualConfigPath)

    if (ext === '.mjs') {
      prettierConfig = (await import(pathToFileURL(actualConfigPath).href)).default
    } else if (ext === '.cjs' || ext === '.js') {
      prettierConfig = await import(pathToFileURL(actualConfigPath).href)
    } else {
      prettierConfig = await prettier.resolveConfig(actualConfigPath)
    }
  }

  return prettierConfig || { singleQuote: true }
}
