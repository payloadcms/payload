 ...
 */ export const updateDocument = async ({ id, accessResults, autosave, collectionConfig, config, data, depth, docWithLocales, draftArg, fallbackLocale, filesToUpload, locale, overrideAccess, overrideLock, payload, populate, publishSpecificLocale, req, select, showHiddenFields })=>{
    ...
    const isSavingDraft = Boolean(draftArg && hasDraftsEnabled(collectionConfig)) && data._status !== 'published';
    ...
    const beforeChangeArgs = {
        ...
        overrideAccess,
        req,
        skipValidation: // only skip validation for drafts when draft validation is false
        isSavingDraft && !hasDraftValidationEnabled(collectionConfig) || // Skip validation for trash operations since they're just metadata updates
    ...
    if (config.localization && collectionConfig.versions) {
        if (publishSpecificLocale) {
            ...
            result = await beforeChange({
                ...beforeChangeArgs,
                docWithLocales: await getLatestCollectionVersion({
                    id,
