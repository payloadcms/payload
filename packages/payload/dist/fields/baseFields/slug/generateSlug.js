 ...
 */ export const generateSlug = ({ slugFieldName, slugify: customSlugify, useAsSlug })=>async ({ collection, data, global, operation, originalDoc, req, value: isChecked })=>{
        if (operation === 'create') {
            if (data) {
            ...
            if (!hasAutosaveEnabled(collection || global)) {
                ...
                if (!userOverride) {
                    if (data) {
                        ...
                        data[slugFieldName] = data?.[useAsSlug] ? slugify({
                            customSlugify,
                            data,
                            req,
                            valueToSlugify: data?.[useAsSlug]
