                    ...
                    if (validationResult?.invalidBlockSlugs?.length) {
                        ...
                        for (const block of siblingData[field.name]){
                            ...
                            if (validationResult.invalidBlockSlugs.includes(block.blockType)) {
                                ...
                                const blockLabelPath = field?.label === false ? fieldLabelPath : buildFieldLabel(fieldLabelPath, `${getTranslatedLabel(field?.label || field?.name, req.i18n)} > ${req.t('fields:block')} ${rowIndex + 1} (${getTranslatedLabel(blockConfig?.labels?.singular || block.blockType, req.i18n)})`);
                                errors.push({
        ...
        case 'blocks':
            {
                ...
                if (Array.isArray(rows)) {
                    ...
                    rows.forEach((row, rowIndex)=>{
                        ...
                        const blockLabelPath = field?.label === false ? fieldLabelPath : buildFieldLabel(fieldLabelPath, `${getTranslatedLabel(field?.label || field?.name, req.i18n)} > ${req.t('fields:block')} ${rowIndex + 1} (${getTranslatedLabel(block?.labels?.singular || blockTypeToMatch, req.i18n)})`);
                        if (block) {
                            promises.push(traverseFields({
                                id,
                                blockData: row,
        ...
        case 'group':
            {
                ...
                await traverseFields({
                    ...
                    docWithLocales,
                    errors,
                    fieldLabelPath: field?.label === false ? fieldLabelPath : buildFieldLabel(fieldLabelPath, getTranslatedLabel(fallbackLabel, req.i18n)),
                    fields: field.fields,
                    global,
                    mergeLocaleActions,
                    operation,
