 ...
 *      from `@payloadcms/ui`, passing a TanStack-bound `getAdminContext`. The injected
...
export async function loadAdminPage({
  ...
  // `renderRoot` calls `getAdminContext` itself with its own overrides (query
  ...
  const boundGetAdminContext: Parameters<typeof renderRoot>[0]['getAdminContext'] = async (
    args,
  ) => {
  ...
  try {
    const node = await renderRoot({
      ...
      config: Promise.resolve(config),
      getAdminContext: boundGetAdminContext,
      importMap,
      key: splat ?? '',
