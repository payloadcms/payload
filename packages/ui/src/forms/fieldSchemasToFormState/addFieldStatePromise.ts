            ...
            if (!block) {
              if (blockTypeToMatch == null) {
                ...
                req.payload.logger.warn({
                  msg: `Skipping malformed block row at "${schemaPath}.${i}": it has no "blockType". This usually indicates corrupt block data such as a gap in the persisted "_order" sequence.`,
                })
                return acc
              }

              throw new Error(
                `Block with type "${blockTypeToMatch}" was found in block data, but no block with that type is defined in the config for field with schema path ${schemaPath}.`,
              )
            }
