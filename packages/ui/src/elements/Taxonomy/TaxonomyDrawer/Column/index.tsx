...
const baseClass = 'taxonomy-drawer-column'
...
export const Column: React.FC<ColumnProps> = ({
  ...
  const collectionLabel = collectionConfig
    ? getTranslation(collectionConfig.labels?.singular || taxonomySlug, i18n)
    : taxonomySlug

  const canCreate = permissions?.collections?.[taxonomySlug]?.create

  const [DocumentDrawer, , { openDrawer }] = useDocumentDrawer({
    collectionSlug: taxonomySlug,
  })
