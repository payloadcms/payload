...
import type { SelectionWithPath } from '../TaxonomyDrawer/types.js'
...
import { useTaxonomyDrawer } from '../TaxonomyDrawer/index.js'
import { SelectedTaxonomies } from './SelectedTaxonomies.js'
import './index.scss'

const baseClass = 'taxonomy-field'

type Value = (number | string)[] | null | (number | string)

export type TaxonomyFieldClientProps = {
  Icon?: React.ReactNode
  parentFieldName?: string
} & RelationshipFieldClientProps

export const TaxonomyFieldClient: React.FC<TaxonomyFieldClientProps> = (props) => {
  const {
    field,
    field: {
  ...
  const taxonomySlug = Array.isArray(relationToProp) ? relationToProp[0] : relationToProp
  ...
  const [TaxonomyDrawer, , { openDrawer }] = useTaxonomyDrawer({
    Icon,
    parentFieldName,
    taxonomySlug,
    useAsTitle,
  ...
  const taxonomyLabel = collectionConfig?.labels?.plural
        ...
        <div className={`${baseClass}__content`}>
          ...
          {!readOnly && (
            <Button
              ...
              size="small"
            >
              {t('general:select')} {taxonomyLabel}
            </Button>
          )}
        </div>
        {AfterInput}
