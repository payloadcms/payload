...
export const buildColumnState = (args: Args): Column[] => {
  ...
  const sorted: Column[] = sortedFieldMap?.reduce((acc, clientField, columnIndex) => {
    ...
    if (isActive && !activeColumnsIndices.includes(columnIndex)) {
      activeColumnsIndices.push(columnIndex)
    }
      ...
      renderedCells: isActive
        ? docs.map((doc, rowIndex) => {
            return renderCell({
              ...
              enableRowSelections,
              i18n,
              isLinkedColumn: columnIndex === activeColumnsIndices[0],
              payload,
              rowIndex,
              serverField,
            })
