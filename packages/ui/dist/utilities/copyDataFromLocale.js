    ...
    if (fieldAffectsData(field)) {
      switch (field.type) {
        ...
        case 'blocks':
          ...
          [null, undefined].includes(toLocaleData[field.name]) || Array.isArray(toLocaleData[field.name]) && toLocaleData[field.name].length === 0) && field.name in fromLocaleData) {
            toLocaleData[field.name] = fromLocaleData[field.name];
            break;
          }
          // if the field has a value - loop over the data from target
        ...
        case 'group':
          {
            if (fieldAffectsData(field) && field.name in toLocaleData && fromLocaleData?.[field.name] !== undefined) {
