...
export const DuplicateDocument = ({
  ...
  const handleDuplicate = useCallback(async args => {
    ...
    try {
      const res = await requests.post(formatAdminURL({
        apiRoute,
  ...
  }, [adminRoute, apiRoute, collectionConfig, i18n, id, localeCode, onDuplicate, redirectAfterDuplicate, router, setModified, singularLabel, slug, startRouteTransition, t]);
