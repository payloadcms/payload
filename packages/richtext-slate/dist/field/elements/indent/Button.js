        ...
        if (dir === 'left') {
            if (isElementActive(editor, 'li')) {
                ...
                const matchedParentList = Editor.above(editor, {
                    at: listPath,
                    match: (n)=>!Editor.isEditor(n) && isBlockElement(editor, n)
                });
                if (matchedParentList) {
                    ...
                    if (parentListItem.children.length > 1) {
                        ...
                        Transforms.unwrapNodes(editor, {
                            ...
                            match: (node, path)=>{
                                const matches = !Editor.isEditor(node) && Element.isElement(node) && listTypes.includes(node.type) && path.length === parentListItemPath.length + 1;
                                return matches;
                            }
                        });
                        ...
                        Transforms.setNodes(editor, {
                            ...
                            at: parentListItemPath,
                            match: (node, path)=>{
                                const matches = !Editor.isEditor(node) && Element.isElement(node) && node.type !== 'li' && path.length === parentListItemPath.length + 1;
                                return matches;
                            }
                        });
                        ...
                        if (Element.isElement(newParentNode) && newParentNode.type === 'li') {
                            ...
                            Transforms.liftNodes(editor, {
                                at: parentListItemPath,
                                match: (node, path)=>{
                                    const matches = !Editor.isEditor(node) && Element.isElement(node) && path.length === parentListItemPath.length + 1 && node.type === 'li';
                                    return matches;
                                }
                            });
                        }
        ...
        if (dir === 'right') {
            ...
            if (isCurrentlyOL || isCurrentlyUL) {
                ...
                if (hasPrecedingLI) {
                    ...
                    const [precedingLIChildren] = Editor.node(editor, [
                        ...precedingLIPath,
                        0
                    ]);
