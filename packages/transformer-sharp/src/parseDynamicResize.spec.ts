...
describe('parseDynamicResize', () => {
  ...
  it.each([true, false])(
    'should parse a valid withoutEnlargement=%s alongside width',
    (withoutEnlargement) => {
      expect(parse(`width=500&withoutEnlargement=${withoutEnlargement}`)).toEqual({
        height: undefined,
        isRouted: true,
        valid: true,
        width: 500,
  ...
  it.each([
    ...
    'width=500&withoutEnlargement=true&withoutEnlargement=false',
  ])('should reject a repeated parameter: %s', (query) => {
    expect(parse(query)).toMatchObject({ isRouted: true, valid: false })
  })
