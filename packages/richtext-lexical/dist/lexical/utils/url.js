export function sanitizeUrl(url) {
  ...
  const DATA_URL_PATTERN = /^data:(?:image\/(?:bmp|gif|jpeg|jpg|png|tiff|webp)|video\/(?:mpeg|mp4|ogg|webm)|audio\/(?:mp3|oga|ogg|opus));base64,[a-z\d+/]+=*$/i;
  url = String(url).trim();
 ...
 * @param url
 */
export function validateUrlMinimal(url) {
  if (!url) {
    return false;
  }
  return !url.includes(' ');
