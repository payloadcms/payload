...
// This prevents the merge cells button from appearing when attempting to merge unmerged cells with
// already merged cells
function isTableSelectionValid(selection: TableSelection): boolean {
  ...
  for (let i = 0; i < nodes.length; i++) {
    ...
    if ($isTableCellNode(node)) {
      ...
      if (node.__colSpan > 1 || node.__rowSpan > 1) {
        return false
      }
    }
  }
  ...
  useEffect(() => {
    editor.getEditorState().read(() => {
      ...
      if ($isTableSelection(selection)) {
        ...
        setCanMergeCells(
          isTableSelectionValid(selection) &&
