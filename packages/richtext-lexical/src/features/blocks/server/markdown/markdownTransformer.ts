...
> | null {
  ...
  if (isInlineBlock) {
    toReturn.push(({ allNodes, allTransformers }) => {
      const enabledNodes = getEnabledNodesFromServerNodes({
        nodes: allNodes,
      })
      return {
        type: 'text-match',
        ...
        export: (node) => {
          ...
          const lexicalToMarkdown = getLexicalToMarkdown(enabledNodes, allTransformers)
        ...
        replace(node, match) {
          ...
          const markdownToLexical = getMarkdownToLexical(enabledNodes, allTransformers)
