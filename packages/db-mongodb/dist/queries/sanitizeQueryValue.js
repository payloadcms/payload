...
export const sanitizeQueryValue = ({ field, hasCustomID, locale, operator, parentIsLocalized, path, payload, val })=>{
    ...
    if ([
        ...
        if ([
            'equals',
            'not_equals'
        ].includes(operator) && Array.isArray(formattedValue) && 'hasMany' in field && field.hasMany) {
                ...
                formattedValue = formattedValue.map((item)=>{
                    if (typeof item === 'object' && 'value' in item) {
                        ...
                        if (customIDType === 'number') {
                            ...
                            return {
                                relationTo: relTo,
                                value: Number.isNaN(parsed) ? item.value : parsed
                            };
                        }
                        if (Types.ObjectId.isValid(item.value)) {
                            return {
                                relationTo: relTo,
                                value: new Types.ObjectId(item.value)
                            };
                        }
                        return item;
                    }
