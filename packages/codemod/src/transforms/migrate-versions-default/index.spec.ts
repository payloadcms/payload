...
import { migrateVersionsDefault } from './index.js'

const here = dirname(fileURLToPath(import.meta.url))
const fixture = (name: string) => readFile(join(here, name), 'utf8')

describe('migrate-versions-default', () => {
  it('adds versions: false to CollectionConfig objects without a versions property', async () => {
    const input = await fixture('basic.input.ts')
    const output = await fixture('basic.output.ts')

    const result = await runTransform({ source: input, transform: migrateVersionsDefault })
  ...
  it('is idempotent', async () => {
    ...
    const result = await runTransform({ source: output, transform: migrateVersionsDefault })

    expect(result).toBe(output)
  })

  it('does not modify collections that already have a versions property', async () => {
  ...
  it('adds versions: false to GlobalConfig objects without a versions property', async () => {
