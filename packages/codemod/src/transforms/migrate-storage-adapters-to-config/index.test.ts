...
describe('migrate-storage-adapters-to-config', () => {
  it('moves s3Storage from plugins to storageAdapters', async () => {
...
`
    ...
    expect(result).toContain('storageAdapters')
    expect(result).toContain("s3Storage({ bucket: 'a'")
    expect(result).toContain("gcsStorage({ bucket: 'b'")
  })

  it('no-ops when storageAdapters already contains the adapter (idempotency on partial migration)', async () => {
