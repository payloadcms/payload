...
- `add-override-access-true` — adds an explicit `overrideAccess: true` to Local API calls (`payload.find(...)`, `req.payload.create(...)`, and the rest) that omit it, preserving the Payload 3 default of skipping access control now that the property is required. Detection is syntactic so it works on JavaScript projects with no `tsconfig.json`. Deliberately never touches internal operations such as `findOperation(...)`, where a missing value meant `false` and inserting `true` would disable access control. Skips `resetPassword` (already required), calls whose argument object contains a spread, and non-Payload receivers. Review each inserted `true` and switch to `false` wherever the operation acts on behalf of a user.
