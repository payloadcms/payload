...
- `rename-disable-local-strategy` — renames the `auth.disableLocalStrategy` property to `auth.localStrategy` and inverts the value semantics: `true` becomes `localStrategy: false`, `false` is dropped (the new default is `localStrategy: true`), and the object form `{ enableFields, optionalPassword }` becomes `{ enabled: false, disableFields: false, optionalPassword: true }`.
