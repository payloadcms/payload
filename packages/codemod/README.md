...
- `migrate-next-generate-viewport-export` — adds `export { generateViewport }` to app router layout files that already import Payload's shared viewport helper, preserving Next.js viewport behavior without touching custom viewport implementations.
