...
- `migrate-field-component-types`: replaces concrete `*FieldClientComponent` and `*FieldServerComponent` imports from `payload` with the corresponding props types wrapped in `React.FC`. Explicit class components, re-exports, namespace references, and inline imports are left unchanged with notes for manual migration.
