...
- `remove-default-locale-publish-option` — removes `defaultLocalePublishOption` from the root `localization` config. The admin Publish button now always defaults to the active locale, with "Publish all locales" available via the secondary button; the option no longer has any effect.
