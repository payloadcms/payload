...
- `migrate-payload-request-creation`: renames request helpers and admin-context types from `payload` and `payload/internal`, and converts `createLocalReq(options, payload)` when the payload argument is a plain identifier; collisions, other payload expressions, and unsupported uses are left unchanged with notes for manual migration.
