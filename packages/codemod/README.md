...
- `migrate-version-action-api` — rewrites leftover `draft` operation options to `version` on reads and `action` on writes, migrates the removed all-locale publication flags to `locale: 'all'`, and removes `typescript.strictDraftTypes`. Automatic rewrites (only when the operation is statically identifiable):
  ...
  - Static `publishAllLocales: true` with `action: 'publish'` → `locale: 'all'`
  - Static `unpublishAllLocales: true` with `action: 'unpublish'` → `locale: 'all'`
  - An existing static locale is replaced with `'all'`; static false all-locale flags are removed
  - Equivalent unambiguous REST query params and GraphQL mutation arguments
  ...
  - Dynamic all-locale flags or locales, conflicting publication flags/actions, and all-locale flags without an explicit matching action
