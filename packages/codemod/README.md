...
- `migrate-deduped-exports` — rewrites imports of types and utilities that used to be re-exported from `@payloadcms/ui` and `@payloadcms/next/utilities` to their canonical sources in `payload` / `payload/shared`. Pass-through re-exports (`Column`, `ListViewSlots`, `ListViewClientProps`, `mergeListSearchAndWhere`, `EntityType`, `mergeHeaders`, `headersWithCors`, `createPayloadRequest`, `addDataAndFileToRequest`, `sanitizeLocales`, `addLocalesToRequestFromData`) move to `payload`/`payload/shared`. Renamed types (`ListPreferences` → `CollectionPreferences`, `ListComponentClientProps` → `ListViewClientProps`, `ListComponentServerProps` → `ListViewServerProps`) are imported with an `as` alias so existing usages keep compiling; rename usages manually to drop the alias.
