import type { DynamicTransformParseResult } from './types.js'
...
/**
 * Strictly parses the dynamic transformation query parameters (`width`, `height`,
 * `withoutEnlargement`) - deliberately the same surface `@payloadcms/transformer-sharp`
 * recognizes, since both share one file endpoint and a URL should not mean different
 * things depending on which transformer is installed. Cloudinary-specific settings
 * (crop, gravity, format, quality) are configured on the transformer, not per request.
 *
 * The presence of any recognized parameter routes the request - a malformed combination
 * is `valid: false` (mapped to `400`), not treated as an ordinary read. Every other query
 * key (`draft`, `depth`, `where`, `prefix`, …) is ignored.
 */
export function parseDynamicTransform({
  limits,
  searchParams,
}: {
  limits: { maxHeight: number; maxPixels: number; maxWidth: number }
  searchParams: URLSearchParams
}): DynamicTransformParseResult {
