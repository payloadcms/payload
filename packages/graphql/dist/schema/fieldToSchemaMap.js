...
export const fieldToSchemaMap = {
    ...
    collapsible: ({ collectionSlug, config, field, forceNullable, graphqlResult, newlyCreatedBlockType, objectTypeConfig, parentIsLocalized, parentName })=>field.fields.reduce((objectTypeConfigWithCollapsibleFields, subField)=>{
    ...
    group: ({ collectionSlug, config, field, forceNullable, graphqlResult, newlyCreatedBlockType, objectTypeConfig, parentIsLocalized, parentName })=>{
    ...
    join: ({ collectionSlug, field, graphqlResult, objectTypeConfig, parentName })=>{
        ...
        const joinType = {
            type: new GraphQLObjectType({
                ...
                fields: {
                    docs: {
                        type: new GraphQLNonNull(Array.isArray(field.collection) ? GraphQLJSON : new GraphQLList(new GraphQLNonNull(graphqlResult.collections[field.collection].graphQL.type)))
                    },
                    hasNextPage: {
            ...
            async resolve (parent, args, context, info) {
                ...
                const fullWhere = combineQueries(where, Array.isArray(targetField.relationTo) ? {
                    [field.on]: {
                        equals: {
    ...
    tabs: ({ collectionSlug, config, field, forceNullable, graphqlResult, newlyCreatedBlockType, objectTypeConfig, parentIsLocalized, parentName })=>field.tabs.reduce((tabSchema, tab)=>{
            if (tabHasName(tab)) {
    ...
    text: ({ field, forceNullable, objectTypeConfig, parentIsLocalized })=>({
            ...
            [formatName(field.name)]: formattedNameResolver({
                type: withNullableType({
                    type: field.hasMany === true ? new GraphQLList(new GraphQLNonNull(GraphQLString)) : GraphQLString,
                    field,
                    forceNullable,
                    parentIsLocalized
                }),
