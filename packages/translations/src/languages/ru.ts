...
export const ruTranslations: DefaultTranslationsObject = {
  ...
  error: {
    ...
    actionWillLockoutPreset: 'Это действие заблокирует вам доступ к этой предустановке.',
    attachmentContentInvalid: 'Содержимое вложения должно быть строкой или буфером',
    attachmentMissingContentAndPath: 'В вложении отсутствуют как содержимое, так и путь',
    attachmentMissingFilename: 'Отсутствует имя файла вложения',
    authenticationRequiredForExport:
      'Для создания экспортов требуется аутентификация пользователя.',
    autosaving: 'При автосохранении этого документа возникла проблема.',
    busboyMultipartError: 'Ошибка Busboy при разборе multipart-запроса',
    collectionSlugRequired: 'Требуется слаг коллекции',
    contentTypeExpectedJSON: 'Content-Type должен быть application/json',
    ...
    eitherCollectionOrGlobalRequired: 'Должно быть указано collection или globalSlug.',
    emailOrPasswordIncorrect: 'Указанный email или пароль неверен.',
    expectedResponseFromUploadHandler: 'Ожидается ответ от обработчика загрузки.',
    failedToFetchFromURL: 'Не удалось получить файл по указанному URL.',
    failedToResetLayout: 'Не удалось сбросить макет.',
    failedToSaveLayout: 'Не удалось сохранить макет.',
    fieldMustBeSpecified: 'Поле должно быть указано',
    fieldsNotInitialized: 'Поля не инициализированы.',
    filenameRequired: 'Требуется имя файла.',
    ...
    invalidFileURL: 'Недопустимый URL файла',
    invalidJSON: 'Недопустимый JSON',
    invalidJSONFormat: 'Недопустимый формат JSON',
    invalidRequestArgs: 'В запрос переданы недопустимые аргументы: {{args}}',
    joinFieldsNotAllowedInArraysBlocksOrGlobals:
      'Поля объединения не могут быть добавлены в массивы, блоки или глобальные поля.',
    ...
    missingIDOfVersionToRestore: 'Отсутствует ID версии для восстановления.',
    missingRequiredData: 'Отсутствуют необходимые данные.',
    noConnectionToDatabase: 'beginTransaction вызван без подключения к базе данных',
    noFileDataForImport: 'Не предоставлены данные файла для импорта',
    noFilesUploaded: 'Не было загружено ни одного файла.',
    noMatchedField: 'Не найдено подходящего поля для "{{label}}"',
    noPayloadProvided: 'Не предоставлен payload',
    ...
    orderableJoinsMustTargetSingleCollection:
      'Сортируемые объединения должны быть направлены на одну коллекцию',
    pastingFromURLNotEnabled: 'Вставка из URL не включена для этой коллекции.',
    ...
    redirectTargetNotAllowed: 'Цель перенаправления не разрешена.',
    relationshipFieldNotFound: 'Поле связи не найдено',
    relationshipNotFound: 'Связь не найдена',
    requestDataRequired: 'Требуются данные запроса.',
    requestNotEligibleForFileUpload: 'Запрос не подходит для загрузки файла',
    requestURLMissing: 'Отсутствует URL запроса.',
    restoringTitle:
    ...
    unexpectedArrayCollectionSlug: 'Неожиданный массив collectionSlug, должен быть указан parent',
    ...
    uploadConfigHandlersMissing: 'uploadConfig.handlers отсутствует для {{collection}}',
    uploadthingUnknownError: 'Ошибка при загрузке файла с uploadthing: Неизвестная ошибка',
    urlNotAllowed: 'Указанный URL недопустим.',
    ...
    userRequiredForImport: 'Для операций импорта требуется пользователь',
    validURLRequired: 'Требуется действительный URL.',
    valueMustBeUnique: 'Значение должно быть уникальным',
    vercelBlobUploadError: 'Ошибка маршрута загрузки клиента storage-vercel-blob',
