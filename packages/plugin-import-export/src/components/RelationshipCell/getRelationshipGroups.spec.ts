...
const summarize = ({ groups }: { groups: ReturnType<typeof group> }) =>
...
describe('getRelationshipGroups', () => {
  describe('monomorphic', () => {
    ...
    it('should collect a list of bare IDs into one group', () => {
      expect(summarize({ groups: group({ relationTo: 'posts', value: [1, 2] }) })).toEqual([
        'Posts: 1, 2',
      ])
    })
    ...
    it('should title a populated document using useAsTitle', () => {
      expect(
        summarize({
          groups: group({ relationTo: 'posts', value: { id: 3, title: 'The Wall' } }),
        }),
      ).toEqual(['Posts: The Wall'])
    })

    it('should fall back to untitled and ID for a document with no useAsTitle value', () => {
      expect(summarize({ groups: group({ relationTo: 'posts', value: { id: 3 } }) })).toEqual([
      ...
      expect(
        summarize({
          groups: group({
            ...
            value: { relationTo: 'posts', value: 3 },
          }),
        }),
      ).toEqual(['Posts: 3'])
    })
    ...
    it('should fall back to the collection slug when the collection has no labels', () => {
      expect(
        summarize({
          groups: group({
            relationTo: ['pages'],
            value: { relationTo: 'pages', value: 4 },
          }),
        }),
      ).toEqual(['pages: 4'])
    })
  ...
  describe('capping', () => {
    it('should not report remaining options when the group fits', () => {
      expect(summarize({ groups: group({ relationTo: 'posts', value: [1, 2, 3] }) })).toEqual([
        'Posts: 1, 2, 3',
      ])
    })

    it('should cap a group at three options and count the rest', () => {
      expect(summarize({ groups: group({ relationTo: 'posts', value: [1, 2, 3, 4, 5] }) })).toEqual(
        ['Posts: 1, 2, 3 (+2)'],
      )
    })
  ...
  describe('unresolvable values', () => {
    it('should render an object carrying no ID as JSON rather than [object Object]', () => {
      expect(
        summarize({ groups: group({ relationTo: 'posts', value: { slug: 'no-id-here' } }) }),
      ).toEqual(['Posts: {"slug":"no-id-here"}'])
    })
