...
- **Masking a table cell doesn't stop its content from affecting layout** — a mask paints over a cell after layout, but an auto-layout table still sizes its columns from the cell's real (masked-out) text. A non-deterministic value in a masked cell (a live timestamp, a generated ID) can in principle still shift that column's width by a pixel and reflow every other column in the row, failing the comparison intermittently even though nothing in the visible content changed. If a masked cell sits in a table, force `table-layout: fixed` (via `page.addStyleTag`) before the screenshot so column widths stop depending on cell content at all. The signature to watch for: an intermittent failure — sometimes clean, sometimes not — with a different pixel-diff count each time it fails, concentrated at column boundaries and in text right next to the masked cell. (`posts-list-view` in `test/admin/e2e/visual/e2e.spec.ts` masks `.cell-id` inside a `table-layout: auto` table and is the closest example of this shape in the repo — a flake there was investigated for this exact mechanism and it did _not_ reproduce across several fresh-reseed comparisons, so treat this pitfall as a real risk to design against, not a confirmed explanation for that specific incident.)
