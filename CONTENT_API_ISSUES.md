...
- DB Adapter: `/Users/gjablonski/Documents/GitHub/enterprise-plugins/packages/figma/`
...
2. **Query Mapping** (`convertPayloadWhereToContentAPI()`):
   ...
   - Payload's `id` field → Content API's `key` field
   - Example: `{ id: { equals: "..." } }` → `{ path: "key", operator: "equals", ... }`

3. **Response Mapping** (`unwrapDocument()`):
   - Content API's `key` → Payload's `id`
   - Example: `doc.key` → `id: doc.key`
...
- **Payload**: `limit ?? (usePagination ? 10 : 0)` → default 10 with pagination, 0 without
- **DB Adapters**: `if (limit === 0) { limit = undefined }` → omit LIMIT in SQL when 0
- **Content API**: `limit ?? 10` → default 10, then `if (limit > 0) { query.limit(limit) }`
