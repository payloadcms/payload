...
/**
 * Runs E2E tests with coverage collection enabled.
 * This is a wrapper around runE2E.ts that enables coverage instrumentation.
 */
...
if (!suiteName) {
  ...
  if (files.length !== totalFiles) {
    ...
    console.log(`\n\nExecuting all ${files.length} E2E tests with coverage...\n\n`)
  }
  ...
  console.log(`\n\nExecuting all ${allSuitesInFolder.length} E2E tests with coverage...\n\n`)
...
function executePlaywright(
  ...
  console.log(`Executing ${paths.join(', ')} with coverage...`)

  // Use coverage config
  const playwrightCfg = path.resolve(dirname, 'playwright.coverage.config.ts')
  ...
  const child = spawn('pnpm', spawnDevArgs, {
    ...
    env: {
      ...process.env,
      ENABLE_COVERAGE: 'true', // Pass coverage flag to dev server
    },
    stdio: 'inherit',
  })
