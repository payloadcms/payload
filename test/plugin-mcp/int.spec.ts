...
const getApiKey = async (
  ...
  const doc = await payload.create({
    ...
    data: {
      enableAPIKey: true,
      label: 'Test API Key',
      // @ts-expect-error - update is a valid property
...
describe('@payloadcms/plugin-mcp', () => {
  ...
  describe('defaultDepth: 0 edge case', () => {
    let payloadDepthZero: Payload
    let restClientDepthZero: NextRESTClient
    let userIdDepthZero: string

    beforeAll(async () => {
      const initialized = await initPayloadInt(dirname, 'config-depth-zero.ts')
      payloadDepthZero = initialized.payload
      restClientDepthZero = initialized.restClient

      const data = await restClientDepthZero
        .POST('/users/login', {
          body: JSON.stringify({
            email,
            password,
      ...
      // @ts-expect-error - data.user is a valid property
      userIdDepthZero = data.user.id
    })

    afterAll(async () => {
      await payloadDepthZero.destroy()
    })

    it('should verify defaultDepth is 0', () => {
      expect(payloadDepthZero.config.defaultDepth).toBe(0)
    })

    it('should handle API key authentication when defaultDepth is 0', async () => {
      const doc = await payloadDepthZero.create({
        collection: 'payload-mcp-api-keys',
        data: {
          enableAPIKey: true,
          label: 'Test API Key - depth 0',
          posts: { find: true, create: true },
          apiKey: randomUUID(),
          user: userIdDepthZero,
        },
      })

      const apiKey = doc.apiKey as string

      const response = await restClientDepthZero.POST('/mcp', {
        headers: {
          Authorization: `Bearer ${apiKey}`,
          Accept: 'application/json, text/event-stream',
          'Content-Type': 'application/json',
