...
> {
  ...
  // For Content API: No need for unique contentSystemId per suite
  // Tests run sequentially (--runInBand), just like other adapters
  // The adapter's init() will call /dev/clear-db when PAYLOAD_DROP_DATABASE=true
