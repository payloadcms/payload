...
describe('localizeStatus migration', () => {
  ...
  const describeMongo =
    process.env.PAYLOAD_DATABASE === 'mongodb' ? global.describe : global.describe.skip

  let mongoPayload: Payload

  describeMongo('MongoDB', () => {
    // eslint-disable-next-line jest/no-duplicate-hooks
    beforeAll(async () => {
      const result = await initPayloadInt(dirname, undefined, undefined, 'localizeStatus.config.ts')
      mongoPayload = result.payload
    }, 30000)

    // eslint-disable-next-line jest/no-duplicate-hooks
    afterAll(async () => {
      if (mongoPayload?.db && typeof mongoPayload.db.destroy === 'function') {
        await mongoPayload.db.destroy()
      }
    }, 30000)
    ...
    describe('MongoDB version status migration', () => {
      it('should migrate version._status from string to per-locale object', async () => {
        ...
        const post = await mongoPayload.create({
          collection: 'testMigrationPosts',
          data: { title: 'MongoDB Test Post' },
        })

        // Publish the post
        await mongoPayload.update({
          id: post.id,
          collection: 'testMigrationPosts',
        ...
        const connection = (mongoPayload.db as any).connection
      ...
      it('should handle publishedLocale when migrating', async () => {
        ...
        const article = await mongoPayload.create({
          collection: 'testMigrationArticles',
          data: { title: 'Article' },
          locale: 'en',
        })
