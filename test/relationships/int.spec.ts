  ...
  describe('Polymorphic Array Operations', () => {
    describe('polymorphic $append operations', () => {
      it('should append polymorphic relationships using $append', async () => {
        ...
        // Create object-writes document with initial polymorphic relationship
        const doc = await payload.create({
          collection: 'object-writes',
          data: {
            manyPoly: [
        ...
        expect(doc.manyPoly).toHaveLength(1)
        expect(doc.manyPoly[0]).toEqual({
          relationTo: 'movies',
          value: movie.id,
        })
        ...
        expect(result.manyPoly).toHaveLength(2)
        expect(result.manyPoly).toContainEqual({
          relationTo: 'movies',
          value: movie.id,
        })
        expect(result.manyPoly).toContainEqual({
      ...
      it('should prevent duplicates in polymorphic relationships with $append', async () => {
        ...
        // Create object-writes document with polymorphic relationship
        const doc = await payload.create({
          collection: 'object-writes',
          data: {
            manyPoly: [
        ...
        expect(result.manyPoly).toHaveLength(1) // Should still be 1, no duplicates
        expect(result.manyPoly[0]).toEqual({
          relationTo: 'movies',
          value: movie.id,
        })
      })
    ...
    describe('polymorphic $remove operations', () => {
      it('should remove polymorphic relationships using $remove', async () => {
        // Create movies
        const movie1 = await payload.create({
          collection: 'movies',
          data: { title: 'Test Movie 1' },
        })
        const movie2 = await payload.create({
          collection: 'movies',
          data: { title: 'Test Movie 2' },
        ...
        // Create object-writes document with multiple polymorphic relationships
        const doc = await payload.create({
          collection: 'object-writes',
          data: {
            manyPoly: [
        ...
        expect(doc.manyPoly).toHaveLength(2)
        ...
        expect(result.manyPoly).toHaveLength(1)
        expect(result.manyPoly[0]).toEqual({
          relationTo: 'movies',
          value: movie2.id,
        })
        ...
        const doc = await payload.create({
          ...
          data: {
            manyPoly: [
              ...
              { relationTo: 'movies', value: movie3.id },
            ],
          },
          depth: 0,
        })

        expect(doc.manyPoly).toHaveLength(3)
    ...
    it('should handle localized relationships with $append', async () => {
      ...
      // Create object-writes document with localized polymorphic relationships
      const doc = await payload.create({
        collection: 'object-writes',
        data: {
          manyPoly: [
      ...
      // Append relationship in different locale
      const result = await payload.db.updateOne({
        collection: 'object-writes',
        id: doc.id,
        data: {
    ...
    it('should handle localized relationships with $remove', async () => {
      // Create movies for testing
      const movie1 = await payload.create({
        collection: 'movies',
        data: { name: 'Movie 1' },
      ...
      // Create object-writes document with multiple localized polymorphic relationships
      const doc = await payload.create({
        collection: 'object-writes',
        data: {
          manyPoly: [
      ...
      expect(result.manyPoly).not.toContainEqual({
        relationTo: 'movies',
        value: movie1.id,
      })
      expect(result.manyPoly).not.toContainEqual({
