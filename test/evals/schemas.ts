...
// Number fields intentionally omit .min()/.max(): Anthropic's structured-output
// schema rejects `minimum`/`maximum`. The 0–1 range is conveyed via .describe()
// and enforced by clamping where the value is consumed (see scoreConfigChange).

export const AnswerSchema = z.object({
  answer: z.string().describe('The answer or generated code'),
  confidence: z.number().describe('Confidence score from 0 (unsure) to 1 (certain)'),
})
