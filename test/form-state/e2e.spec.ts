...
test.describe('Form State', () => {
  ...
  test('should render computed values after save', async () => {
    ...
    await page.locator('#field-computedArray #array-row-0').isVisible()

    await removeArrayRow(page, { fieldName: 'computedArray' })

    await page.locator('#field-computedArray .array-row-0').isHidden()

    await saveDocAndAssert(page)

    await expect(page.locator('#field-computedArray #array-row-0')).toBeVisible()

    await expect(
      page.locator('#field-computedArray #array-row-0 #field-computedArray__0__customTextField'),
    ).toHaveValue('This is a computed value.')

    await expect(
      page.locator('#field-computedArray #array-row-0 #field-computedArray__0__defaultTextField'),
    ).toHaveValue('This is a computed value.')
  })
