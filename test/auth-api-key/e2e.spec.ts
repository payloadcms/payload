...
test.describe('API key fields', () => {
  test('should show only the masked value for an existing API key', async () => {
    ...
    await expect(apiKeyLocator).toHaveValue('•'.repeat(24))
  ...
  test('should generate an API key locally during create and persist it on save', async () => {
    ...
    await expect(page.locator('.field-description-apiKey')).toHaveText(
  ...
  test('should generate an API key when enableAPIKey is false', async () => {
    const disabledAPIKeyUser = await payload.create({
      collection: apiKeysSlug,
      data: {
        name: 'Disabled API key user',
        enableAPIKey: false,
      },
    })

    await page.goto(url.edit(disabledAPIKeyUser.id))
    await page.locator('#generate-api-key').click()
    ...
    await expect(async () => {
      ...
      expect(response.user?.id).toBe(disabledAPIKeyUser.id)
    }).toPass({ timeout: POLL_TOPASS_TIMEOUT })
  })
  ...
  test('should regenerate and revoke an API key', async () => {
    ...
    const regenerateButton = page.locator('.api-key__toggle-button-wrap #regenerate-api-key')
    const revokeButton = page.locator('#revoke-api-key')

    await regenerateButton.click()
    await expect(page.getByText('invalidate the previous key', { exact: false })).toBeVisible()
    ...
    await expect(
      page.getByText(`revoke the API key for user ${user.id}`, { exact: false }),
    ).toBeVisible()
    await expect(page.locator('.confirmation-modal strong')).toHaveText(String(user.id))
    await expect(
      page.getByText('The key will no longer be usable, and this action cannot be undone.'),
    ).toBeVisible()
    await expect(apiKeyLocator).not.toHaveValue('')
    await page.locator('#confirm-action').click()
  ...
  test('should apply and remove the field highlight when regenerating and revoking a key', async () => {
    ...
    await expect(apiKeyInputWrap).toHaveClass(/api-key__input-wrap--highlighted/)
    ...
    await expect(
      page
        ...
        .filter({ hasText: 'API key revoked successfully.' }),
    ).toHaveText('API key revoked successfully.')
    await expect(apiKeyInputWrap).not.toHaveClass(/api-key__input-wrap--highlighted/)
  })
})
...
test.describe('revealable keys', () => {
  ...
  test('should allow admins to manage API keys when custom access permits it', async () => {
    ...
    await page.locator('#generate-api-key').click()
    await expect(page.getByText(`user ${revealableKey.id}`, { exact: false })).toBeVisible()
    await expect(page.locator('.confirmation-modal strong')).toHaveText(String(revealableKey.id))
    await page.locator('#confirm-action').click()
  ...
  test('should reveal a stored key when configured', async () => {
    const apiKey = uuid()
    const revealableKey = await payload.create({
    ...
    await expect(page.locator('.field-description-apiKey')).toHaveCount(0)
    const revealButton = page.locator('.api-key__toggle-button-wrap #reveal-api-key')

    await expect(revealButton).toBeVisible()
    let releaseRevealRequest: (() => void) | undefined
    await page.route(
      `**/${revealableKeysSlug}/${revealableKey.id}/api-key/reveal`,
      async (route) => {
        await new Promise<void>((resolve) => {
          releaseRevealRequest = resolve
        })
    ...
    await expect(revealButton).toHaveAttribute('aria-busy', 'true')
    await expect(revealButton.locator('.spinner')).toBeVisible()
    releaseRevealRequest?.()
    ...
    await expect(page.locator('.field-description-apiKey')).toHaveCount(0)
    await page.unroute(`**/${revealableKeysSlug}/${revealableKey.id}/api-key/reveal`)
  })

  test('should not show a persisted key when only enableAPIKey is true', async () => {
    const revealableKey = await payload.create({
      collection: revealableKeysSlug,
      data: {
        enableAPIKey: true,
