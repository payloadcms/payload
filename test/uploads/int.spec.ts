...
test.suite({ config: './config.ts', resetBetweenTests: false })('Collections - Uploads', () => {
  ...
  test.describe('req.file preservation across nested Local API calls', () => {
    test('should preserve req.file when a hook calls the Local API with the original req', async ({
      payload,
    }) => {
      const filePath = path.resolve(dirname, './image.png')
      const file = await getFileByPath(filePath)
