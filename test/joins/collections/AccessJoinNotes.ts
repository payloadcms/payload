...
export const AccessJoinNotes: CollectionConfig = {
  ...
  access: {
    read: ({ req }) => {
      ...
      if (req.context.useOperatorHandlerAccessConstraint) {
        return { title: { not_like: 'nino' } }
      }

      if (req.context.useSystemIDOperatorHandlerAccessConstraint) {
        return { id: { not_equals: req.context.excludedChildID } }
      }
