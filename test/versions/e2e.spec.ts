...
describe('Versions', () => {
  ...
  describe('Collections with draft validation', () => {
    ...
    test('- can save drafts with validation errors and continue editing without being shown publishing validation', async () => {
      // Use draftCollectionSlug which has required title field but no validation enabled on draft saving
      await page.goto(url.create)
      ...
      await wait(2000) // Wait 2 seconds for debounced validation to potentially trigger

      // Verify no validation errors appear even after waiting
      await expect(page.locator('.field-error')).toHaveCount(0)
