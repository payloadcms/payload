  ...
  upload: joi.alternatives().try(
    joi.object({
      ...
      resizeOption: joi.object().keys({
        ...
        kernel: joi.string().allow(null),
        withoutEnlargement: joi.bool().allow(null),
        fastShrinkOnLoad: joi.bool().allow(null),
      }).allow(null),
      formatOption: joi.array().items(joi.string(), joi.object()),
    }),
    joi.boolean(),
  ),
});
