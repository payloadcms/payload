...
async function create(incomingArgs: Arguments): Promise<Document> {
  ...
  if (!collectionConfig.upload.disableLocalStorage) {
    await uploadFiles(payload, filesToUpload, req.i18n);
  }
  ...
  if (collectionConfig.auth && !collectionConfig.auth.disableLocalStrategy) {
    ...
    try {
      ...
      if (error.name === 'UserExistsError') {
        throw new ValidationError([{ message: error.message, field: 'email' }], req.i18n);
      }
      throw error;
    }
  } else {
    try {
      ...
      throw error.code === 11000 && error.keyValue
        ? new ValidationError([{ message: req.i18n.t('error:valueMustBeUnique'), field: Object.keys(error.keyValue)[0] }], req.i18n)
        : error;
    }
  }
