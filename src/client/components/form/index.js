...
class Form extends Component {
	...
	submit = e => {
		...
		Object.keys(this.childRefs).forEach((field, i) => {
		...
		// If form is AJAX, fetch data
			...
			Object.keys(this.state.fields).forEach((field, i) => {
			...
			api.requests[this.props.method](this.props.action, data).then(
				res => {
		...
		let children = React.Children.map(this.props.children, child => {
			...
			if (typeof(child.type) === 'function') {
				...
				return React.cloneElement(child, {
					ref: (el) => this.childRefs[child.props.id] = el,
