...
export const text: Validate<unknown, unknown, TextField> = (
  value: string,
  { t, minLength, maxLength: fieldMaxLength, required, payload },
) => {
...
export const password: Validate<unknown, unknown, TextField> = (
  value: string,
  { t, required, maxLength: fieldMaxLength, minLength, payload },
) => {
  let maxLength: number;
...
export const json: Validate<
  unknown,
  unknown,
  JSONField & { jsonError?: string }
> = (value: string, { t, required, jsonError }) => {
  if (required && !value) {
    return t('validation:required');
  }
...
const validateFilterOptions: Validate = async (
  value,
  { t, filterOptions, id, user, data, siblingData, relationTo, payload },
) => {
