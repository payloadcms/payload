...
```ts
import { migrations } from './migrations' // migrations/index.ts is generated by migrate:create
import { postgresAdapter } from '@payloadcms/db-postgres'
...
### Rename a field (two-migration pattern)

Renaming a field safely requires **two migrations**. In Migration 1, both the old field (`body`) and the new field (`content`) are present in the config and schema simultaneously — so you can read from `body` and write to `content`. Only after Migration 1 succeeds in production do you remove `body` from the config and write Migration 2 to drop the column.

**Removing `body` from the config before backfilling will lose data** — `doc.body` will be undefined once the field is gone from the schema.

**Migration 1: add `content`, backfill from `body` while both fields exist**

```ts
// see test/database/up-down-migration/migrations/ for real examples
import type { MigrateUpArgs } from '@payloadcms/db-postgres'

// At this point: both 'body' (old) and 'content' (new) are in the config.
export async function up({ payload, req }: MigrateUpArgs): Promise<void> {
  const limit = 100
  let page = 1
  ...
  while (true) {
    ...
    for (const doc of docs) {
      if (doc.content || !doc.body) continue // already migrated or no value to copy
      await payload.update({
        collection: 'posts',
        id: doc.id,
...
After this migration succeeds in production, remove the `body` field from your collection config and write a second migration to drop the column. Removing `body` from the config before backfilling will lose data.

**Migration 2: drop the old `body` column (after `body` is removed from config)**
...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`ALTER TABLE posts DROP COLUMN IF EXISTS body`)
}
```
...
For collections with many documents, paginate to avoid memory pressure. The [rename example above](#rename-a-field-two-migration-pattern) already uses this pattern — apply it to any bulk update migration:
  ...
  while (hasMore) {
    ...
    for (const doc of docs) {
      await payload.update({
        collection: 'posts',
        id: doc.id,
        data: { slug: slugify(doc.title) }, // your transformation here
        req,
      })
    }
